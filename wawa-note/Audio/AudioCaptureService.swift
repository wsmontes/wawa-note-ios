...
final class AudioCaptureService: ObservableObject, @unchecked Sendable {
  ...
  var captureInputPortType: String {
    sessionManager.bestAvailableInput?.portType.rawValue ?? "unknown"
  }
  ...
  var onSegmentCreated:
    ((_ closedInfo: ClosedSegmentInfo?, _ newSegment: RecordingSegment) -> Void)?
  ...
  init(fileWriter: AudioFileWriter = AudioFileWriter()) {
    ...
    fileWriter.onWriteFailure = { [weak self] error in
      DispatchQueue.main.async {
        ...
        self.stopRecording()
        AppLog.error(
          "audio", "Recording terminated by write failure: \(error.localizedDescription)")
      }
    }
  }
  ...
  private func observeNotifications() {
    ...
    observers.append(
      nc.addObserver(forName: AVAudioSession.interruptionNotification, object: nil, queue: q) {
        [weak self] n in self?.handleInterruption(n)
      })
    observers.append(
      nc.addObserver(
        forName: AVAudioSession.mediaServicesWereResetNotification, object: nil, queue: q
    ...
    observers.append(
      nc.addObserver(forName: AVAudioSession.routeChangeNotification, object: nil, queue: q) {
    ...
    if let eng = engine {
      observers.append(
        nc.addObserver(forName: .AVAudioEngineConfigurationChange, object: eng, queue: q) {
          [weak self] n in self?.handleEngineConfigChange(n)
        })
    }
  }
            ...
            do {
              ...
              if attempt < 2 {
                ...
                AppLog.audio.warning(
                  "Interruption resume attempt \(attempt + 1) failed — retrying in \(delayNs / 1_000_000)ms"
                )
  ...
  private func handleRouteChange(_ n: Notification) {
    ...
    AppLog.audio.info(
      "Route change: \(reason.rawValue) — input: \(self.sessionManager.currentInputPortName)")
    ...
    case .oldDeviceUnavailable:
      ...
      if sessionManager.isInputAvailable {
        ...
        Task { @MainActor [weak self] in
          await self?.rebuildEngineForCurrentRoute(
            forceBuiltInMic: false, reason: "oldDeviceUnavailable")
        }
      } else {
    ...
    case .newDeviceAvailable:
      ...
      Task { @MainActor [weak self] in
        await self?.rebuildEngineForCurrentRoute(
          forceBuiltInMic: false, reason: "newDeviceAvailable")
      }
    case .override:
  ...
  private func _rebuildEngineForCurrentRoute(forceBuiltInMic: Bool, reason: String) async {
    ...
    if let closed = closedInfo {
      onSegmentClosed?(closed)
      AppLog.audio.info(
        "rebuildEngine(\(reason)): segment \(closed.index) checkpointed — \(closed.fileName) \(closed.fileSize) bytes"
      )
    }
    ...
    do {
      ...
      AppLog.audio.error(
        "rebuildEngine(\(reason)): session reconfigure failed — \(error.localizedDescription)")
      stopRecording()
      return
    }
    ...
    if forceBuiltInMic {
      if let builtIn = (sessionManager.session.availableInputs ?? []).first(where: {
        ...
        do {
          ...
          AppLog.audio.warning(
            "rebuildEngine(\(reason)): setPreferredInput failed — \(error.localizedDescription)")
        }
      }
    }
    ...
    do {
      ...
      AppLog.audio.error(
        "rebuildEngine(\(reason)): failed to open new segment — \(error.localizedDescription)")
      stopRecording()
      return
    }
    ...
    AppLog.event(
      "audio",
      "rebuildEngine(\(reason)): recording continued on \(portName) (\(portType)) @ \(Int(sampleRate))Hz"
    )
  }
  ...
  private func _rebuildEngineLightweight(reason: String) async {
    ...
    AppLog.audio.info(
      "rebuildEngineLightweight(\(reason)): replacing engine (session stays active)")
    ...
    reRegisterEngineObserver(for: self.engine!)
    AppLog.event(
      "audio", "rebuildEngineLightweight(\(reason)): engine replaced — \(currentInputPortName)")
  }
  ...
  private func buildAndStartEngine(reason: String) -> Bool {
    ...
    do {
      ...
      AppLog.audio.error(
        "buildAndStartEngine(\(reason)): engine start failed — \(error.localizedDescription)")
      inputNode.removeTap(onBus: 0)
      engine.stop()
      engine.reset()
      self.engine = nil
  ...
  func attemptResume(forceRecording: Bool = false) async {
    guard state == .paused else {
      if forceRecording && (state == .recording || state == .paused) {
        AppLog.audio.warning(
          "attemptResume: forceRecording but already active (state=\(String(describing: self.state)))"
        )
      }
      return
    }
  ...
  private func reRegisterEngineObserver(for newEngine: AVAudioEngine) {
    ...
    observers.append(
      nc.addObserver(forName: .AVAudioEngineConfigurationChange, object: newEngine, queue: q) {
        [weak self] n in self?.handleEngineConfigChange(n)
      })
  }
}
