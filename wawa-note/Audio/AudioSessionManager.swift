...
final class AudioSessionManager {
  ...
  var isBluetoothWithoutMic: Bool {
    ...
    let availableWithMic = (session.availableInputs ?? []).contains { avail in
      (avail.portType == .bluetoothHFP || avail.portType == .bluetoothA2DP
        || avail.portType == .bluetoothLE)
  ...
  func selectBestInputForRecording() {
    ...
    if let preferred {
      do {
        ...
        AppLog.audio.error(
          "setPreferredInput failed: \(error.localizedDescription) | all=\(self.availableInputsDescription)"
        )
      }
    } else {
      AppLog.audio.error(
        "No valid recording input. availableInputs=\(self.availableInputsDescription)")
    }
  }
  ...
  func adaptToRouteChange() {
    ...
    do {
      ...
      AppLog.error(
        "audio",
        "Failed to adapt session to route change: \(error.localizedDescription) \(snapshot)")
    }
  }
  ...
  var availableInputsDescription: String {
    ...
    let viable = viableInputs(from: candidates)
    let ranked = rankedInputs(from: viable)
    return ranked.enumerated().map {
      "\($0.offset == 0 ? "★" : "·") \($0.element.portName)(\($0.element.portType.rawValue))"
    }.joined(separator: ", ")
  }

  /// Filter to viable recording inputs (has channels, not music-only Bluetooth).
  private func viableInputs(from inputs: [AVAudioSessionPortDescription])
  ...
  private func rankedInputs(from inputs: [AVAudioSessionPortDescription])
    -> [AVAudioSessionPortDescription]
  {
  ...
  var routeDescription: String {
    ...
    let outputs = session.currentRoute.outputs.map { "\($0.portName)(\($0.portType.rawValue))" }
      .joined(separator: ",")
    return
      "inputs=[\(inputs.isEmpty ? "none" : inputs)] outputs=[\(outputs.isEmpty ? "none" : outputs)]"
  }
