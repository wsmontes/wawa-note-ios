...
final class AudioFileWriter: @unchecked Sendable {
  ...
  func writeCheckpoint(meetingId: UUID, segmentIndex: Int, format: AVAudioFormat) {
    queue.async { [weak self] in
      ...
      do {
        ...
        AppLog.error(
          "audio", "Checkpoint: cannot create configs directory — \(error.localizedDescription)")
        return
      }

      let url = self.fileStore.configsDirectoryURL().appendingPathComponent(
        "recording_checkpoint.json")
      do {
  ...
  static func loadCrashCheckpoint(fileStore: FileArtifactStore = FileArtifactStore()) -> (
    UUID, Int, Double
  )? {
    ...
    for candidateURL in [url, bakURL] {
      ...
      guard Date().timeIntervalSince1970 - timestamp < 3600 else {
        AppLog.audio.info(
          "Checkpoint expired: \(Int((Date().timeIntervalSince1970 - timestamp) / 60))min old — discarding"
        )
        return nil
      }
    ...
    for u in [url, bakURL, newURL] {
      if FileManager.default.fileExists(atPath: u.path) {
        do {
          ...
          AppLog.warn(
            "audio",
            "clearCrashCheckpoint: cannot remove \(u.lastPathComponent) — \(error.localizedDescription)"
          )
        }
      }
    }
  ...
  private func isDiskFullError(_ error: Error) -> Bool {
    ...
    if nsError.domain == AVFoundationErrorDomain && nsError.code == -11837 /* disk full */ {
      return true
    }
    return false
  }
