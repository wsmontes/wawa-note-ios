...
final class LiveOCRViewModel: ObservableObject {
    ...
    func startScanning() {
        ...
        videoOutput.setSampleBufferDelegate(delegate, queue: DispatchQueue(label: "ocr.queue", qos: .userInitiated))
    ...
    private func startMotion() {
        ...
        motionManager.startDeviceMotionUpdates(to: .main) { [weak self] m, _ in
            ...
            let delta = sqrt(pow(accel.x - self.lastAccel.x, 2) + pow(accel.y - self.lastAccel.y, 2) + pow(accel.z - self.lastAccel.z, 2))
            self.lastAccel = CMAcceleration(x: accel.x, y: accel.y, z: accel.z)
            let new: MotionState = delta < self.stableG ? .stable : delta < self.shiftG ? .panning : .shifting
    ...
    private func processFrame(_ observations: [VNRecognizedTextObservation]) {
        ...
        let candidates: [TrackedRegion] = observations.compactMap { obs in
            ...
            return TrackedRegion(normalizedRect: obs.boundingBox, text: text, confidence: top.confidence, lastSeenFrame: frameCount, stabilityCount: 1)
        }
    ...
    func saveAsKnowledgeItem(title: String?, context: ModelContext) -> KnowledgeItem? {
        ...
        guard let item = try? svc.createItem(type: .note, title: t, bodyText: body, tags: ["ocr", "live-scan"], inboxDate: Date()) else { return nil }
