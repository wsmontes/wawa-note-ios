    ...
    VStack(alignment: .leading, spacing: 6) {
      HStack(alignment: .top) {
        VStack(alignment: .leading, spacing: 4) {
          ...
          // Stale indicator
          if task.statusRaw == TaskStatus.todo.rawValue,
            task.createdAt.timeIntervalSinceNow < -14 * 86400
          {
            Text("Stale — \(Int(-task.createdAt.timeIntervalSinceNow / 86400))d").font(
              .system(size: 8)
        ...
        .opacity(
          task.statusRaw == TaskStatus.todo.rawValue
            && task.createdAt.timeIntervalSinceNow < -14 * 86400 ? 0.6 : 1.0)
        Spacer()
        Menu {
