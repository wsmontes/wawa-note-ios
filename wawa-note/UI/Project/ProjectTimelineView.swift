...
struct TimelineCluster: Identifiable {
  ...
  var actionCount: Int {
    events.filter { !$0.actionItems.isEmpty }.count + events.filter { $0.kind == .task }.count
  }
...
struct ProjectTimelineView: View {
        ...
        HStack(spacing: AppSpacing.xs) {
          ForEach(TimelineEventKind.allCases, id: \.rawValue) { kind in
            Button {
              ...
              .background(
                selectedKinds.contains(kind) ? kind.color.opacity(0.15) : Color(.tertiarySystemFill)
              )
    ...
    HStack(alignment: .top, spacing: 10) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        HStack(spacing: 4) {
          ...
          Spacer()
          Text(event.date.formatted(date: .omitted, time: .shortened)).font(.caption2)
        ...
        if !event.connectedTo.isEmpty {
          HStack(spacing: 4) {
            ...
            Text("\(event.connectedTo.count) connection\(event.connectedTo.count > 1 ? "s" : "")")
  ...
  private func loadTimeline() {
    ...
    let itemDescriptor = FetchDescriptor<KnowledgeItem>(
    ...
    if let items = try? ctx.fetch(itemDescriptor) {
      for item in items {
        ...
        if let analysis = try? FileArtifactStore().readArtifact(
      ...
      for task in tasks {
        allEvents.append(
          TimelineEvent(
            ...
            subtitle: "Task · \(task.status.rawValue.capitalized)", date: task.createdAt,
            kind: .task, sourceItemID: task.sourceItemID))
        if task.status == .done {
          allEvents.append(
            TimelineEvent(
    ...
    self.clusters = grouped.map { start, evts in
      ...
      return TimelineCluster(
        weekStart: start, label: label, events: evts.sorted { $0.date > $1.date })
    ...
    if let allEdges = try? edgeSvc.recentEdges(limit: 500) {
      self.connectors = allEdges.compactMap { edge in
        ...
        return TimelineConnector(
          fromEventID: edge.fromID, toEventID: edge.toID, edgeType: edge.edgeType)
      }
    }
