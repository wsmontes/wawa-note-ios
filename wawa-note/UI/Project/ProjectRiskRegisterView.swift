    ...
    VStack(spacing: 0) {
      if isLoading {
        ...
        VStack(spacing: AppSpacing.md) {
          ...
          Text("Risks are identified when items are analyzed.").font(.caption).foregroundStyle(
            .secondary)
        }
        Spacer()
      } else {
  ...
  private func riskRow(_ r: RiskItem) -> some View {
    VStack(alignment: .leading, spacing: AppSpacing.xs) {
      HStack(spacing: AppSpacing.sm) {
        Image(systemName: "exclamationmark.shield.fill").font(.caption).foregroundStyle(
      ...
      HStack(spacing: AppSpacing.sm) {
        Label(r.sourceItemTitle, systemImage: "doc.text").font(.caption2).foregroundStyle(.tertiary)
          .lineLimit(1)
        Spacer()
        Text(r.sourceItemDate.formatted(date: .abbreviated, time: .omitted)).font(.caption2)
