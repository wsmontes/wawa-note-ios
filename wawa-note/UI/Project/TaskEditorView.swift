...
struct TaskEditorView: View {
  ...
  init(mode: Mode) {
    ...
    case .edit(let task):
      ...
      _priority = State(
        initialValue: TaskPriority(rawValue: task.priorityRaw ?? "medium") ?? .medium)
  ...
  private func save() {
    ...
    case .create(let projectID):
      ...
      try? service.createTask(
        ...
        priority: priority,
        ownerName: ownerName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
          ? nil : ownerName.trimmingCharacters(in: .whitespacesAndNewlines),
