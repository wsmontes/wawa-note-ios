    ...
    VStack(spacing: 0) {
      if isLoading {
        ...
        VStack(spacing: AppSpacing.md) {
          ...
          Text("Decisions are extracted when items are analyzed.").font(.caption).foregroundStyle(
            .secondary)
        }
        Spacer()
      } else {
        ...
        Menu {
          ForEach([0.0, 0.5, 0.7, 0.9], id: \.self) { threshold in
            Button {
              ...
              Label(
                threshold == 0 ? "All confidence" : "≥ \(Int(threshold * 100))%",
                systemImage: minConfidence == threshold ? "checkmark" : "")
            }
          }
        } label: {
    ...
    VStack(alignment: .leading, spacing: AppSpacing.xs) {
      ...
      HStack(spacing: AppSpacing.sm) {
        Label(d.sourceItemTitle, systemImage: "doc.text").font(.caption2).foregroundStyle(.tertiary)
          .lineLimit(1)
        Spacer()
        Text(d.sourceItemDate.formatted(date: .abbreviated, time: .omitted)).font(.caption2)
