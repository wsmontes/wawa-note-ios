...
struct PromoteToProjectSheet: View {
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 16) {
        ...
        if !preview.tasks.isEmpty {
          sectionHeader(
            "Tasks (\(selectedTaskIDs.count)/\(preview.tasks.count))", icon: "checklist")
          VStack(spacing: 0) {
        ...
        if !preview.people.isEmpty {
          sectionHeader(
            "People (\(selectedPersonIDs.count)/\(preview.people.count))", icon: "person.2")
          VStack(spacing: 0) {
        ...
        if !preview.entities.isEmpty {
          sectionHeader(
            "Entities (\(selectedEntityIDs.count)/\(preview.entities.count))", icon: "cube")
          VStack(spacing: 0) {
        ...
        if !preview.edges.isEmpty {
          sectionHeader(
            "Relationships (\(selectedEdgeIDs.count)/\(preview.edges.count))", icon: "link")
          VStack(spacing: 0) {
  ...
  private func selectableRow(
    isSelected: Bool, onToggle: @escaping () -> Void, @ViewBuilder content: () -> some View
  ) -> some View {
    HStack(spacing: 10) {
  ...
  private func sectionHeader(_ title: String, icon: String) -> some View {
    HStack(spacing: 6) {
      ...
      Text(title).font(.footnote).fontWeight(.semibold).foregroundStyle(.secondary).textCase(
        .uppercase)
    }
    .padding(.horizontal, AppSpacing.lg)
    .padding(.top, AppSpacing.sm)
  ...
  private func generatePreview() async {
    ...
    do {
      ...
      let preview = try await makeService().generatePreview(
        from: item, using: provider, model: model)
