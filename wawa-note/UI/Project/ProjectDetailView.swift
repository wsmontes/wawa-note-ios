  ...
  private func resolveIfNeeded() {
    ...
    if project == nil {
      AppLog.general.warning(
        "ProjectDetailLink: project not found for id \(projectID.uuidString.prefix(8))")
    }
  }
}
...
struct ProjectHomeView: View {
  ...
  var body: some View {
    ...
    .fileImporter(
      ...
      allowedContentTypes: [.json, .plainText, .pdf, .html, .rtf, .audio, .image]
    ) { result in
...
private struct HealthRing: View {
  ...
  var body: some View {
    let color: Color =
      status == "healthy" ? .mint : status == "stale" ? .orange : status == "atRisk" ? .red : .gray
    ZStack {
      Circle()
...
struct ItemsView: View {
  ...
  private func loadItems() {
    ...
    let derivedItems =
      (try? ProjectDerivedItemService(context: modelContext).fetch(for: projectID)) ?? []
...
struct BoardView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      .scrollPosition(
        id: .init(get: { selectedColumn }, set: { if let i = $0 { selectedColumn = i } })
      )
    ...
    return Button {
      ...
      .overlay(
        RoundedRectangle(cornerRadius: 16)
          .stroke(
            dropTarget?.rawValue == task.statusRaw ? Color.green.opacity(0.5) : .clear, lineWidth: 2
          )
      )
    }
    .buttonStyle(.plain)
  ...
  private func loadData() {
    tasks =
      (try? ProjectDerivedItemService(context: modelContext).fetch(for: projectID, type: .task))
      ?? []
...
struct SignalsView: View {
    ...
    List {
      if filteredSignals.isEmpty {
        VStack(spacing: 16) {
          ...
          Text(
            "Signals will appear here when the system detects patterns, risks, or opportunities."
  ...
  private func signalCard(_ s: AgentSuggestion) -> some View {
    ...
    return VStack(alignment: .leading, spacing: 6) {
      HStack {
        ...
        Text(s.type.replacingOccurrences(of: "_", with: " ")).font(.caption).fontWeight(.medium)
          .foregroundStyle(color)
        if s.isCritical {
          Image(systemName: "exclamationmark.triangle.fill").font(.caption2).foregroundStyle(
            .orange)
        }
        Spacer()
        Text(s.createdAt.formatted(.relative(presentation: .named))).font(.caption2)
          .foregroundStyle(.secondary)
      }
  ...
  private func loadSignals() {
    ...
    AppLog.debug(
      "project", "SignalsView.loadSignals done — total=\(all.count) filtered=\(signals.count)")
  }
}
  ...
  var body: some View {
    List {
      ...
      Section {
        if promptEntries.isEmpty {
          ...
          ForEach(promptEntries, id: \.name) { prompt in
            NavigationLink {
              ConfigPromptEditorView(
                promptName: prompt.name, content: prompt.content, category: prompt.category)
            } label: {
              HStack {
                VStack(alignment: .leading, spacing: 2) {
        ...
        Text(
          "\(promptEntries.count) prompts. Edit to customize AI behavior. Changes persist to PromptStore."
        )
      }
      ...
      Section {
        ...
        Text(
          "Analysis frameworks define output structure for each domain. \(schemaEntries.count) built-in."
        )
      }
      ...
      Section {
        ...
        Text(
          "Agent memories are learned patterns. Stale memories (>3 failures) are excluded from search."
        )
      }
    }
    .navigationTitle(project.name)
  ...
  var body: some View {
    Group {
      if isLoading {
        ...
        ScrollView {
          SynthesisContentView(
            synthesis: synthesis, derivedItems: derivedItems, projectID: project.id)
        }
        .refreshable { loadData() }
      } else {
