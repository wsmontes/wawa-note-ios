...
struct ProjectEntitiesView: View {
        ...
        VStack(spacing: 0) {
            if isLoading {
                ...
                VStack(spacing: AppSpacing.md) {
                    ...
                    Text("Entities like organizations, systems, and locations are extracted during analysis.").font(.caption).foregroundStyle(.secondary)
                }
                Spacer()
            } else {
                if !kinds.isEmpty {
                        ...
                        HStack(spacing: AppSpacing.xs) {
                            Button {
                                ...
                                Text("All").font(.caption2)
                                    ...
                                    .background(selectedKind == nil ? Color.accentColor.opacity(0.15) : Color(.tertiarySystemFill))
                                    .clipShape(Capsule())
                            }
                            ForEach(kinds, id: \.self) { kind in
                                Button {
                                    ...
                                    Text(kind.capitalized).font(.caption2)
                                        .padding(.horizontal, AppSpacing.sm).padding(.vertical, AppSpacing.xs)
                                        .background(selectedKind == kind ? Color.accentColor.opacity(0.15) : Color(.tertiarySystemFill))
                                        .clipShape(Capsule())
                                }
                            }
                        }
    ...
    private func loadEntities() async {
        ...
        entities = nameToEntry.map {
            EntitySummary(name: String($0.key.split(separator: "|")[0]), kind: $1.kind, mentionCount: $1.count, sourceItems: $1.items)
