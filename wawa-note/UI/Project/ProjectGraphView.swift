...
struct ProjectGraphView: View {
  ...
  private func filterChip(for et: EdgeType) -> some View {
    ...
    return Button {
      if selected {
        visibleEdgeTypes.remove(et.rawValue)
      } else {
        visibleEdgeTypes.insert(et.rawValue)
      }
    } label: {
  ...
  private var graphCanvas: some View {
    let filteredEdges =
      visibleEdgeTypes.isEmpty
      ? edges : edges.filter { visibleEdgeTypes.contains($0.type.rawValue) }
    ...
    return GeometryReader { geo in
      Canvas { ctx, size in
        for edge in filteredEdges {
          ...
          ctx.stroke(
            path, with: .color(edgeColor(edge.type).opacity(0.4)),
            lineWidth: max(1, CGFloat(edge.weight) * 2))
        }
        for node in nodes {
          let rect = CGRect(
            x: node.x - node.radius, y: node.y - node.radius, width: node.radius * 2,
            height: node.radius * 2)
          ctx.fill(Circle().path(in: rect), with: .color(node.color.opacity(0.8)))
          if node.degree >= 3 {
            ctx.stroke(
              Circle().path(in: rect.insetBy(dx: -2, dy: -2)), with: .color(.white.opacity(0.6)),
              lineWidth: 2)
          }
          ctx.draw(
            Text(node.label.prefix(12)).font(.system(size: 9)).foregroundColor(.primary),
      ...
      .simultaneousGesture(
        LongPressGesture(minimumDuration: 0.3).sequenced(before: DragGesture(minimumDistance: 0))
          .onEnded { value in
            guard case .second(true, let drag) = value, let location = drag?.location else {
              return
            }
            handleTap(location)
          }
  ...
  private func handleTap(_ location: CGPoint) {
    ...
    if let tapped = nodes.first(where: {
      sqrt(pow($0.x - adjusted.x, 2) + pow($0.y - adjusted.y, 2)) < max($0.radius + 14, 30)
    }) {
      selectedNode = tapped
    }
  }
            ...
            if let other = nodes.first(where: { $0.id == otherID }) {
              VStack(alignment: .leading, spacing: AppSpacing.xs) {
                ...
                if let prov = edge.provenance, let provID = UUID(uuidString: prov) {
                  EvidenceCardView(
                    itemTitle: "Source item", itemID: provID, snippet: "Edge derived from analysis",
                    segmentID: nil, confidence: edge.weight, edgeType: edge.type.rawValue)
                }
              }
            }
          }
  ...
  private func loadGraph() async {
      ...
      for nid in [edge.fromID, edge.toID] {
        ...
        if let item = findItem(nid) {
          nodeMap[nid] = GraphNode(
            id: nid, label: item.title, subtitle: item.type.label, kind: .item, degree: 1)
        } else if let task = findTask(nid) {
          nodeMap[nid] = GraphNode(
            id: nid, label: task.title, subtitle: task.status.rawValue.capitalized, kind: .task,
            degree: 1)
        } else if let person = findPerson(nid) {
          nodeMap[nid] = GraphNode(
            id: nid, label: person.displayName, subtitle: person.role ?? "Person", kind: .person,
            degree: 1)
        } else if let entity = findEntity(nid) {
          nodeMap[nid] = GraphNode(
            id: nid, label: entity.displayName, subtitle: entity.kindRaw.capitalized, kind: .entity,
            degree: 1)
        }
      }
    }
    ...
    if let proj = try? ProjectService(context: modelContext).fetch(id: projectID) {
      nodeMap[projectID] = GraphNode(
        id: projectID, label: proj.name, subtitle: "Project", kind: .project, degree: nodeMap.count)
    }

    self.edges = loadedEdges.map {
      GraphEdgeItem(
        fromID: $0.fromID, toID: $0.toID, type: $0.edgeType, weight: $0.weight,
        provenance: $0.provenanceItemID?.uuidString)
    }
