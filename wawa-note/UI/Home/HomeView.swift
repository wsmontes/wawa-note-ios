...
final class HomeViewModel: ObservableObject {
  ...
  func configure(
    ...
    coordinator: RecordingCoordinator, processingQueue: ProcessingQueueService? = nil
  ) {
  ...
  func handleFilePick(_ result: Result<[URL], Error>) {
    ...
    case .success(let urls):
      ...
      if urls.count == 1, let url = urls.first {
        ...
        Task { await importFiles(urls, deleteSource: false) }
      }
  ...
  private func stageSingleImport(_ url: URL) {
    ...
    if importer.formatIdentifier == "audio" {
        ...
        do {
          ...
          await MainActor.run {
            pendingImport = ImportPending(
              url: tempURL, kind: .audio(meta), isFromShareExtension: false)
          }
        } catch {
      ...
      pendingImport = ImportPending(
        url: tempURL, kind: .text(preview), textImporter: importer, isFromShareExtension: false)
    }
  }
  ...
  private func importAudioFile(
    _ url: URL, importer: any FormatImporter, deleteSource: Bool, modelContext: ModelContext,
    pipeline: ContentPipelineService
  ) async {
    guard let coord = coordinator else { return }
    let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent(
      "import_\(UUID().uuidString)_\(url.lastPathComponent)")
    ...
    do { meta = try await importService.extractMetadata(url: tempURL) } catch {
      AppLog.general.error(
        "HomeView: importAudioFile metadata extraction failed — \(error.localizedDescription)")
      return
    }
  ...
  private func detectImageType(url: URL) -> String? {
    ...
    if ["jpg", "jpeg", "png", "heic", "heif", "webp", "gif", "tiff", "bmp"].contains(ext) {
      return ext
    }
    // Try UTI
  ...
  private func importImageFile(
    ...
    guard
      let item = try? itemService.createItem(
        type: .image, title: title, bodyText: nil, inboxDate: Date())
    else { return }
    // Save image
...
struct HomeView: View {
  ...
  @Query(
    ...
    order: .reverse) private var inboxItems: [KnowledgeItem]
  ...
  var body: some View {
    ...
    .fileImporter(
      isPresented: $importVM.showFilePicker,
      allowedContentTypes: importVM.importRouter.allUTTypes(), allowsMultipleSelection: true
    ) { importVM.handleFilePick($0) }
    .sheet(item: $importVM.pendingImport) { imp in
      ImportFormView(
        sourceURL: imp.url, kind: imp.kind, textImporter: imp.textImporter,
    ...
    .alert(
      "Import Error",
      isPresented: Binding(
        get: { importVM.importError != nil }, set: { if !$0 { importVM.importError = nil } })
    ) {
    ...
    .onAppear {
      ...
      importVM.configure(
        modelContext: modelContext, contentPipeline: contentPipeline, coordinator: coordinator,
    ...
    return VStack(spacing: 0) {
      HStack(spacing: 12) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          ...
          Text(
            "\(projectItems.count) items · Updated \(project.updatedAt.formatted(date: .abbreviated, time: .shortened))"
          )
  ...
  private func inboxRow(_ item: KnowledgeItem) -> some View {
    HStack(spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        Text(item.title.isEmpty ? "Untitled" : item.title).font(.subheadline).fontWeight(.medium)
          .lineLimit(1)
        Text(
          "\(item.type.label) · \(item.updatedAt.formatted(date: .abbreviated, time: .shortened))"
        )
  ...
  private func processPhotoItem(_ image: UIImage) async -> [KnowledgeItem] {
    let itemService = KnowledgeItemService(context: modelContext)
    guard let item = try? itemService.createItem(type: .image, title: "Photo", bodyText: nil) else {
      return []
    }
...
final class ScannerViewModel: ObservableObject {
  ...
  func createItems(from images: [UIImage], context: ModelContext) async -> [KnowledgeItem] {
    ...
    if let provider = try? ProviderRouter.resolveActive(context: context) {
      ...
      if let analysis = try? await ImageAnalysisService().analyzeImage(
        firstPageURL, llmProvider: provider, model: model),
        !analysis.isEmpty
      {
...
struct CameraCaptureView: UIViewControllerRepresentable {
  ...
  final class Coordinator: NSObject, UIImagePickerControllerDelegate,
    UINavigationControllerDelegate, @unchecked Sendable
  {
