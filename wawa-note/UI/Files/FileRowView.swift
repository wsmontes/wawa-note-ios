...
struct FileRowView: View {
  ...
  private var contextMenuContent: some View {
    if let onOpen {
      Button {
        ...
        Label(
          node.isDirectory ? "Open Folder" : "Open File",
          systemImage: node.isDirectory ? "arrow.right.circle" : "doc.text.fill")
      }
    }
  ...
  private var folderColor: String {
    node.metadata.isConfigProject
      ? "#64748B" : node.metadata.projectStatus == "archived" ? "#9CA3AF" : "#64748B"
  }
