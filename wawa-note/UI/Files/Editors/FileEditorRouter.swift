...
enum FileEditorRouter {
  ...
  private static func audioPlayerView(node: VFSNode, viewModel: FileBrowserViewModel) -> some View {
    if let audioURL = viewModel.resolveAudioURL(for: node.path) {
      AudioPlayerView(
        audioURL: audioURL,
        title: node.name.replacingOccurrences(of: ".m4a", with: "").replacingOccurrences(
          of: ".mp3", with: ""
  ...
  @ViewBuilder
  private static func textEditorFallback(node: VFSNode, viewModel: FileBrowserViewModel)
    -> some View
  {
