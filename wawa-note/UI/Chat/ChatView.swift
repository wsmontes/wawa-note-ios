...
struct ChatView: View {
  ...
  var body: some View {
          ...
          HStack {
            ...
            HStack(spacing: 8) {
              ...
              if let proj = viewModel.activeProjectName {
                HStack(spacing: 4) {
                  Circle().fill(viewModel.activeProjectColorHex.flatMap { Color(hex: $0) } ?? .blue)
            ...
            HStack(spacing: 8) {
              if viewModel.isGreetingLoading {
                ...
                HStack(spacing: 4) {
                  ...
                  Text(viewModel.activeToolCalls.last?.toolName ?? "Tool").font(.caption2)
      ...
      if dictation.phase == .recording {
        HStack(spacing: 10) {
          ...
          Circle()
            ...
            .animation(
              .easeInOut(duration: 0.3).repeatForever(autoreverses: true), value: dictation.elapsed)
          ...
          HStack(spacing: 2) {
            ForEach(0..<10, id: \.self) { i in
              RoundedRectangle(cornerRadius: 1)
                ...
                .frame(
                  width: 2,
                  height: CGFloat(4 + 14 * abs(sin(dictation.elapsed * 4 + Double(i) * 0.7)))
                )
                .animation(
                  .easeInOut(duration: 0.15).repeatForever(autoreverses: true),
                  value: dictation.elapsed)
            }
          }
          .frame(height: 18)
    ...
    .navigationTitle(
      compact
        ? ""
        : (viewModel.currentConversation?.title.isEmpty != false
          ? "Chat" : viewModel.currentConversation?.title ?? "Chat")
    )
          ...
          LazyVStack(spacing: 12) {
            ...
            if !viewModel.streamingText.isEmpty {
              StreamingMessageView(
                text: viewModel.streamingText, projectColorHex: viewModel.activeProjectColorHex
              )
              .id("streaming")
            }
  ...
  private var emptyState: some View {
    VStack(spacing: 16) {
      ...
      Text(
        "Your AI assistant can search, read, and explore your audio recordings, notes, projects, and connections."
      )
      .font(.subheadline)
      .foregroundStyle(.secondary)
      ...
      AVAudioSession.sharedInstance().requestRecordPermission { granted in
        DispatchQueue.main.async {
          if granted {
            ...
            self.dictation.error = "Microphone access denied. Enable in Settings."
          }
        }
      }
      return
        ...
        ZStack {
          if dictationPhase == .recording {
            Circle()
              ...
              .animation(
                .easeInOut(duration: 0.5).repeatForever(autoreverses: true), value: dictationElapsed
              )
          }
          Group {
          ...
          Image(systemName: "arrow.up.circle.fill")
            ...
            .foregroundColor(
              inputText.trimmingCharacters(in: .whitespaces).isEmpty
                ? Color.secondary.opacity(0.5) : Color.blue)
        }
  ...
  var body: some View {
    VStack(alignment: message.role == .user ? .trailing : .leading, spacing: 4) {
      HStack {
                ...
                ForEach(citations, id: \.itemId) { c in
                  NavigationLink(value: c.itemId) {
                    HStack(spacing: 4) {
                      ...
                      Image(systemName: c.itemType.icon).font(.caption2).foregroundStyle(
                        c.itemType.color)
        ...
        .padding(12)
        .background(
          message.role == .user ? AnyShapeStyle(effectiveColor) : AnyShapeStyle(.thinMaterial)
        )
...
private func shareConversation(viewModel: ChatViewModel) {
  ...
  for msg in messages {
    let role =
      msg.role == .user ? "You" : (msg.role == .assistant ? "Wawa" : "[\(msg.role.rawValue)]")
...
struct ConversationListView: View {
      ...
      Group {
        if viewModel.conversations.isEmpty {
          VStack(spacing: 16) {
            ...
            Text("Start a new conversation in the Chat tab.").font(.subheadline).foregroundStyle(
              .secondary)
          }
        } else {
          List {
  ...
  private var groupedConversations:
    [(title: String, icon: String, conversations: [ChatConversation])]
  {
      ...
      label: AnyView(
        HStack(spacing: 6) {
          if status.phase == "processing" {
            ...
            Image(
              systemName: status.phase == "completed"
...
struct ParsedMessageView: View {
  ...
  init(
    ...
    self._cachedSmartBlocks = State(
      initialValue: SmartBlockParser.parse(message.content, role: message.role))
  ...
  var body: some View {
    Group {
      ...
      // 3. Fallback: markdown parser — skip only single .text blocks from assistant
        ...
        let isLoneTextBlock =
          parsed.count == 1 && message.role == .assistant
          && { if case .text = parsed[0] { true } else { false } }()
        if !isLoneTextBlock {
          VStack(alignment: .leading, spacing: 8) {
          ...
          .padding(12).background(Color(.systemBackground)).clipShape(
...
enum SmartBlockParser {
  static func parse(_ text: String, role: AIRole) -> [ChatBlock] {
    ...
    if blocks.count >= 2 || blocks.containsNonText {
      return injectVFSLinks(blocks, originalText: text)
    }
          ...
          result.append(
            .fileLink(
              FileLinkData(
                ...
                itemType: path.hasSuffix(".md")
                  ? "note" : path.hasSuffix(".json") ? "json" : "unknown",
                snippet: path,
                projectSlug: nil
              )))
  ...
  private static func extractBlocks(_ text: String) -> [ChatBlock] {
    ...
    func flush() {
      ...
      if let tasks = detectCheckboxTasks(section.content), !tasks.isEmpty {
        ...
        if parsed.count > 1
          || (parsed.count == 1 && { if case .text = parsed[0] { false } else { true } }())
        {
    ...
    for line in lines {
      ...
      if cleaned.hasPrefix("## ") || cleaned.hasPrefix("# ") {
        flush()
        currentSection = (
          title: cleaned.replacingOccurrences(of: "#", with: "").trimmingCharacters(
            in: .whitespaces), content: []
        )
      } else if cleaned.hasPrefix("**") && cleaned.contains(":**") {
        flush()
        let title = cleaned.replacingOccurrences(of: "**", with: "").replacingOccurrences(
          of: ":", with: ""
  ...
  private static func detectChoicePrompt(_ lines: [String]) -> ChoicePromptData? {
    ...
    let questionLine =
      lines.first { !numbered.contains($0) && !$0.trimmingCharacters(in: .whitespaces).isEmpty }
      ?? "Choose:"
    let options = numbered.compactMap { line -> ChoiceOption? in
      var s = line.trimmingCharacters(in: .whitespaces)
      while let f = s.first, f.isNumber || f == "." || f == ")" || f == "-" || f == " " {
        s.removeFirst()
      }
    ...
    return options.count >= 2
      ? ChoicePromptData(question: String(questionLine.prefix(150)), options: options) : nil
  }

  private static func detectCheckboxTasks(_ lines: [String]) -> [ChatBlock]? {
    let tasks = lines.filter { line in
      let t = line.trimmingCharacters(in: .whitespaces)
      return t.hasPrefix("- [ ]") || t.hasPrefix("* [ ]") || t.hasPrefix("- [x]")
        || t.hasPrefix("* [x]")
    }
    ...
    return tasks.compactMap { line -> ChatBlock? in
      ...
      return .taskCard(
        TaskCardData(
          taskID: UUID().uuidString, title: title, status: done ? "done" : "todo",
          priority: "medium", owner: nil, projectSlug: nil, needsConfirmation: !done))
    }
  }
  ...
  private static func detectConfirmation(_ text: String) -> ConfirmationData? {
    ...
    guard
      lower.contains("warning") || lower.contains("are you sure") || lower.contains("delete?")
        || lower.contains("confirm")
    else { return nil }
    let first = text.components(separatedBy: "\n").first ?? text
    return ConfirmationData(
      title: "Please confirm", message: String(first.prefix(200)), confirmLabel: "Yes, proceed",
      cancelLabel: "Cancel", confirmValue: "yes", cancelValue: "cancel")
  }
  ...
  private static func convertOutputToChatBlock(_ output: OutputBlock) -> ChatBlock {
    ...
    case .table(let table):
      return .table(TableData(title: table.title, headers: table.headers, rows: table.rows))
    case .code(let code):
      return .code(CodeData(code: code.code, language: code.language, caption: code.caption))
...
struct ChatBlockView: View {
  ...
  var body: some View {
    ...
    case .code(let data):
      CodeBlockView(
        codeBlock: CodeBlock(code: data.code, language: data.language, caption: data.caption))
    case .bulletList(let items):
      BulletListView(items: items)
    case .orderedList(let items):
      OrderedListView(items: items)
        ...
        if let uuid = UUID(uuidString: data.itemID) {
          Button {
            ...
            NotificationCenter.default.post(
              name: .pipelineCompleted, object: data.itemID,
        ...
        DisclosureGroup {
          VStack(alignment: .leading, spacing: 2) {
            ...
            if section.items.count > 10 {
              Text("... and \(section.items.count - 10) more").font(.caption2).foregroundStyle(
                .tertiary)
            }
          }.padding(.leading, 4).padding(.top, 2)
        } label: {
          HStack(spacing: 6) {
            Image(systemName: sectionIcon(section.title)).font(.system(size: 10)).foregroundStyle(
              .blue)
