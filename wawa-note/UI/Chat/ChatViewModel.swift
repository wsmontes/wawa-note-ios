...
final class ChatViewModel: ObservableObject {
  ...
  private func switchToContext(_ context: ChatContext) {
    ...
    guard let conv = try? chatService.findOrCreateConversation(for: effectiveContext) else {
      return
    }
  ...
  private func injectProjectContext(project: Project, conversationId: UUID) {
    ...
    for t in tasks.prefix(20) {
      taskLines +=
        "  \(t.id.uuidString).json  [\(t.statusRaw)]  \(t.title)  priority=\(t.priorityRaw)\n"
    }
    var itemLines = ""
    for it in items.prefix(10) {
      itemLines +=
        "  \(it.id.uuidString.prefix(8)).json  [\(it.typeRaw)]  \(it.title)  (\(it.statusRaw))\n"
    }
    ...
    let cdMsg = ChatMessage(
      ...
      toolCalls: [
        PersistedToolCall(
          id: UUID().uuidString, name: "run_command", arguments: cdArgs, status: .completed)
      ],
    ...
    let lsMsg = ChatMessage(
      ...
      toolCalls: [
        PersistedToolCall(
          id: UUID().uuidString, name: "run_command", arguments: lsArgs, status: .completed)
      ],
      blocks: [.text("ls")]
    )
  ...
  func pregenerateGreeting(for context: ChatContext) {
    ...
    let model =
      ...
      ? (try? ActiveProviderManager.shared.getActiveProvider(context: ctx))?.defaultModel
  ...
  func sendMessage() {
    ...
    let userMsg = ChatMessage(
      conversationId: conv.id, role: .user, content: text, projectColorHex: activeProjectColorHex)
    ...
    let model =
      selectedModel.isEmpty
      ? (try? ActiveProviderManager.shared.getActiveProvider(context: ctx))?.defaultModel
        ?? Self.defaultChatModel : selectedModel
    ...
    let slugForContext = activeProjectID.flatMap { pid in
      try? ProjectService(context: ctx).fetch(id: pid)?.slug.replacingOccurrences(
        of: "/", with: "-")
    }
    ...
    let activeDefaultModel = (try? ActiveProviderManager.shared.getActiveProvider(context: ctx))?
      .defaultModel
    let execModel =
      selectedModel.isEmpty ? (activeDefaultModel ?? ChatViewModel.defaultChatModel) : selectedModel
    let advModel =
      selectedModel.isEmpty ? (activeDefaultModel ?? ChatViewModel.defaultChatModel) : selectedModel
    let loop = AgentLoop(
      registry: registry, toolContext: toolContext, mode: mode, executorModel: execModel,
      ...
      do {
        ...
        for try await event in stream {
          ...
          case .toolCallStarted(let name, let id, let args):
            activeToolCalls.append(
              ToolCallProgress(
                id: id, toolName: name, status: .running, displaySummary: "Calling \(name)...",
                error: nil))
          case .toolCallCompleted(let name, let id, let summary):
            if let idx = activeToolCalls.firstIndex(where: { $0.id == id }) {
              activeToolCalls[idx] = ToolCallProgress(
                id: id, toolName: name, status: .completed, displaySummary: summary, error: nil)
            }
  ...
  private func handleCommand(_ text: String) {
    ...
    case "analyze":
      ...
      response =
        "Re-analysis triggered for item `\(arg)`. Check the Knowledge detail view for progress."
    case "prompt":
      guard !arg.isEmpty else {
    ...
    case "memories":
      ...
      response = all.map {
        "- \($0.isStale ? "[STALE] " : "")\($0.pattern) (\($0.successCount)S/\($0.failCount)F)"
      }.joined(separator: "\n")
    case "search":
      guard !arg.isEmpty else {
      ...
      let results = SearchService(fileStore: FileArtifactStore()).searchNow(query: arg, in: items)
        .prefix(5)
  ...
  func runCommandDirectly(_ command: String) {
    ...
    let slug = activeProjectID.flatMap { pid in
      try? ProjectService(context: ctx).fetch(id: pid)?.slug.replacingOccurrences(
        of: "/", with: "-")
    }
    let toolCtx = ToolContext(
      modelContext: ctx, activeProjectID: activeProjectID, activeProjectName: activeProjectName,
    ...
    let tc = PersistedToolCall(
      id: UUID().uuidString, name: "run_command", arguments: "{\"command\":\"\(command)\"}",
      status: .completed)
    let assistantMsg = ChatMessage(
