...
struct InboxView: View {
  ...
  var body: some View {
    ...
    .confirmationDialog(
      "Move to Trash?", isPresented: $showDeleteConfirmation, presenting: itemToDelete
    ) { item in
      ...
      HStack(spacing: 8) {
        ForEach(InboxFilter.allCases, id: \.self) { filter in
          Button {
            ...
            .padding(.vertical, 8)
            .background(
              filterMode == filter ? Color.accentColor : Color(.secondarySystemGroupedBackground)
            )
            .clipShape(Capsule())
          }
        }
  ...
  private var filteredItems: [KnowledgeItem] {
    ...
    let configProjectID = projects.first(where: {
      $0.slug == ConfigProjectService.configProjectSlug
    })?.id
    ...
    switch filterMode {
    case .needsReview:
      result = result.filter { $0.inboxDate != nil && $0.analysisProviderId == nil }
  ...
  private var needsReviewCount: Int {
    ...
    if let configID = projects.first(where: { $0.slug == ConfigProjectService.configProjectSlug })?
      .id
    {
