...
struct SettingsView: View {
    ...
    NavigationStack {
      List {
          ...
          VStack(alignment: .leading, spacing: 4) {
            if transcriptionMode == .whisper {
              if hasWhisperProvider {
                Text(
                  "Whisper API will be used. Language is auto-detected — no need to pick a locale.")
              } else {
                Text(
                  "No provider with audio support configured. Add an OpenAI API key in AI Services to use Whisper."
                )
                .foregroundStyle(.orange)
              }
            } else {
              Text(
                "On-device transcription. Works offline, no API key needed. Locale: \(supportedLocales().joined(separator: ", "))"
              )
        ...
        Section {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text(
              "When enabled, new recordings are transcribed and analyzed automatically after saving."
            )
            if autoAnalyze {
              Text(
                "Automatic analysis uses the selected model above. Pick a fast, affordable model for cost efficiency."
              )
              .foregroundStyle(.secondary)
            }
          }
        ...
        Section {
          ...
          VStack(alignment: .leading, spacing: 4) {
            Text(
              "Voice Processing enhances speech clarity and reduces background noise. Disable for raw audio capture (music, ambient sound)."
            )
            if speakerphoneMode {
              Text(
        ...
        // Data Export
        Section {
          if let jsonData = try? InstanceExportService().exportComplete(
            context: modelContext, includeHistory: false),
            let jsonStr = String(data: jsonData, encoding: .utf8)
          {
            let stats = InstanceExportService().exportStatistics(context: modelContext)
            ShareLink(
              item: jsonStr,
              preview: SharePreview("Wawa Note Export", image: Image(systemName: "doc.text"))
            ) {
              HStack {
                ...
                VStack(alignment: .trailing) {
                  ...
                  let size = ByteCountFormatter.string(
                    fromByteCount: Int64(jsonData.count), countStyle: .file)
          ...
          Text(
            "Exports all projects, items, analyses, signals, frames, prompts, and configuration as a complete JSON file. File may be large."
          )
        }
        ...
        Section {
          ...
          Text(
            "Sync meeting notes with anarlog desktop app. Choose a shared folder to enable bidirectional import and export of .md session notes."
          )
        }
        ...
        Section {
          ...
          Text(
            "Configure how the AI analyzes your content. Each skill defines a procedure and output format for a specific type of content."
          )
        }
        ...
        Section {
          ...
          Text(
            "Fine-tune AI behavior. Lenses define analysis perspectives, Model Resolution sets fallback chains per task, and Summary Cache stores previous analysis results to avoid redundant API calls."
          )
        }
      }
  ...
  struct DebugLogView: View {
    ...
    var body: some View {
      Group {
        if isLoading {
          ...
          VStack(spacing: 12) {
            ...
            Text("Logs will appear here as the app runs.").font(.subheadline).foregroundStyle(
              .secondary)
          }
        } else {
          ScrollView {
      ...
      .safeAreaInset(edge: .top) {
        Text(
          "⚠️ Logs may contain personal data (transcripts, file paths, metadata). API keys are automatically redacted. Review before sharing."
        )
        .font(.caption2)
        .foregroundStyle(.orange)
        .padding(8)
    ...
    static func retrieveOSLogs() -> String? {
      ...
      for entry in (try? store.getEntries(at: position)) ?? AnySequence([]) {
        ...
        guard
          msg.contains("audio") || msg.contains("Audio") || msg.contains("recording")
            || msg.contains("Recording") || msg.contains("route") || msg.contains("Route")
            || msg.contains("engine") || msg.contains("Engine") || msg.contains("session")
      ...
      List {
        if lensEntries.isEmpty {
          ContentUnavailableView(
            ...
            description: Text(
              "AI lenses are defined in ai_config.json. Add lenses to enable different analysis perspectives."
            )
          )
        } else {
    ...
    var body: some View {
      List {
        Section {
          ...
          Text(
            "The summary cache avoids redundant API calls by reusing analysis results when the transcript, template, and model haven't changed."
          )
        }
      ...
      .confirmationDialog(
        ...
        Text(
          "This will remove \(stats.entries) cached summaries. Future analyses will require fresh API calls."
        )
      }
    }
  }
  ...
  private func supportedLocales() -> [String] {
    ...
    let configured = Set([
      "pt-BR", "pt-PT", "en-US", "es-ES", "fr-FR", "de-DE", "it-IT", "ja-JP", "zh-CN",
    ])
