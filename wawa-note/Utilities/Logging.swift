...
final class FileLogService: @unchecked Sendable {
  ...
  private func logSessionStart() {
    ...
    let info =
      "SESSION_START device=\(model) os=\(osStr) app=\(version)(\(build)) \(previousSessionCrashed ? "⚠️ PREVIOUS_CRASH" : "clean")\n"
  ...
  func retrieveLogs() -> String {
    ...
    if let e = fileManager.enumerator(
      at: cachesDir, includingPropertiesForKeys: [.contentModificationDateKey])
    {
    ...
    for (pattern, replacement) in patterns {
      if let regex = try? NSRegularExpression(pattern: pattern, options: []) {
        result = regex.stringByReplacingMatches(
          in: result, options: [], range: NSRange(location: 0, length: result.utf16.count),
