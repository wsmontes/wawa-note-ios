    ...
    private func transcribeSingle(url: URL, prompt: String?, meetingId: UUID) async throws -> Transcript {
        ...
        for attempt in 0...Self.maxRetries {
            ...
            do {
                ...
                // Parse segments from verbose_json when available, fall back to plain text
                let segments: [TranscriptSegment]
                if let segsArray = json["segments"] as? [[String: Any]], !segsArray.isEmpty {
                    segments = segsArray.compactMap { segDict in
                        guard let text = segDict["text"] as? String,
                            !text.trimmingCharacters(in: .whitespaces).isEmpty
                        else { return nil }
                        return TranscriptSegment(
                            meetingId: meetingId,
                            startTime: segDict["start"] as? Double ?? 0,
                            endTime: segDict["end"] as? Double,
                    ...
                    let body = String(data: resData, encoding: .utf8) ?? "<no body>"
                    AppLog.transcription.error("Parse error: unexpected format — \(body.prefix(300))")
                    throw TranscriptionError.recognitionFailed("Remote transcription returned unexpected format")
                }
