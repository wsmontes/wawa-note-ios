...
struct ScannedCode: Identifiable, Codable, Sendable {
  ...
  var symbologyName: String {
    ...
    if raw.hasPrefix("org.iso") {
      return raw.replacingOccurrences(of: "org.iso.", with: "").replacingOccurrences(
        of: "org.gs1.", with: "")
    }
    return raw
  }
  ...
  var detectedDataType: DetectedDataType {
    ...
    if v.range(of: #"^[A-Z0-9]+$"#, options: .regularExpression) != nil && v.count > 4 {
      return .code
    }
    return .text
  }
...
struct ScanSession: Codable, Sendable {
  ...
  func toTextDocument() -> String {
    ...
    if let ended = endedAt {
      lines.append("---")
      lines.append(
        "Scanned \(codes.count) code(s) in \(formatDuration(ended.timeIntervalSince(startedAt)))")
    }
