...
enum TemplateParser {
    static func parse(_ markdown: String, id: String) -> AITemplate? {
        ...
        for line in frontmatter.split(separator: "\n") {
            ...
            case "activation":
                activation = TemplateActivation(rawValue: value.replacingOccurrences(of: "\"", with: "")) ?? .manual
            case "globs":
                let cleaned = value.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "")
                globs = cleaned.split(separator: ",").map { $0.trimmingCharacters(in: .whitespaces).replacingOccurrences(of: "\"", with: "") }
            case "renderer":
                renderer = TemplateRenderer(rawValue: value.replacingOccurrences(of: "\"", with: "")) ?? .richText
        ...
        for section in sections {
            ...
            if trimmed.hasPrefix("System") {
                ...
                responseSchema = String(trimmed.dropFirst(15)).trimmingCharacters(in: .whitespacesAndNewlines)
            }
        }
