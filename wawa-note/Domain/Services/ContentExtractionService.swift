...
final class ContentExtractionService {
  ...
  init(
    ...
    // Never pass nil to the engine — use device best-guess as fallback.
    // This ensures the SFSpeechRecognizer always gets an explicit locale
    // rather than relying on the engine's internal device-language default.
    self.preferredLocale = preferredLocale ?? TranscriptionLocaleProvider.bestGuessLocale
  }
  ...
  func extractTextFromAudio(_ item: KnowledgeItem) async -> String? {
    ...
    let manifest: RecordingManifest? =
      hasManifest ? try? fileStore.readRecordingManifest(for: item.id) : nil
    ...
    let segmentDetails: [String] =
      manifest?.segments.sorted(by: { $0.index < $1.index }).map { seg in
        ...
        let segSize =
          segExists
          ? (try? FileManager.default.attributesOfItem(atPath: segURL.path)[.size] as? Int) ?? 0 : 0
        let segDur = segExists ? Self.audioDuration(url: segURL) : 0
        return
          "seg[\(seg.index)]=\(seg.fileName) exists=\(segExists) size=\(segSize) dur=\(String(format: "%.1f", segDur))s"
      } ?? []
    ...
    guard audioDuration >= 1.0 else {
      AppLog.audio.error(
        "Transcription validation FAILED: audio too short (\(String(format: "%.1f", audioDuration))s) — marking as failed"
      )
      if let fresh = try? KnowledgeItemService(context: modelContext).fetchItem(id: item.id) {
        fresh.status = .failed
        try? modelContext.save()
    ...
    AppLog.provider.info(
      "ContentExtraction: transcription finished — engine=\(engineLabel) elapsed=\(elapsed)ms chars=\(chars)"
    )
  ...
  private func transcribeSingleFile(item: KnowledgeItem) async -> String? {
    ...
    do {
      ...
      AppLog.provider.info(
        "ContentExtraction: transcription complete (\(result.segments.count) segments)")
  ...
  private func loadExistingTranscriptText(for itemID: UUID) -> String? {
    guard
      let transcript = try? fileStore.readArtifact(
  ...
  func extractTextFromImage(_ item: KnowledgeItem) async -> String? {
    ...
    if let provider = try? ProviderRouter.resolveActive(context: modelContext) {
      let msg = AIMessage(
        role: .user, content: [.text("Describe this image."), .imageFile(imageURL)])
  ...
  func analyze(text: String, item: KnowledgeItem) async throws -> Bool {
    ...
    let model =
      configModel ?? settings.resolveAutoAnalysisModel(context: modelContext)
      ?? settings.autoAnalysisModel
    ...
    AppLog.provider.info(
      "ContentExtraction.analyze: source=\(sourceCtx.sourceType.rawValue), model=\(model)")

    do {
      let result = try await AnalysisService().analyze(
        transcript: transcript, using: provider, model: model, meetingId: item.id,
  ...
  private func analyzeStructured(
    transcript: Transcript, item: KnowledgeItem, sourceCtx: SourceContext, provider: any AIProvider,
    model: String
  ) async -> Bool {
    do {
      let result = try await AnalysisService().analyze(
  ...
  func analyzeDynamic(text: String, item: KnowledgeItem, framework: ProjectFramework) async -> Bool
  {
    ...
    AppLog.provider.info(
      "ContentExtraction.analyzeDynamic: framework=\(framework.id), source=\(sourceCtx.sourceType.rawValue), model=\(model)"
    )
    ...
    do {
      ...
      try resultData.write(
        to: fileStore.itemDirectoryURL(for: item.id).appendingPathComponent("analysis.dynamic.json")
      )
      ...
      AppLog.provider.error(
        "ContentExtraction.analyzeDynamic: failed for item \(item.id): \(error)")
      return false
    }
  }
  ...
  private func splitLongParagraph(_ text: String, itemID: UUID, startIdx: Int)
    -> [TranscriptSegment]
  {
    ...
    for sentence in sentences {
      ...
      if candidate.count <= Self.maxChunkChars {
        ...
        if !current.isEmpty {
          segments.append(
            TranscriptSegment(
              meetingId: itemID, startTime: Double(idx), text: current, sourceEngineId: "text-chunk"
            ))
          idx += 1
        }
        current = sentence
...
final class ImageAnalysisService {
  ...
  func analyzeImage(_ imageURL: URL, llmProvider: any AIProvider, model: String) async throws
    -> String
  {
      ...
      let request = AIRequest(
        ...
        messages: [
          ...
          AIMessage(
            ...
            content: [
              .text(
                "Analyze this document image. Describe its type, content, layout, and any notable visual elements."
              ),
              .imageFile(imageURL),
            ]),
        ],
...
final class LocationIntelligenceService {
  ...
  static func nearbyItems(
    to coordinate: CLLocationCoordinate2D, radiusMeters: Double, in items: [KnowledgeItem]
  ) -> [KnowledgeItem] {
