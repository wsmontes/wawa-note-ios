...
final class ProjectConversionService {
  ...
  func generatePreview(from item: KnowledgeItem, using provider: any AIProvider, model: String)
    async throws -> ConversionPreview
  {
  ...
  func executeConversion(from item: KnowledgeItem, preview: ConversionPreview) throws -> Project {
    ...
    for ce in preview.edges {
      let fromID = resolveRef(
        ce.fromRef, item: item, project: project, tasks: taskRefs, people: personRefs,
        entities: entityRefs)
      let toID = resolveRef(
        ce.toRef, item: item, project: project, tasks: taskRefs, people: personRefs,
        entities: entityRefs)
