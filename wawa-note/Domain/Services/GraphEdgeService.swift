...
final class GraphIntelligenceService {
    ...
    private func buildGraphDescription(project: Project, items: [KnowledgeItem], tasks: [TaskItem], edges: [GraphEdge]) -> String {
        ...
        for item in items {
            ...
            if let analysis = try? fileStore.readArtifact(MeetingAnalysis.self, fileName: "analysis.json", meetingId: item.id) {
                ...
                if !analysis.actionItems.isEmpty {
                    desc += "  Actions: " + analysis.actionItems.map { "\($0.task) (\($0.owner ?? "?"))" }.joined(separator: "; ") + "\n"
                }
                if !analysis.risks.isEmpty {
                    desc += "  Risks: " + analysis.risks.map(\.risk).joined(separator: "; ") + "\n"
                }
                if !analysis.entities.isEmpty {
                    desc += "  Entities: " + analysis.entities.map { "\($0.name)(\($0.type.rawValue))" }.joined(separator: ", ") + "\n"
                }
            }
        }
        desc += "\n"
        ...
        if !edges.isEmpty {
            ...
            for e in edges {
                let from = items.first(where: { $0.id == e.fromID })?.title ?? e.fromID.uuidString.prefix(8).description
                let to = items.first(where: { $0.id == e.toID })?.title ?? e.toID.uuidString.prefix(8).description
