...
final class KnowledgeItemService {
  ...
  func deleteItems(_ items: [KnowledgeItem]) throws -> (deleted: Int, failed: [UUID]) {
    ...
    for item in items {
      ...
      do {
        ...
        AppLog.storage.error(
          "KnowledgeItemService: batch delete file removal failed — \(item.id.uuidString): \(error.localizedDescription)"
        )
    ...
    if !successIDs.isEmpty {
      let annPred = FetchDescriptor<Annotation>(
        predicate: #Predicate { successIDs.contains($0.itemID) })
