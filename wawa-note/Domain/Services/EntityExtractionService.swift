...
final class EntityExtractionService {
  ...
  init(
    context: ModelContext, entityService: EntityService? = nil, edgeService: GraphEdgeService? = nil
  ) {
  ...
  func extractAndPersist(from analysis: MeetingAnalysis, sourceItemID: UUID) throws -> [Entity] {
    ...
    AppLog.general.info(
      "EntityExtraction: persisted \(persisted.count) entities from item \(sourceItemID)")
    return persisted
  }
