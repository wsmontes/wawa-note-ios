...
struct AnalysisSkillService {
  ...
  func analyze(
    ...
    let (userPrompt, _) = await buildUserPrompt(
      item: item, template: template, context: modelContext)
    ...
    for attempt in 1...maxRetries + 1 {
      ...
      let request = AIRequest(
        model: model, messages: messages, temperature: params.temperature,
      ...
      do {
          ...
          let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
        {
          ...
          if evalResult.isPassing {
            ...
            lastError = evalResult.errors.map { "\($0.field): \($0.message)" }.joined(
              separator: "; ")
          }
        } else {
  ...
  private func buildSystemPrompt(
    skill: AnalysisSkill, template: MeetilyTemplateService.MeetilyTemplate
  ) -> String {
  ...
  private func parseAnalysisJSON(
    _ jsonString: String, template: MeetilyTemplateService.MeetilyTemplate, meetingId: UUID
    ...
    return MeetingAnalysis(
      ...
      shortSummary: (json["short_summary"] ?? json["shortSummary"] ?? json["summary"]
    ...
    for key in ["decisions", "key_decisions", "Decisions", "Key Decisions"] {
      if let items = json[key] as? [[String: Any]] {
        return items.compactMap { item in
          guard let title = (item["title"] ?? item["decision"] ?? item["item"]) as? String,
            !title.isEmpty
          else { return nil }
          return Decision(
            title: title, details: (item["details"] ?? item["description"] ?? "") as? String ?? "")
        }
      }
    }
    return []
  ...
  private func parseActions(_ json: [String: Any]) -> [ActionItem] {
    for key in ["action_items", "actions", "Action Items", "next_steps", "Next Steps"] {
      if let items = json[key] as? [[String: Any]] {
        return items.compactMap { item in
          guard
            let task = (item["task"] ?? item["action"] ?? item["title"] ?? item["item"]) as? String,
            !task.isEmpty
          else { return nil }
  ...
  private func parseRisks(_ json: [String: Any]) -> [Risk] {
    for key in ["risks", "Risks", "risks_issues", "Risks & Issues"] {
      if let items = json[key] as? [[String: Any]] {
        return items.compactMap { item in
          guard
            let risk = (item["risk"] ?? item["title"] ?? item["item"] ?? item["description"])
              as? String, !risk.isEmpty
          else { return nil }
          return Risk(
            risk: risk, details: (item["details"] as? String) ?? "",
      ...
      if let items = json[key] as? [Any] {
        return items.compactMap { item in
          ...
          if let dict = item as? [String: Any],
            let q = (dict["question"] ?? dict["item"]) as? String, !q.isEmpty
          {
