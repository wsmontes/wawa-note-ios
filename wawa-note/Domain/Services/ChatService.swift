...
final class ChatService {
  ...
  func createConversation(
    title: String = "", providerId: UUID? = nil, model: String? = nil, contextKey: String? = nil
  ...
  func fetchConversations() throws -> [ChatConversation] {
    ...
    if FileManager.default.fileExists(atPath: url.path) {
      ...
      if size > 0 {
        ...
        AppLog.general.warning(
          "ChatService: index.json parse failed (size=\(size)) — trying backup")
      }
    }
    ...
    if FileManager.default.fileExists(atPath: bakURL.path) {
      let size =
        (try? FileManager.default.attributesOfItem(atPath: bakURL.path)[.size] as? Int64) ?? 0
    ...
    if FileManager.default.fileExists(atPath: msgURL.path) {
      do {
        ...
        AppLog.general.error(
          "ChatService: cannot delete messages file for conversation \(id) — \(error.localizedDescription)"
        )
        throw error
      }
    }
