...
final class ContentPipelineService: ObservableObject {
  ...
  func process(
    _ itemID: UUID, using modelContext: ModelContext, forceReanalysis: Bool = false,
    extractionOnly: Bool = false
  ) {
    guard activeJobs[itemID] == nil else {
      AppLog.provider.info(
        "ContentPipeline: item \(itemID) already being processed, skipping duplicate call")
      return
    }
    ...
    activeJobs[itemID] = Task { @MainActor in
      ...
      if AutomationSettings.shared.autoTranscribe {
        if item.type == .audio, item.transcriptionEngineId == nil {
          ...
          do { try modelContext.save() } catch {
            AppLog.provider.error(
              "ContentPipeline: save failed (→transcribing): \(error.localizedDescription)")
          }
          ...
          if let transcribedText = await extractionSvc.extractTextFromAudio(item) {
            ...
            if let fresh = try? KnowledgeItemService(context: modelContext).fetchItem(id: itemID) {
              ...
              do { try modelContext.save() } catch {
                AppLog.provider.error(
                  "ContentPipeline: save failed (transcribe→pendingReview): \(error.localizedDescription)"
                )
              }
            }
          } else {
        ...
        if item.type == .image, item.bodyText == nil {
          ...
          if let ocrText = await extractionSvc.extractTextFromImage(item) {
            ...
            AppLog.provider.info(
              "ContentPipeline: extraction complete for item \(itemID) — \(ocrText.count) chars, vision=\(hasVision)"
            )
            NotificationCenter.default.post(
              name: .contentPipelineStageChanged, object: itemID.uuidString,
              userInfo: [
                "stage": "OCR done (\(ocrText.count) chars" + (hasVision ? " + vision)" : ")")
              ])
            ...
            if let fresh = try? KnowledgeItemService(context: modelContext).fetchItem(id: itemID) {
              ...
              do { try modelContext.save() } catch {
                AppLog.provider.error(
                  "ContentPipeline: save failed (OCR→pendingReview): \(error.localizedDescription)")
              }
            }
          } else {
      ...
      let project = item.projectID.flatMap { pid in
        try? ProjectService(context: modelContext).fetch(id: pid)
      }
      ...
      let agentMode: AgentMode =
        pipelineDef?.params?.agentMode == "deep"
        ? .deep : (pipelineDef?.params?.agentMode == "fast" ? .fast : .auto)

      let registry = AgentToolRegistry(tools: tools)
      let config = AIConfigService.shared
      let activeProviderConfig = ActiveProviderManager.shared.getActiveProvider(
      ...
      let resolvedAnalysisModel = config.resolvedModelFor(
        feature: "analysis", context: modelContext)
      ...
      while attemptCount < maxAttempts {
        ...
        do {
          for try await event in stream {
            ...
            case .toolCallCompleted(let name, let id, let summary):
              ...
              NotificationCenter.default.post(
                name: .contentPipelineStageChanged, object: itemID.uuidString,
                userInfo: [
                  "tool": name, "summary": summary, "events": agentEvents, "itemTitle": item.title,
                ])
            ...
            case .finished:
              AppLog.provider.info(
                "Pipeline agent completed for item \(itemID) on attempt \(attemptCount)")
              agentEvents.append(
                PipelineAgentEvent(
            ...
            case .truncated(let reason, let progress):
              AppLog.provider.warning(
                "Pipeline agent truncated for item \(itemID): \(reason) (\(progress))")
            ...
            case .error(let error):
              AppLog.provider.error(
                "Pipeline agent error [attempt \(attemptCount)]: \(error.localizedDescription)")
          ...
          AppLog.provider.error(
            "Pipeline agent stream error [attempt \(attemptCount)]: \(error.localizedDescription)")
          lastError = error.localizedDescription
          failed = true
        }
          ...
          if store.artifactExists(fileName: "analysis.json", meetingId: itemID) {
              ...
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
            {
              ...
              if let dd = dynamicData {
                try? dd.write(
                  to: store.itemDirectoryURL(for: itemID).appendingPathComponent(
                    AppFileConstants.dynamicAnalysisFileName))
              }
            }
            if !failed {
            ...
            AppLog.provider.warning(
              "Pipeline attempt \(attemptCount): agent finished but no analysis.json found")
            lastError =
        ...
        if !failed { break }
        AppLog.provider.warning(
          "Pipeline attempt \(attemptCount) failed, \(maxAttempts - attemptCount) remaining")
...
final class LensCatalogService {
  ...
  private var frameworkLenses: [Lens] {
    [
      Lens(
        id: "builtin/meeting", name: "Meeting Analysis",
        description: "Decisions, actions, risks, dates, entities", icon: "mic.fill",
        category: .domain, framework: FrameworkService.meetingFramework),
      Lens(
        id: "builtin/research", name: "Research",
        description: "Hypotheses, findings, themes, sources", icon: "magnifyingglass",
        category: .domain, framework: FrameworkService.researchFramework),
      Lens(
        id: "builtin/brainstorm", name: "Brainstorm",
        description: "Ideas, themes, questions, creative exploration", icon: "lightbulb.fill",
        category: .domain, framework: FrameworkService.brainstormFramework),
      Lens(
        id: "builtin/journal", name: "Journal",
        description: "Themes, mood, people, places, reflections", icon: "book.fill",
        category: .personal, framework: FrameworkService.journalFramework),
      Lens(
        id: "builtin/coaching", name: "Coaching",
        description: "Competencies, commitments, breakthroughs", icon: "figure.mind.and.body",
        category: .domain, framework: FrameworkService.coachingFramework),
      Lens(
        id: "builtin/legal", name: "Legal Brief",
        description: "Citations, statutes, depositions, privilege", icon: "building.columns.fill",
        category: .domain, framework: FrameworkService.legalFramework),
      Lens(
        id: "builtin/product", name: "Product Spec",
        description: "User stories, requirements, bugs, constraints", icon: "hammer.fill",
        category: .domain, framework: FrameworkService.productFramework),
      Lens(
        id: "builtin/blank", name: "Adaptive",
        description: "AI chooses sections based on content — no fixed template", icon: "doc.fill",
  ...
  func applyLens(_ lens: Lens, to project: Project) {
    if let fw = lens.framework {
      project.frameworkJSON = (try? JSONEncoder().encode(fw)).flatMap {
...
final class FrameworkService {
  ...
  static var meetingFramework: ProjectFramework {
    let schema = AnalysisOutputSchema(
      type: "object",
      properties: [
        "short_summary": SchemaProperty(
          type: "string", items: nil, properties: nil, description: "One-line summary"),
        "detailed_summary": SchemaProperty(
          type: "string", items: nil, properties: nil, description: "Detailed summary"),
          ...
          items: SchemaItems(
            ...
            properties: [
              "title": SchemaProperty(
                type: "string", items: nil, properties: nil, description: nil),
    ...
    return ProjectFramework(
      ...
      description:
        "Extracts decisions, action items, risks, open questions, dates, and entities from meeting content.",
      itemAnalysis: AnalysisConfig(
        ...
        renderAs: [
          FieldRenderer(
            field: "short_summary", type: .card, title: "Summary", icon: "text.alignleft"),
          FieldRenderer(
            field: "decisions", type: .list, title: "Decisions", icon: "checkmark.shield"),
          FieldRenderer(
            field: "action_items", type: .list, title: "Action Items", icon: "checklist"),
          FieldRenderer(
            field: "risks", type: .list, title: "Risks", icon: "exclamationmark.triangle"),
          FieldRenderer(
            field: "open_questions", type: .list, title: "Open Questions",
      ...
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "You are a project knowledge analyst. Analyze how this item relates to the project.",
      ...
      edgeTypes: [
        "supports", "contradicts", "references", "relates_to", "precedes", "mentions",
        "assigned_to",
      ]
    )
  }
  ...
  static var researchFramework: ProjectFramework {
    ProjectFramework(
      ...
      itemAnalysis: AnalysisConfig(
        ...
        outputSchema: AnalysisOutputSchema(
          ...
          properties: [
            ...
            "themes": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
      ...
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "You synthesize research projects. Identify emerging patterns, confirmed/refuted hypotheses, and gaps.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)
      ),
      views: [
        ViewDefinition(id: "items", title: "Items", type: .list, source: "items"),
        ViewDefinition(
          id: "hypotheses", title: "Hypotheses", type: .cards, source: "analysis.hypotheses"),
        ViewDefinition(id: "graph", title: "Graph", type: .graph, source: "edges"),
        ViewDefinition(id: "timeline", title: "Timeline", type: .timeline, source: "items"),
      ],
  ...
  static var brainstormFramework: ProjectFramework {
    ProjectFramework(
      ...
      name: "Brainstorm",
      description:
        "Captures ideas, clusters themes, and surfaces questions from brainstorming sessions.",
      itemAnalysis: AnalysisConfig(
        systemPrompt:
        ...
        outputSchema: AnalysisOutputSchema(
          ...
          properties: [
            ...
            "themes": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "Emerging themes"),
            "questions": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
        ...
        renderAs: [
          ...
          FieldRenderer(field: "themes", type: .chips, title: "Themes", icon: "tag"),
          FieldRenderer(
            field: "questions", type: .list, title: "Questions", icon: "questionmark.circle"),
        ]
      ),
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "You synthesize brainstorming projects. Identify dominant themes, idea clusters, and unexplored areas.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)
      ),
      views: [
  ...
  static var journalFramework: ProjectFramework {
    ProjectFramework(
      ...
      name: "Journal",
      description:
        "Personal journal with theme tracking, mood patterns, and cross-reference discovery.",
      itemAnalysis: AnalysisConfig(
        systemPrompt:
        ...
        outputSchema: AnalysisOutputSchema(
          ...
          properties: [
            ...
            "themes": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "Themes"),
            "people_mentioned": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "People mentioned"),
            "places": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
      ...
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "You synthesize personal journals. Identify recurring themes, mood patterns, and evolving perspectives.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)
      ),
      views: [
  ...
  static var coachingFramework: ProjectFramework {
    ProjectFramework(
      id: "builtin/coaching", name: "Coaching",
      description:
        "Tracks competency demonstrations, behavioral shifts, and session outcomes across coaching engagements.",
      itemAnalysis: AnalysisConfig(
        systemPrompt:
        ...
        outputSchema: AnalysisOutputSchema(
          ...
          properties: [
            "short_summary": SchemaProperty(
              type: "string", items: nil, properties: nil, description: "One-line session summary"),
            "competency_demonstrations": SchemaProperty(
              type: "array",
              items: SchemaItems(
                type: "object",
                properties: [
                  "competency": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "evidence": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "level": SchemaProperty(
                    type: "string", items: nil, properties: nil,
                    description: "demonstrated|developing|absent"),
            ...
            "commitments": SchemaProperty(
              ...
              items: SchemaItems(
                ...
                properties: [
                  "description": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "deadline": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "follow_through": SchemaProperty(
                    type: "string", items: nil, properties: nil,
                    description: "pending|in_progress|completed|abandoned"),
                ]), properties: nil, description: "Commitments made"),
            "aha_moments": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "Breakthrough insights"),
            "next_session": SchemaProperty(
              type: "string", items: nil, properties: nil,
              description: "What to prepare for next session"),
          ], required: ["short_summary"]),
        renderAs: [
          FieldRenderer(
            field: "short_summary", type: .card, title: "Summary", icon: "text.alignleft"),
          FieldRenderer(
            field: "competency_demonstrations", type: .list, title: "Competencies", icon: "star"),
          FieldRenderer(field: "commitments", type: .list, title: "Commitments", icon: "checklist"),
          FieldRenderer(
            field: "aha_moments", type: .chips, title: "Breakthroughs", icon: "lightbulb"),
        ]
      ),
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "Track competency growth, commitment completion rates, and emerging themes across coaching sessions.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)),
      views: [
        ViewDefinition(id: "sessions", title: "Sessions", type: .list, source: "items"),
        ViewDefinition(
          id: "competencies", title: "Competencies", type: .cards,
          source: "analysis.competency_demonstrations"),
  ...
  static var legalFramework: ProjectFramework {
    ProjectFramework(
      id: "builtin/legal", name: "Legal Brief",
      description:
        "Connects case citations, statutes, depositions, and party positions with full provenance chains.",
      itemAnalysis: AnalysisConfig(
        systemPrompt:
          ...
          properties: [
            ...
            "case_citations": SchemaProperty(
              ...
              items: SchemaItems(
                type: "object",
                properties: [
                  "case_name": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "citation": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "relevance": SchemaProperty(
                    type: "string", items: nil, properties: nil,
                    description: "supports|contradicts|distinguishes"),
            ...
            "deposition_quotes": SchemaProperty(
              ...
              items: SchemaItems(
                type: "object",
                properties: [
                  "quote": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "witness": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "significance": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                ]), properties: nil, description: "Key deposition quotes"),
            "statutes": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "Statutes referenced"),
            "privilege_concerns": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "Potential privilege issues"),
          ], required: ["short_summary"]),
        renderAs: [
          FieldRenderer(
            field: "short_summary", type: .card, title: "Summary", icon: "text.alignleft"),
          FieldRenderer(
            field: "case_citations", type: .list, title: "Cases", icon: "building.columns"),
          FieldRenderer(
            field: "deposition_quotes", type: .list, title: "Depositions", icon: "quote.bubble"),
          FieldRenderer(field: "statutes", type: .chips, title: "Statutes", icon: "book.pages"),
          FieldRenderer(
            field: "privilege_concerns", type: .list, title: "Privilege", icon: "lock.shield"),
        ]
      ),
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "Synthesize legal strategy. Identify conflicting testimonies, supporting precedents, and gaps in evidence.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)),
      views: [
        ViewDefinition(id: "documents", title: "Documents", type: .list, source: "items"),
        ViewDefinition(
          id: "cases", title: "Cases", type: .cards, source: "analysis.case_citations"),
        ViewDefinition(id: "timeline", title: "Timeline", type: .timeline, source: "items"),
        ViewDefinition(id: "graph", title: "Graph", type: .graph, source: "edges"),
      ],
  ...
  static var productFramework: ProjectFramework {
    ProjectFramework(
      id: "builtin/product", name: "Product Spec",
      description:
        "Connects user stories, requirements, constraints, and bugs with full traceability.",
      itemAnalysis: AnalysisConfig(
        systemPrompt:
          ...
          properties: [
            ...
            "user_stories": SchemaProperty(
              ...
              items: SchemaItems(
                type: "object",
                properties: [
                  "story": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "role": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "criteria": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: "Acceptance criteria"),
              ...
              items: SchemaItems(
                ...
                properties: [
                  ...
                  "priority": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: "P0/P1/P2/P3"),
                  "source": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
            ...
            "bugs": SchemaProperty(
              type: "array", items: SchemaItems(type: "string"), properties: nil,
              description: "Bug IDs referenced"),
            "design_decisions": SchemaProperty(
              type: "array",
              items: SchemaItems(
                type: "object",
                properties: [
                  "decision": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
                  "rationale": SchemaProperty(
                    type: "string", items: nil, properties: nil, description: nil),
        ...
        renderAs: [
          FieldRenderer(
            field: "short_summary", type: .card, title: "Summary", icon: "text.alignleft"),
          FieldRenderer(
            field: "user_stories", type: .list, title: "Stories", icon: "person.text.rectangle"),
          FieldRenderer(
            field: "requirements", type: .list, title: "Requirements", icon: "list.bullet.clipboard"
          ),
          FieldRenderer(field: "constraints", type: .chips, title: "Constraints", icon: "hammer"),
          FieldRenderer(
            field: "design_decisions", type: .list, title: "Decisions", icon: "paintpalette"),
        ]
      ),
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "Synthesize product specs. Identify requirement conflicts, missing criteria, and cross-component dependencies.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)),
      views: [
        ViewDefinition(id: "specs", title: "Specs", type: .list, source: "items"),
        ViewDefinition(
          id: "stories", title: "Stories", type: .cards, source: "analysis.user_stories"),
        ViewDefinition(
          id: "requirements", title: "Reqs", type: .table, source: "analysis.requirements"),
        ViewDefinition(id: "graph", title: "Deps", type: .graph, source: "edges"),
      ],
      entityKinds: ["story", "requirement", "constraint", "component", "bug"],
      edgeTypes: [
        "supports", "contradicts", "references", "relates_to", "precedes", "blockedBy", "produces",
      ]
    )
  }
  ...
  static var blankFramework: ProjectFramework {
    ProjectFramework(
      ...
      projectSynthesis: SynthesisConfig(
        systemPrompt:
          "Synthesize this project's items. Identify whatever patterns, themes, or insights are most relevant.",
        outputSchema: AnalysisOutputSchema(type: "object", properties: [:], required: nil)
      ),
      views: [
        ViewDefinition(id: "items", title: "Items", type: .list, source: "items"),
...
enum ProjectHealthEngine {
  ...
  static func compute(for projectID: UUID, context: ModelContext) -> HealthResult? {
    ...
    let allEdges =
      (try? edgeSvc.recentEdges(limit: 500))?.filter {
        itemIDs.contains($0.fromID) || itemIDs.contains($0.toID)
      } ?? []
    var decisionCount = 0
    var riskCount = 0
    ...
    let medianAge: Double =
      ...
      ? -1
      : (ages.count % 2 == 0
        ? (ages[ages.count / 2 - 1] + ages[ages.count / 2]) / 2 : ages[ages.count / 2])
    let entityCount = Set(allEdges.flatMap { [$0.fromID, $0.toID] }).count
    let graphDensity =
      entityCount > 1 ? Double(allEdges.count) / Double(entityCount * (entityCount - 1)) : 0
    ...
    let status =
      score >= 70 ? "healthy" : score >= 40 ? "stale" : score >= 30 ? "atRisk" : "dormant"
    var anomalies: [String] = []
    if medianAge < 0 {
      anomalies.append("No items in project")
    } else if medianAge > 7 {
      anomalies.append("Silence burst: \(Int(medianAge))d")
    }
    if decisionVelocity < 0.5 && items.count >= 3 { anomalies.append("Decision drought") }
    if actionDebtRatio > 0.7 && totalTasks > 0 {
      anomalies.append("Action debt: \(Int(actionDebtRatio*100))%")
    }
    ...
    if pendingSignals.count >= 5 {
      anomalies.append("Signal backlog: \(pendingSignals.count) pending")
    }
    return HealthResult(
      score: score, status: status, decisionVelocity: decisionVelocity,
      actionDebtRatio: actionDebtRatio, evidenceFreshnessDays: medianAge,
  ...
  static func updateProject(_ pid: UUID, context: ModelContext) {
    guard let r = compute(for: pid, context: context),
      let p = try? ProjectService(context: context).fetch(id: pid)
    else { return }
...
final class PromptStore: ObservableObject {
  ...
  func createPrompt(name: String, category: String, content: String, description: String? = nil)
    -> EditablePrompt
  {
  ...
  private func extractVariables(from template: String) -> [String] {
    ...
    let matches =
      pattern?.matches(in: template, range: NSRange(template.startIndex..., in: template)) ?? []
...
final class AgentMemoryStore: ObservableObject {
  ...
  func linkToItem(memoryId: UUID, itemId: UUID, context: ModelContext) {
    ...
    try? annotationService.upsert(
      [
        CapturedAnnotation(
          source: "agent_memory", key: "memory_id", value: memoryId.uuidString, confidence: 1.0),
        CapturedAnnotation(
          source: "agent_memory", key: "pattern", value: mem.pattern, confidence: nil),
        CapturedAnnotation(
          source: "agent_memory", key: "strategy", value: mem.strategy, confidence: nil),
...
enum ContentParser {
  ...
  private static func parseTable(from lines: [String], startIndex: Int) -> (
    TableBlock?, Int, [ParseError]
  ) {
    var errors: [ParseError] = []
    ...
    guard tableLines.count >= 2 else {
      errors.append(
        ParseError(
          ...
          reason: "Table needs at least 2 rows (header + separator)",
          suggestion: "Add a separator row: |---|---|"))
      ...
      if cells.count != headerCells.count {
        errors.append(
          ParseError(
            ...
            suggestion:
              "Each row must have exactly \(headerCells.count) cells matching: \(headerCells.joined(separator: " | "))"
          ))
      }
      rows.append(cells)
    }
      ...
      if parts.count >= 2 {
        ...
        for part in parts.dropFirst() {
          if part.hasPrefix("owner:") {
            owner = part.replacingOccurrences(of: "owner:", with: "").trimmingCharacters(
              in: .whitespaces)
          } else if part.hasPrefix("due:") {
            dueDate = part.replacingOccurrences(of: "due:", with: "").trimmingCharacters(
              in: .whitespaces)
          } else if part.hasPrefix("priority:") {
            priority = part.replacingOccurrences(of: "priority:", with: "").trimmingCharacters(
              in: .whitespaces)
          }
        }
      }
  ...
  private static func parseCodeBlock(from lines: [String], startIndex: Int) -> (CodeBlock?, Int) {
    ...
    let lang = lines[startIndex].replacingOccurrences(of: "```", with: "").trimmingCharacters(
      in: .whitespaces)
    ...
    return (
      CodeBlock(code: code, language: lang.isEmpty ? nil : lang, caption: nil), i - startIndex
    )
  }
...
struct SkillTemplate: Codable, Identifiable, Sendable {
  ...
  static let builtIn: [SkillTemplate] = [
    SkillTemplate(
      ...
      allowedTools: [
        "search_knowledge", "get_item", "list_items", "get_project", "get_connections",
        "get_analysis",
      ],
    ...
    SkillTemplate(
      ...
      systemPrompt:
        "You are a content extraction agent. Extract text from the item and return it. Do NOT run analysis.",
...
@objc @MainActor protocol WawaJSExports: JSExport {
  ...
  func createTask(_ title: String, owner: String?, due: String?, projectId: String?) -> [String:
    Any]
...
final class WawaJSBridge: NSObject, WawaJSExports {
  ...
  func getItemAnalysis(_ id: String) -> [String: Any]? {
    guard let uuid = UUID(uuidString: id),
      let analysis = try? fileStore.readArtifact(
        MeetingAnalysis.self, fileName: "analysis.json", meetingId: uuid)
    else { return nil }
    return [
  ...
  func getProjectTasks(_ projectId: String) -> [[String: Any]] {
    ...
    return tasks.map { t in
      [
        "id": t.id.uuidString, "title": t.title, "status": t.status.rawValue,
...
final class FieldAuthorityService {
  ...
  func firstBlockedField(
    attemptedFields: [String], model: any FieldProvidence, by origin: FieldOrigin
  ) -> String? {
...
final class VersioningService {
  ...
  func diff(between older: ProjectSnapshot, and newer: ProjectSnapshot, context: ModelContext)
    -> [ChangeRecord]
  {
    let all = (try? context.fetch(FetchDescriptor<ChangeRecord>())) ?? []
    return all.filter {
      ($0.snapshotID == newer.id || $0.snapshotID == nil) && $0.timestamp > older.createdAt
    }
  }
  ...
  func restore(to snapshot: ProjectSnapshot, context: ModelContext) {
    ...
    for (key, value) in lastValues {
      ...
      guard parts.count == 3, let eid = UUID(uuidString: String(parts[1])), let val = value else {
        continue
      }
      applyRestore(
    ...
    case "TaskItem":
      guard
        let task = try? context.fetch(
          FetchDescriptor<TaskItem>(predicate: #Predicate { $0.id == entityID })
        ).first
      else { return }
      switch field {
    ...
    case "KnowledgeItem":
      guard
        let item = try? context.fetch(
          FetchDescriptor<KnowledgeItem>(predicate: #Predicate { $0.id == entityID })
        ).first
      else { return }
      switch field {
...
final class OntologyInertiaService {
  ...
  func computeInertia(projectID: UUID, context: ModelContext) -> Double {
    ...
    let edgeDensity =
      items.count > 1 ? Double(projectEdges.count) / Double(items.count * (items.count - 1)) : 0
    ...
    return min(
      (itemFactor * 0.35 + ageFactor * 0.25 + densityFactor * 0.4) - contradictionPenalty, 1.0)
  }
...
final class PresetExportService {
  ...
  func builtInPresets() -> [Preset] {
    LensCatalogService.shared.allLenses.map { lens in
      var fwJSON: String?
      if let fw = lens.framework {
        fwJSON = (try? JSONEncoder().encode(fw)).flatMap { String(data: $0, encoding: .utf8) }
      }
      return Preset(
...
enum ConfigResolver {
  static func resolve(_ filename: String, projectSlug: String? = nil) -> Data? {
    ...
    let bundlePaths = [
      filename, "Pipelines/\(filename)", "Skills/\(filename)", "Schemas/\(filename)",
    ]
  ...
  static func resolve<T: Decodable>(_ filename: String, projectSlug: String? = nil, as type: T.Type)
    -> T?
  {
