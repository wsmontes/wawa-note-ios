...
enum ItemContextBuilder {
  ...
  static func buildItemContext(item: KnowledgeItem, fileStore: FileArtifactStore) -> String {
    ...
    ctx +=
      "Type: \(item.type.label) | Created: \(item.createdAt.formatted(date: .complete, time: .shortened))\n"
      ...
      if !analysis.actionItems.isEmpty {
        ctx +=
          ...
          + analysis.actionItems.map {
            "- \($0.task) (owner: \($0.owner ?? "unassigned"), confidence: \(Int(($0.confidence ?? 0) * 100))%)"
          }.joined(separator: "\n") + "\n"
      }
      if !analysis.risks.isEmpty {
        ctx +=
          "\nRisks:\n"
          + analysis.risks.map { "- \($0.risk)" + ($0.details.isEmpty ? "" : ": \($0.details)") }
          .joined(separator: "\n") + "\n"
      }
    ...
    } else if let dynamic = try? fileStore.readArtifact(
      DynamicAnalysis.self, fileName: AppFileConstants.dynamicAnalysisFileName, meetingId: item.id),
  ...
  static func buildItemSummary(item: KnowledgeItem, fileStore: FileArtifactStore) -> String {
    ...
    ctx +=
      "   UUID: \(item.id.uuidString) | Created: \(item.createdAt.formatted(date: .abbreviated, time: .omitted))\n"
    if let analysis = try? fileStore.readArtifact(
      MeetingAnalysis.self, fileName: "analysis.json", meetingId: item.id)
    {
      ctx += "   Summary: \(analysis.shortSummary)\n"
      ...
      if !analysis.actionItems.isEmpty {
        ctx +=
          "   Actions: \(analysis.actionItems.map { "\($0.task) (\($0.owner ?? "?"))" }.joined(separator: " | "))\n"
      }
      if !analysis.risks.isEmpty {
