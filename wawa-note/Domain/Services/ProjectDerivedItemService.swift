...
final class ProjectDerivedItemService {
  ...
  func markItemsForReprocessing(projectID: UUID, context reprocessContext: String, itemIDs: [UUID])
  {
    ...
    for itemID in itemIDs {
      do {
        try projectSvc.markForReprocessing(
          itemID: itemID, projectID: projectID, context: reprocessContext)
      } catch {
        AppLog.general.error(
          "ProjectDerivedItemService: failed to mark item \(itemID) for reprocessing: \(error)")
      }
    }
  }
}
