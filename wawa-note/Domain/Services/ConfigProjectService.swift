...
final class ConfigProjectService {
  ...
  private static func createConfigProject(context: ModelContext) -> Project? {
    ...
    do {
      project = try svc.create(
        ...
        summary:
          "App configuration: providers, prompts, settings, and agent memories. Edit files directly to customize Wawa Note."
      )
    } catch {
      AppLog.general.error(
        "ConfigProjectService: failed to create config project — \(error.localizedDescription)")
      return nil
    }
    ...
    project.intention =
      "System configuration project. Contains AI providers, prompt templates, app settings, and agent memories."
    project.intentionIsAutoGenerated = true
    project.isHidden = true
    context.safeSave(context: "config-project-create", itemId: project.id)
  ...
  static func syncConfigProject(context: ModelContext) {
    ...
    let configPrefixes = [
      "Provider: ", "Prompt: ", "App Settings", "Agent Memories", "Memory: ", "Skill: ", "Lens: ",
      "Model Preset: ",
    ]
    var existingByTitle: [String: KnowledgeItem] = [:]
    for item in existing
    where configPrefixes.contains(where: { item.title.hasPrefix($0) || item.title == $0 }) {
    ...
    func upsertItem(title: String, body: String, tags: [String]) {
      ...
      if let existingItem = existingByTitle[title] {
        ...
        let item = KnowledgeItem(
          type: .note, title: title, status: .draft,
    ...
    for config in configs {
      let keyPresent =
        config.apiKeyKeychainIdentifier.map { (try? keyStore.loadAPIKey(for: $0)) != nil } ?? false
      let dict: [String: Any] = [
        "name": config.name,
        "type": config.typeRaw,
    ...
    let settingsJSON =
      (try? JSONSerialization.data(withJSONObject: settingsDict, options: .prettyPrinted))
      .flatMap { String(data: $0, encoding: .utf8) } ?? "{}"
    ...
    for mem in memories {
      ...
      let memJSON =
        (try? JSONSerialization.data(withJSONObject: memDict, options: .prettyPrinted))
        .flatMap { String(data: $0, encoding: .utf8) } ?? "{}"
    ...
    for skill in skills {
      ...
      let skillJSON =
        (try? JSONSerialization.data(withJSONObject: skillDict, options: .prettyPrinted))
        .flatMap { String(data: $0, encoding: .utf8) } ?? "{}"
    ...
    for (lensId, lens) in lenses {
      ...
      let lensJSON =
        (try? JSONSerialization.data(withJSONObject: lensDict, options: .prettyPrinted))
        .flatMap { String(data: $0, encoding: .utf8) } ?? "{}"
    ...
    for (modelName, preset) in presets {
      ...
      let presetJSON =
        (try? JSONSerialization.data(withJSONObject: presetDict, options: .prettyPrinted))
        .flatMap { String(data: $0, encoding: .utf8) } ?? "{}"
      upsertItem(
        title: "Model Preset: \(modelName)", body: presetJSON, tags: ["config", "model-preset"])
    }
    ...
    // Single save
    context.safeSave(context: "config-sync-to-project")
    AppLog.event(
      "config",
      "Synced wawa-note-config: \(createdCount)c/\(changedCount)u/\(skippedCount)s — \(configs.count) providers, \(prompts.count) prompts, settings, \(memories.count) memories, \(skills.count) skills, \(lenses.count) lenses"
    )
  }
  ...
  static func buildSettingsSnapshot() -> [String: Any] {
    ...
    s["anarlogAutoExport"] =
      defs.object(forKey: UserDefaultsKey.anarlogAutoExport) as? Bool ?? false
  ...
  static func applyConfigFromProject(context: ModelContext) -> [String: Int] {
    ...
    var counts: [String: Int] = [
      "providers": 0, "prompts": 0, "settings": 0, "memories": 0, "errors": 0,
    ]
    ...
    for item in items {
      ...
      if item.title.hasPrefix("Prompt: ") {
          ...
          let name = meta["name"] as? String
        {
          ...
          if store.prompt(named: name) != nil {
            ...
            _ = store.createPrompt(
              name: name, category: meta["category"] as? String ?? "custom", content: content)
          }
          counts["prompts"]! += 1
        }
      }
      ...
      if item.title == "App Settings" {
        ...
        let defs = UserDefaults.standard
        if let v = json["autoTranscribe"] as? Bool {
          defs.set(v, forKey: UserDefaultsKey.autoTranscribe)
        }
        if let v = json["autoAnalyze"] as? Bool { defs.set(v, forKey: UserDefaultsKey.autoAnalyze) }
        if let v = json["autoAnalysisModel"] as? String {
          defs.set(v, forKey: UserDefaultsKey.autoAnalysisModel)
        }
        if let v = json["autoAnalysisProvider"] as? String {
          defs.set(v, forKey: UserDefaultsKey.autoAnalysisProvider)
        }
        if let v = json["voiceProcessing"] as? Bool {
          defs.set(!v, forKey: UserDefaultsKey.audioRawMode)
        }
        if let v = json["speakerphoneMode"] as? Bool {
          defs.set(v, forKey: UserDefaultsKey.audioSpeakerphoneMode)
        }
        if let v = json["developerModeEnabled"] as? Bool {
          defs.set(v, forKey: UserDefaultsKey.developerModeEnabled)
        }
        counts["settings"]! += 1
      }
    ...
    context.safeSave(context: "config-apply-from-project")
    AppLog.event(
      "config",
      "Applied from config project: \(counts["providers"]!)p/\(counts["prompts"]!)pr/\(counts["settings"]!)s/\(counts["memories"]!)m (\(counts["errors"]!) errors)"
    )
    return counts
  }
  ...
  static func importConfigBundle(_ bundle: [String: Any], context: ModelContext) -> [String: Int] {
    ...
    if let providers = bundle["providers"] as? [[String: Any]] {
      for p in providers {
        ...
        let existing =
          (try? context.fetch(FetchDescriptor<AIProviderConfigModel>())).flatMap { $0 } ?? []
        if existing.contains(where: { $0.name == name && $0.typeRaw == typeRaw }) { continue }
        let config = AIProviderConfigModel(
          name: name, type: ProviderType(rawValue: typeRaw) ?? .openAICompatible)
    ...
    if let prompts = bundle["prompts"] as? [[String: Any]] {
      for p in prompts {
        ...
        if store.prompt(named: name) != nil {
          ...
          _ = store.createPrompt(
            name: name, category: p["category"] as? String ?? "custom", content: content)
        }
        counts["prompts"]! += 1
      }
    }
    ...
    if let skills = bundle["skills"] as? [[String: Any]] {
      for s in skills {
        guard let name = s["name"] as? String,
          let displayName = s["displayName"] as? String
        else { continue }
      ...
      let decoded = try? JSONDecoder().decode([AgentMemory].self, from: data)
    {
      for mem in decoded {
        AgentMemoryStore.shared.write(
          pattern: mem.pattern, strategy: mem.strategy,
    ...
    context.safeSave(context: "config-import")
    AppLog.event(
      "config",
      "Imported config: \(counts["providers"]!)p/\(counts["prompts"]!)pr/\(counts["skills"]!)s/\(counts["memories"]!)m"
    )
    return counts
  }
}
