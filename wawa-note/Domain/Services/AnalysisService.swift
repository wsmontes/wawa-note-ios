...
final class AnalysisService: @unchecked Sendable {
  ...
  func analyze(
    ...
    AppLog.provider.info(
      "Analysis: source=\(sourceCtx.sourceType.rawValue), \(totalChars) chars, model \(model), chunk limit: \(maxChunk)"
    )

    if totalChars <= maxChunk {
      let body = segmentsText.joined(separator: "\n")
      let userPrompt =
        prefix + (configService.renderPrompt(for: "analysis", variables: ["transcript": body]))
      return try await singleAnalysis(
        provider: provider, model: model, systemPrompt: systemPrompt, userPrompt: userPrompt,
        meetingId: meetingId)
    }
    ...
    return try await mapReduceAnalysis(
      chunks: chunks, provider: provider, model: model, systemPrompt: systemPrompt,
      meetingId: meetingId, sourceContext: sourceCtx)
  }
  ...
  func analyzeDynamic(
    ...
    let segmentsText = transcript.segments.map {
      "[\($0.id.uuidString)|\(formatTime($0.startTime))] \($0.text)"
    }
    let body = segmentsText.joined(separator: "\n")
      ...
      let json = String(data: schemaData, encoding: .utf8)
    {
      ...
      schemaJSON =
        "{\"type\":\"object\",\"properties\":{\"short_summary\":{\"type\":\"string\"}},\"required\":[\"short_summary\"]}"
    }

    let userPrompt =
      "\(prefix)Analyze the following content. Return ONLY valid JSON matching this schema:\n\n\(schemaJSON)\n\nCONTENT:\n\(body)"
    ...
    let schemaId =
      sourceContext.sourceType == .recording
      ? "dynamic/recording" : "dynamic/\(sourceContext.sourceType.rawValue)"
    return DynamicAnalysis(
      itemId: meetingId, providerId: provider.id, model: model, schemaId: schemaId, results: results
    )
  }

  // MARK: - Direct (single request)

  private func singleAnalysis(
    provider: any AIProvider, model: String, systemPrompt: String, userPrompt: String,
    meetingId: UUID
  ) async throws -> MeetingAnalysis {
    let params = configService.requestParams(for: "analysis", model: model)
    let request = AIRequest(
  ...
  private func mapReduceAnalysis(
    chunks: [TextChunk], provider: any AIProvider, model: String, systemPrompt: String,
    meetingId: UUID, sourceContext: SourceContext
  ) async throws -> MeetingAnalysis {
    let chunkSummaries = await summarizeChunksWithLimit(
      chunks, provider: provider, model: model, sourceContext: sourceContext)
    ...
    if failedCount > 0 {
      AppLog.provider.warning(
        "Map phase: \(failedCount)/\(chunkSummaries.count) chunks failed after retries")
    }
  ...
  private func summarizeChunksWithLimit(
    _ chunks: [TextChunk], provider: any AIProvider, model: String, sourceContext: SourceContext
  ) async -> [(index: Int, value: String?)] {
  ...
  private func summarizeChunkWithRetry(
    _ chunk: TextChunk, index: Int, total: Int, provider: any AIProvider, model: String,
    sourceContext: SourceContext
  ) async -> String? {
    ...
    let request = AIRequest(
      ...
      messages: [
        AIMessage(
          ...
          content: [
            .text(
              "You are a concise \(summarizerLabel) summarizer. Return only the summary text, no JSON."
            )
          ]),
  ...
  private func sendWithRetry(provider: any AIProvider, request: AIRequest, maxRetries: Int)
    ...
    for attempt in 0...maxRetries {
      do {
        ...
        if retryable && attempt < maxRetries {
          ...
          AppLog.provider.warning(
            "Retrying after \(delay)s (attempt \(attempt + 1)/\(maxRetries), \(error))")
  ...
  private func parseResponse(
    _ content: String, meetingId: UUID, providerId: String, model: String, provider: any AIProvider
    ...
    AppLog.provider.warning(
      "Initial parse failed or empty shortSummary. Requesting JSON fix from provider...")

    if let parsed = await tryRetryWithFix(
      provider: provider, model: model, failedJSON: cleaned, meetingId: meetingId)
    {
      return buildAnalysis(from: parsed, meetingId: meetingId, providerId: providerId, model: model)
    }
    ...
    return buildFallback(
      rawContent: content, meetingId: meetingId, providerId: providerId, model: model)
  }

  /// Retry once with a "fix your JSON" prompt. Returns parsed DTO or nil.
  private func tryRetryWithFix(
    provider: any AIProvider, model: String, failedJSON: String, meetingId: UUID
    ...
    let request = AIRequest(
      ...
      messages: [
        AIMessage(
          ...
          content: [
            .text(
              "You are a JSON repair assistant. Output ONLY valid JSON. No markdown, no code fences."
            )
          ]),
    ...
    do {
      let response = try await provider.send(request)
      saveRawResponse(
        response.content, meetingId: meetingId, filename: "provider.response.fix_attempt.txt")
      let cleaned = ProviderAdapter.normalizeJSON(response.content)
      if let data = cleaned.data(using: .utf8),
        let parsed = tryDecode(AnalysisResponse.self, from: data)
      {
  ...
  private func buildAnalysis(
    from parsed: AnalysisResponse, meetingId: UUID, providerId: String, model: String
  ) -> MeetingAnalysis {
    MeetingAnalysis(
      ...
      decisions: parsed.decisions?.map {
        Decision(
          title: $0.title, details: $0.details ?? "",
          sourceSegmentIds: parseIDs($0.sourceSegmentIds), confidence: $0.confidence)
      } ?? [],
      actionItems: parsed.actionItems?.map {
        ActionItem(
          task: $0.task, owner: $0.owner, dueDate: parseDate($0.dueDate),
          sourceSegmentIds: parseIDs($0.sourceSegmentIds), confidence: $0.confidence)
      } ?? [],
      risks: parsed.risks?.map {
        Risk(
          risk: $0.risk, details: $0.details ?? "", sourceSegmentIds: parseIDs($0.sourceSegmentIds),
          confidence: $0.confidence)
      } ?? [],
      openQuestions: parsed.openQuestions?.map {
        OpenQuestion(
          question: $0.question, sourceSegmentIds: parseIDs($0.sourceSegmentIds),
          confidence: $0.confidence)
      } ?? [],
      importantDates: parsed.importantDates?.map {
        ImportantDate(
          date: $0.date, meaning: $0.meaning ?? "", sourceSegmentIds: parseIDs($0.sourceSegmentIds))
      } ?? [],
  ...
  private func buildFallback(rawContent: String, meetingId: UUID, providerId: String, model: String)
    -> MeetingAnalysis
  {
  ...
  func saveRawResponse(
    _ content: String, meetingId: UUID, fileStore: FileArtifactStore = FileArtifactStore(),
    filename: String = "provider.response.raw.txt"
  ) {
