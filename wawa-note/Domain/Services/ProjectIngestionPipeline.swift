...
final class ProjectIngestionPipeline: ObservableObject {
  ...
  func ingest(itemID: UUID, projectID: UUID, using modelContext: ModelContext) async {
    guard self.activeIngestionCount < self.maxConcurrentIngestions else {
      AppLog.provider.warning(
        "ProjectIngestion: rate limited (\(self.activeIngestionCount) active)")
      return
    }
  ...
  private func runIngestion(itemID: UUID, projectID: UUID, context: ModelContext) async {
    ...
    guard let provider = try? ProviderRouter.resolveActive(context: context) else {
      AppLog.provider.error(
        "ProjectIngestion: NO AI PROVIDER configured. Cannot enrich project \(projectID). Go to Settings to add one."
      )
    ...
    guard let project = try? projSvc.fetch(id: projectID),
      ...
      AppLog.provider.error(
        "ProjectIngestion: project \(projectID) or item \(itemID) NOT FOUND in SwiftData")
      postIngestionFailed(itemID: itemID, projectID: projectID, error: .projectNotFound)
      return
    }

    AppLog.provider.info(
      "ProjectIngestion: project=\(project.name), item=\(newItem.title), provider=\(provider.id)")
    ...
    if project.holdIngestionForDoubts {
      ...
      if !criticalDoubts.isEmpty {
        AppLog.provider.info(
          "ProjectIngestion: holding ingestion for project \(projectID) — \(criticalDoubts.count) critical doubt(s)"
        )
    ...
    let prompt = buildIngestionPrompt(
      projectContext: projectContext, newItemContext: itemContext, framework: framework)
    ...
    AppLog.provider.info(
      "ProjectIngestion: using model=\(model) for project \(project.name) (\(allItems.count) items)"
    )
    ...
    do {
      ...
      AppLog.provider.info(
        "ProjectIngestion: raw response (\(rawContent.count) chars): \(String(rawContent.prefix(500)))"
      )
      ...
      AppLog.provider.warning(
        "ProjectIngestion: initial parse failed. Retrying with JSON fix prompt...")

      let fixPrompt = buildFixPrompt(
        originalPrompt: prompt, rawContent: rawContent, framework: framework)
      ...
      let fixRequest = AIRequest(
        ...
        messages: [
          AIMessage(
            ...
            content: [
              .text(
                "You are a JSON repair assistant. Output ONLY valid JSON matching the requested schema."
              )
            ]),
      ...
      postIngestionFailed(
        ...
        error: .jsonParseFailed("All parse attempts failed after JSON fix retry"))
    } catch {
      AppLog.provider.error("ProjectIngestion: AI call failed after retries: \(error)")
      postIngestionFailed(
        itemID: itemID, projectID: projectID, error: .aiFailed(error.localizedDescription))
    }
  }
  ...
  private func buildProjectContext(
    ...
    if !tasks.isEmpty {
      ...
      for t in tasks {
        ...
        if let due = t.dueAt {
          ctx += " | due: \(due.formatted(date: .abbreviated, time: .omitted))"
        }
        ctx += "\n"
      }
      ctx += "\n"
    ...
    if !allEdges.isEmpty {
      ...
      for e in allEdges {
        ...
        let fromTitle =
          otherItems.first(where: { $0.id == e.fromID })?.title
          ?? String(e.fromID.uuidString.prefix(8))
        let toTitle =
          otherItems.first(where: { $0.id == e.toID })?.title ?? String(e.toID.uuidString.prefix(8))
  ...
  private func buildIngestionPrompt(
    projectContext: String, newItemContext: String, framework: ProjectFramework
  ) -> String {
    """
  ...
  private func buildFixPrompt(
    originalPrompt: String, rawContent: String, framework: ProjectFramework
  ) -> String {
    let edgeTypeList = framework.edgeTypes.joined(separator: "|")
    let schema = """
      {
  ...
  private func applyResults(
    ...
    if let newTasks = response.new_tasks {
      let existingTitles = Set(
        existingTasks.map { $0.title.lowercased().trimmingCharacters(in: .whitespaces) })
    ...
    let contribution =
      response.project_summary_contribution ?? response.project_summary_update ?? ""
    if !contribution.isEmpty {
      ...
      if auth.canModify(field: "summary", of: project, by: .llm) {
        ...
        do { try context.save() } catch {
          AppLog.provider.error(
            "ProjectIngestion: failed to save summary: \(error.localizedDescription)")
        }
      } else {
        let sugSvc = SuggestionGatingService(context: context)
  ...
  private func findTask(byTitle title: String, in tasks: [TaskItem]) -> TaskItem? {
    let trimmed = title.trimmingCharacters(in: .whitespacesAndNewlines)
    if let exact = tasks.first(where: {
      $0.title.localizedCaseInsensitiveCompare(trimmed) == .orderedSame
    }) {
      return exact
    }
  ...
  private func parseIngestionJSON(_ rawContent: String) -> IngestionResponse? {
    ...
    do {
      ...
      AppLog.provider.info(
        "ProjectIngestion: decoded — \(response.connections?.count ?? 0) connections, \(response.new_tasks?.count ?? 0) new tasks"
      )
      return response
    } catch {
      AppLog.provider.error(
        "ProjectIngestion: JSON decode failed: \(error). Raw: \(String(rawContent.prefix(300)))")
      return nil
    }
  }
  ...
  private func saveRawIngestionResponse(_ content: String, itemID: UUID, label: String) {
    ...
    try? content.data(using: .utf8)?.write(
      to: fileStore.itemDirectoryURL(for: itemID).appendingPathComponent(
        "project.ingestion.\(label).txt"))
  }
    ...
    for attempt in 0...maxRetries {
      do {
        ...
        if retryable && attempt < maxRetries {
          ...
          AppLog.provider.warning(
            "ProjectIngestion: retrying after \(delay)s (attempt \(attempt + 1)/\(maxRetries), \(error))"
          )
