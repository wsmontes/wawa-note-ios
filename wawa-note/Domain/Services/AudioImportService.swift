...
final class AudioImportService: @unchecked Sendable {
  ...
  func convertToAAC(inputURL: URL, outputURL: URL) async throws {
    ...
    status = ExtAudioFileGetProperty(
      inputFile, kExtAudioFileProperty_FileDataFormat, &propSize, &sourceFormat)
    ...
    status = ExtAudioFileSetProperty(
      inputFile, kExtAudioFileProperty_ClientDataFormat, propSize, &clientFormat)
    guard status == noErr else {
      throw ImportError.conversionFailed(
        NSError(
          domain: "import", code: -4,
          userInfo: [
            NSLocalizedDescriptionKey: "Cannot set input client format (OSStatus \(status))"
          ]))
    }
    ...
    status = ExtAudioFileCreateWithURL(
      outputURL as CFURL, kAudioFileM4AType, &outputFormat, nil, AudioFileFlags.eraseFile.rawValue,
      &outputFile)
    ...
    status = ExtAudioFileSetProperty(
      outputFile, kExtAudioFileProperty_ClientDataFormat, propSize, &clientFormat)
    guard status == noErr else {
      throw ImportError.conversionFailed(
        NSError(
          domain: "import", code: -6,
          userInfo: [
            NSLocalizedDescriptionKey: "Cannot set output client format (OSStatus \(status))"
          ]))
    }
