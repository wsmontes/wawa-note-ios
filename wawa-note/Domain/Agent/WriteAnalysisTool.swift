...
final class AnalysisSchemaStore {
  ...
  private static func builtInSchemas() -> [String: AnalysisSchema] {
    let raw: [(String, String, String, String, [String: Any])] = [
        ...
        [
          ...
          "contradictions": [
            ...
            "items": [
              "type": "object",
              "properties": [
                "statement_a": ["type": "string"], "statement_b": ["type": "string"],
      ...
      (
        "qa_transcript", "Q&A Transcript", "Q&A exchanges, interviews, structured conversations",
        "conversation",
        [
          "short_summary": ["type": "string"],
          "exchanges": [
      ...
      (
        "people_tracker", "People Tracker", "Attendees, commitments, sentiment, accountability",
        "meeting",
        [
          "short_summary": ["type": "string"],
          "attendees": [
      ...
      (
        "research_synthesis", "Research Synthesis", "Hypotheses, findings, sources, methodology",
        "research",
        [
          "short_summary": ["type": "string"],
          "hypotheses": [
            ...
            "items": [
              ...
              "properties": [
                ...
                "confidence": [
                  "type": "string", "enum": ["high", "medium", "low", "speculative"],
                ],
      ...
      (
        "timeline_narrative", "Timeline Narrative", "Chronological events, cause-effect chains",
        "analysis",
        [
          "short_summary": ["type": "string"],
          "events": [
      ...
      (
        "journal_personal", "Journal / Personal",
        "Personal reflection, mood, people, places, themes", "personal",
        [
          "short_summary": ["type": "string"],
      ...
      (
        ...
        "Key points, entities, relationships — for notes, articles", "general",
        [
          "short_summary": ["type": "string"],
          "key_points": ["type": "array", "items": ["type": "string"]],
          "entities": [
            ...
            "items": [
              ...
              "properties": [
                ...
                "type": [
                  "type": "string",
                  "enum": ["person", "organization", "tool", "concept", "place", "event"],
                ],
              ],
            ],
          ],
...
struct SelectSchemaTool: AgentTool {
  ...
  let parameters = AIToolParameters(
    properties: [
      "schema_name": AIToolProperty(
        type: "string", description: "Name of the schema to use (from catalog).")
    ],
  ...
  @MainActor func execute(_ arguments: [String: any Sendable], context: ToolContext) async throws
    -> ToolResult
  {
    guard let name = arguments["schema_name"] as? String else {
      return ToolResult(
        content: "Error: schema_name required", isError: true, displaySummary: "Missing")
    }
    guard let schema = AnalysisSchemaStore.shared.schema(named: name) else {
      return ToolResult(
        content: "Schema '\(name)' not found.\n\n\(AnalysisSchemaStore.shared.catalog())",
        isError: true, displaySummary: "Unknown")
    }
    ...
    return ToolResult(
      content: "Schema: \(schema.displayName)\nRequired: \(req)\nFields: \(fields)",
      displaySummary: "Schema: \(schema.displayName)")
  }
}
...
struct SelectSkillTool: AgentTool {
  ...
  let parameters = AIToolParameters(
    properties: [
      "skill_name": AIToolProperty(
        type: "string", description: "Name of the skill to use (from catalog).")
    ],
    required: ["skill_name"]
  )
    ...
    -> ToolResult
  {
    guard let name = arguments["skill_name"] as? String else {
      return ToolResult(
        content: "Error: skill_name required", isError: true, displaySummary: "Missing")
    }
  ...
  func execute(_ arguments: [String: any Sendable], context: ToolContext) async throws -> ToolResult
  {
    ...
    guard jsonStr.utf8.count <= Self.maxAnalysisSize else {
      ...
      AppLog.provider.error(
        "write_analysis: analysisJson too large — \(String(format: "%.1f", sizeMB)) MB (max 1 MB)")
      return ToolResult(
    ...
    guard let data = jsonStr.data(using: .utf8),
      ...
      return ToolResult(
        content:
          "Error: analysisJson is not valid JSON. Check for unescaped quotes, trailing commas, or missing braces. First 200 chars: \(preview)",
    ...
    do {
      ...
      let url = fileStore.itemDirectoryURL(for: itemId).appendingPathComponent(
        AppFileConstants.analysisFileName)
      ...
      guard let verifyData = try? Data(contentsOf: url),
        ...
        return ToolResult(
          content:
            "Error: analysis write verification failed — the file may be corrupted. Please retry.",
          isError: true, displaySummary: "Write verification failed")
      }

      AppLog.provider.info(
        "write_analysis: saved \(json.count) sections (\(prettyData.count) bytes) to \(url.path)")
  ...
  static func validateJSON(_ json: [String: Any], against schema: AnalysisSchema) -> [String]? {
    ...
    for (key, value) in json where key != "_metadata" {
      guard let prop = props[key] else {
        errors.append(
          "Unknown field '\(key)' — not in schema. Available fields: \(props.keys.sorted().joined(separator: ", "))"
        )
        continue
      }
      switch prop.type {
...
struct WriteSpeakersTool: AgentTool {
  ...
  func execute(_ arguments: [String: any Sendable], context: ToolContext) async throws -> ToolResult
  {
    guard let jsonStr = arguments["speakersJson"] as? String,
      ...
      return ToolResult(
        content: "Error: speakersJson must be valid JSON", isError: true,
        displaySummary: "Invalid JSON")
    }
    ...
    let enriched = json.merging([
      "_metadata": [
        "writtenBy": "WriteSpeakersTool", "timestamp": ISO8601DateFormatter().string(from: Date()),
      ]
    ]) { $1 }
    let prettyData = try JSONSerialization.data(
      withJSONObject: enriched, options: [.prettyPrinted, .sortedKeys])
  ...
  private func validateSpeakersSchema(_ json: [String: Any]) -> [String]? {
    ...
    for (i, sp) in speakers.enumerated() {
      ...
      if sp["resolved_to"] as? String == nil {
        errors.append("\(prefix).resolved_to: required string")
      }
      ...
      if sp["evidence_summary"] as? String == nil {
        errors.append("\(prefix).evidence_summary: required string")
      }
    }
    ...
    if let pending = json["pending_confirmations"] as? [[String: Any]] {
      for (i, pc) in pending.enumerated() {
        ...
        if pc["speaker_label"] as? String == nil {
          errors.append("\(prefix).speaker_label: required")
        }
        ...
        for (j, c) in candidates.enumerated() {
          if c["name"] as? String == nil {
            errors.append("\(prefix).candidates[\(j)].name: required")
          }
          if c["evidence"] as? String == nil {
            errors.append("\(prefix).candidates[\(j)].evidence: required")
          }
        }
