...
enum VFSService {
    ...
    private static func updateProjectFromJSON(_ project: Project, jsonText: String, context: ToolContext) throws {
        ...
        // Build ProjectUpdateFields for fields managed by ProjectService
        var fields = ProjectUpdateFields()
        if let v = json["name"] as? String { fields.name = v }
        if let v = json["summary"] as? String { fields.summary = v }
        if let v = json["intention"] as? String { fields.intention = v }
        if let v = json["customInstructions"] as? String { fields.customInstructions = v }
        if let v = json["colorHex"] as? String { fields.colorHex = v }
        if let v = json["iconName"] as? String { fields.iconName = v }
        if let v = json["status"] as? String, let status = ProjectStatus(rawValue: v) { fields.status = status }
        if let v = json["holdIngestionForDoubts"] as? Bool { fields.holdIngestionForDoubts = v }

        if fields.hasChanges {
            _ = try? ProjectService(context: context.modelContext).update(
                id: project.id, fields: fields, origin: .llm
            )
        }

        // Fields not yet in ProjectUpdateFields — system/internal
