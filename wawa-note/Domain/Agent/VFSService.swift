...
enum VFSService {
  ...
  static func resolve(_ raw: String?, context: ToolContext) -> VFSPath {
    ...
    case "inbox":
      ...
      if let matched = allItems.first(where: {
        $0.title.caseInsensitiveCompare(inboxItemStr) == .orderedSame
      }) {
      ...
      if candidates.count == 1 {
        return hasSubPath ? .inboxItemFile(id: candidates[0].id) : .inboxItem(id: candidates[0].id)
      }
    ...
    case "projects":
      ...
      case "tasks":
        ...
        if let id = UUID(uuidString: taskIDStr) {
          return .projectTask(projectSlug: dirName, projectID: pid, taskID: id)
        }
      ...
      case "analysis":
        if subParts.count > 1 {
          ...
          if let id = UUID(uuidString: idStr) {
            return .projectAnalysis(projectSlug: dirName, projectID: pid, itemID: id)
          }
    ...
    case "agent":
      ...
      case "chat":
        if parts.count > 2, let id = UUID(uuidString: parts[2]) {
          return .agentChatConversation(id: id)
        }
        return .agentChat
  ...
  static func projectMatches(_ project: Project, dirName: String) -> Bool {
    ...
    return safe == dirName || project.slug == dirName
      || project.name.caseInsensitiveCompare(dirName) == .orderedSame
  }
  ...
  private static func node(for vpath: VFSPath, context: ToolContext) -> VFSNode? {
    ...
    case .project(_, let pid):
      guard let p = try? ProjectService(context: context.modelContext).fetch(id: pid) else {
        return nil
      }
  ...
  private static func children(for vpath: VFSPath, context: ToolContext) -> [VFSNode] {
    ...
    case .projectEdges(let slug, let pid):
      ...
      return all.map { e in
        .file(
          path: "\(base)/\(e.id.uuidString).json",
          name: "\(e.edgeTypeRaw)-\(e.id.uuidString.prefix(8)).json",
          nodeType: .jsonFile, modifiedAt: e.createdAt,
          metadata: VFSNodeMetadata(swiftDataID: e.id, edgeType: e.edgeTypeRaw)
        )
      }
    ...
    case .projectSignals(let slug, let pid):
      ...
      return signals.map { s in
        .file(
          path: "\(base)/\(s.id.uuidString).json",
          name: "\(s.type)-\(s.id.uuidString.prefix(8)).json",
          nodeType: .jsonFile,
    ...
    case .configSchemas(let slug, _):
      ...
      return FrameworkService.allBuiltInFrameworks.sorted(by: { $0.key < $1.key }).map {
        (name, fw) in
        .file(
  ...
  private static func itemFileChildren(slug: String, itemID: UUID, context: ToolContext)
    -> [VFSNode]
  {
    guard let item = try? KnowledgeItemService(context: context.modelContext).fetchItem(id: itemID)
    else { return [] }
    let base: String
    if slug == "inbox" {
    ...
    if fm.fileExists(atPath: segmentsDir.path) {
      let segFiles =
        (try? fm.contentsOfDirectory(at: segmentsDir, includingPropertiesForKeys: [.fileSizeKey]))
        ?? []
      nodes.append(
        .directory(
      ...
      at: dir, includingPropertiesForKeys: [.fileSizeKey, .contentModificationDateKey])
    {
      for url in contents {
        ...
        if name.hasPrefix(scanPattern)
          && (name.hasSuffix(".jpg") || name.hasSuffix(".jpeg") || name.hasSuffix(".png"))
        {
    ...
    if fm.fileExists(atPath: exportsDir.path) {
      let exportContents =
        (try? fm.contentsOfDirectory(at: exportsDir, includingPropertiesForKeys: nil)) ?? []
  ...
  private static func makeItemDirNode(item: KnowledgeItem, path: String, context: ToolContext)
    -> VFSNode
  {
    let dir = context.fileStore.itemDirectoryURL(for: item.id)
    let fm = FileManager.default
    ...
    if fm.fileExists(atPath: dir.appendingPathComponent("analysis.dynamic.json").path) {
      fileCount += 1
    }
    let scanCount =
  ...
  private static func analysisFilesForItem(slug: String, itemID: UUID, context: ToolContext)
    -> [VFSNode]
  {
  ...
  private static func fileContent(for vpath: VFSPath, context: ToolContext) -> String? {
    ...
    case .projectTask(_, _, let taskID):
      guard let t = try? TaskService(context: context.modelContext).fetch(id: taskID) else {
        return nil
      }
      return formatTaskJSON(t)

    case .inboxItem(let id):
      guard let item = try? KnowledgeItemService(context: context.modelContext).fetchItem(id: id)
      else { return nil }
      return formatItemJSON(item, fileStore: context.fileStore)
  ...
  static func readItemFile(_ rawPath: String, context: ToolContext) -> String? {
    ...
    if isInbox {
      ...
      guard parts.count >= 5,
        ...
        let id = resolveItemID(from: parts[3], context: context)
          ?? UUID(uuidString: stripJSONSuffix(parts[3]))
      else { return nil }
      itemID = id
  ...
  private static func writeFileContent(_ text: String, to vpath: VFSPath, context: ToolContext)
    throws
  {
    switch vpath {
    case .project(let slug, let pid):
  ...
  static func writeItemFile(_ rawPath: String, content: String, context: ToolContext) throws {
    ...
    case "transcript.json":
      let url = context.fileStore.itemDirectoryURL(for: itemID).appendingPathComponent(
        "transcript.json")
  ...
  private static func deleteNode(_ vpath: VFSPath, context: ToolContext) throws {
    ...
    case .projectItemContents(_, _, let itemID), .inboxItemFile(let itemID):
      ...
      throw VFSError.cannotDelete(
        path:
          "Individual item files must be deleted by overwriting with empty content, or by deleting the parent item."
      )
    ...
    case .configProvider(let name):
      let configs =
        (try? context.modelContext.fetch(FetchDescriptor<AIProviderConfigModel>())) ?? []
  ...
  private static func updateProjectFromJSON(
    _ project: Project, jsonText: String, context: ToolContext
  ) throws {
    ...
    if let v = json["intention"] as? String {
      project.intention = v
    ...
    if let v = json["status"] as? String, let status = ProjectStatus(rawValue: v) {
  ...
  static func updateItemFromJSON(_ item: KnowledgeItem, jsonText: String, context: ToolContext)
    throws
  {
    guard let data = jsonText.data(using: .utf8),
      let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
    ...
    try? KnowledgeItemService(context: context.modelContext).updateItem(
      item, title: newTitle, bodyText: newBody, tags: newTags)
    ...
    if let v = json["scheduledDate"] as? String {
      item.scheduledDate = ISO8601DateFormatter().date(from: v)
    }
  ...
  static func updateTaskFromJSON(_ task: TaskItem, jsonText: String, context: ToolContext) throws {
    ...
    if let v = json["sourceItemID"] as? String, let sid = UUID(uuidString: v) {
      task.sourceItemID = sid
    }
    ...
    try? svc.updateTask(
      task, title: newTitle, ownerName: newOwner, priority: newPriority, dueAt: newDue)
  }
  ...
  private static func configProviderNodes(
    context: ToolContext, base: String = "/projects/wawa-note-config"
  ) -> [VFSNode] {
  ...
  static func fuzzyMatchUUID(
    prefix: String, in projectID: UUID, context: ToolContext, type: FuzzyEntityType
  ) -> UUID? {
  ...
  private static func matchTask(_ q: String, in pid: UUID, context: ToolContext) -> UUID? {
    let tasks = (try? TaskService(context: context.modelContext).tasks(for: pid)) ?? []
    if let m = tasks.first(where: { $0.title.caseInsensitiveCompare(q) == .orderedSame }) {
      return m.id
    }
  ...
  private static func matchItem(_ q: String, in pid: UUID, context: ToolContext) -> UUID? {
    let items = (try? ProjectService(context: context.modelContext).items(in: pid)) ?? []
    if let m = items.first(where: { $0.title.caseInsensitiveCompare(q) == .orderedSame }) {
      return m.id
    }
  ...
  static func itemToDict(_ item: KnowledgeItem, fileStore: FileArtifactStore, fields: [String]?)
    -> [String: Any]
  {
    var dict: [String: Any] = [
      "id": item.id.uuidString, "type": item.typeRaw, "title": item.title,
...
enum VFSError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .invalidField(let field, let value, let valid):
      return
        "Invalid value '\(value)' for field '\(field)'. Valid: \(valid.joined(separator: ", "))"
