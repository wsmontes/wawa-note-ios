...
struct ToolResult: Sendable {
  ...
  init(
    content: String, blocks: [ChatBlock]? = nil, citations: [ChatCitation] = [],
    isError: Bool = false, displaySummary: String? = nil
  ) {
    ...
    for (key, prop) in schema.properties {
      ...
      if let allowed = prop.enum {
        if let strValue = value as? String, !allowed.contains(strValue) {
          return
            "Parameter '\(key)' for tool '\(name)': value '\(strValue)' not in allowed values: \(allowed.joined(separator: ", "))"
        }
      }
    }
