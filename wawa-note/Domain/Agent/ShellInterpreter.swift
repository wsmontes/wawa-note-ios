...
enum ShellInterpreter {
  ...
  private static func dispatchPipe(_ cmd: ShellCommand, stdin: String, context ctx: ToolContext)
    -> ToolResult
  {
    switch cmd.name {
  ...
  private static func dispatch(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    case "man":
      ...
      if topic.isEmpty {
        ...
        result = shellErr(
          "man: no entry for '\(topic)'. Try 'man' without arguments to see available commands.")
      }
  ...
  private static func autoLsAfterCd(_ cdResult: ToolResult, _ ctx: ToolContext) -> ToolResult {
    ...
    let lsCmd = ShellCommand(
      name: "ls", args: [], flags: [:], redirectTarget: nil, redirectBody: nil)
    let listing = handleLs(lsCmd, ctx)
    let combined = "\(cdResult.content)\n\(listing.content)"
    return ToolResult(
      content: combined, citations: [], isError: false, displaySummary: cdResult.displaySummary)
  }
  ...
  static func tokenize(_ raw: String) -> ShellCommand {
    ...
    return ShellCommand(
      name: name, args: args, flags: flags, redirectTarget: redirectTarget,
      redirectBody: redirectBody, appendMode: appendMode)
  }
  ...
  private static func handleLs(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    case .projects:
      ...
      var lines = [
        "/projects/ (\(projects.count) project(s)) — use cd with the directory name", "",
      ]
      for p in projects {
        let taskCount = (try? TaskService(context: ctx.modelContext).tasks(for: p.id).count) ?? 0
        lines.append(
          "  \(VFSService.safeDirName(p))/    \"\(p.name)\"  [\(p.statusRaw)]  \(taskCount) tasks")
      }
      return ok(lines.joined(separator: "\n"))
    ...
    case .project(let slug, let pid):
      ...
      lines.append(
        "project.json   \(p.statusRaw.capitalized)  health=\(p.healthStatus ?? "N/A")  tasks=\(tasks.count)  items=\(items.count)"
      )
    ...
    case .projectItems(let slug, let pid):
      ...
      for item in filtered {
        ...
        let hasTrans = FileManager.default.fileExists(
          atPath: ctx.fileStore.itemDirectoryURL(for: item.id).appendingPathComponent(
            "transcript.json"
          ).path)
        ...
        cards.append(
          .itemCard(
            ItemCardData(
              itemID: item.id.uuidString, title: item.title, type: item.typeRaw,
              status: item.statusRaw, durationSeconds: item.durationSeconds, projectSlug: slug,
              hasTranscript: hasTrans, hasAnalysis: hasAnalysis)))
      }
      return ok(lines.joined(separator: "\n"), blocks: cards)

    case .projectTasks(let slug, let pid):
      var tasks = (try? TaskService(context: ctx.modelContext).tasks(for: pid)) ?? []
      if let status = statusFilter, status != "true" {
      ...
      if tasks.isEmpty {
        return ok(
          "No tasks",
          blocks: [.text("No tasks yet. Use touch tasks/ --title \"...\" to create one.")])
      }
      ...
      for t in tasks {
        ...
        let prioEmoji =
          t.priorityRaw == "critical"
          ? "🔴" : t.priorityRaw == "high" ? "🟠" : t.priorityRaw == "medium" ? "🔵" : "⚪"
    ...
    case .projectSignals(let slug, let pid):
      ...
      for s in signals {
        lines.append(
          "\(s.id.uuidString.prefix(8))  [\(s.type)]  \(s.title)  critical=\(s.isCritical)")
      }
      return ok(lines.joined(separator: "\n"))
    ...
    case .projectAnalysis(let slug, let pid, let itemID):
      if let iid = itemID {
        ...
        if isTranscript {
          ...
          return shellErr(
            "cat: analysis/\(iid.uuidString.prefix(8)).transcript.json: No transcript found")
        }
      ...
      for item in items {
        ...
        let hasAnalysis =
          ...
          || FileManager.default.fileExists(
            atPath: dir.appendingPathComponent("analysis.dynamic.json").path)
        ...
        if hasAnalysis || hasTranscript {
          fileEntries.append(
            (
              prefix: String(item.id.uuidString.prefix(8)), hasAnalysis: hasAnalysis,
    ...
    case .agentMemories:
      ...
      for m in memories {
        ...
        let fc = m.failCount
        lines.append(
          "\(m.id.uuidString.prefix(8))  pattern=\(m.pattern.prefix(40))  success=\(sc) fail=\(fc)")
      }
      return ok(lines.joined(separator: "\n"))
    ...
    case .agentChat:
      ...
      for c in conversations {
        ...
        lines.append(
          "  \(c.id.uuidString.prefix(8))  \(c.title)  msgs=\(c.messageCount)  \(preview.prefix(60))"
        )
      }
      return ok(lines.joined(separator: "\n"))
    ...
    case .projectPeople(let slug, let pid):
      ...
      for e in peopleEdges.prefix(limit) {
        ...
        lines.append(
          "  \(e.id.uuidString.prefix(8)).json  personID=\(e.toID.uuidString.prefix(8))  type=\(e.edgeTypeRaw)  since=\(ts)"
        )
      }
      return ok(lines.joined(separator: "\n"))
  ...
  private static func handleCat(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    case .project(let slug, let pid):
      ...
      if jsonOutput {
        let dict: [String: Any] = [
          "name": p.name, "slug": p.slug, "dirName": VFSService.safeDirName(p),
          "status": p.statusRaw,
      ...
      if let score = p.healthScore {
        lines.append("Health: \(Int(score * 100))% (\(p.healthStatus ?? "unknown"))")
      }
    ...
    case .projectItem(_, _, let itemID):
      ...
      guard let item = try? KnowledgeItemService(context: ctx.modelContext).fetchItem(id: itemID)
      else {
    ...
    default:
      return shellErr(
        "cat: cannot read directory. Use ls to list contents, then cat <file> to read")
    }
  }
  ...
  private static func handleCd(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    if target == "/projects" || target == "projects" {
      let lsCmd = ShellCommand(
        name: "ls", args: ["/projects"], flags: [:], redirectTarget: nil, redirectBody: nil)
    ...
    let absTarget =
      target.hasPrefix("/") ? target : "/projects/\(ctx.activeProjectSlug ?? "")/\(target)"
  ...
  private static func handleFind(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
      ...
      for t in tasks {
        ...
        cards.append(
          .taskCard(
            TaskCardData(
              taskID: t.id.uuidString, title: t.title, status: t.statusRaw, priority: t.priorityRaw,
              owner: t.ownerName, projectSlug: ctx.activeProjectSlug,
              needsConfirmation: t.statusRaw != "done")))
      }
      return ok(lines.joined(separator: "\n"), blocks: cards)
    }
    ...
    if let pslug = projectFilter {
      ...
      if let proj = allProjects.first(where: { VFSService.projectMatches($0, dirName: pslug) }) {
        results = results.filter { $0.projectID == proj.id }
      }
    }
    if sinceDays > 0 {
      results = results.filter {
        $0.createdAt >= Date().addingTimeInterval(-Double(sinceDays) * 86400)
      }
    }
    ...
    for item in results {
      let pn =
        item.projectID.flatMap { pid in
          (try? ProjectService(context: ctx.modelContext).fetch(id: pid)).map {
            VFSService.safeDirName($0)
          }
        } ?? "-"
      ...
      cards.append(
        .itemCard(
          ItemCardData(
            itemID: item.id.uuidString, title: item.title, type: item.typeRaw,
            status: item.statusRaw, durationSeconds: item.durationSeconds, projectSlug: pn,
  ...
  private static func handleGrep(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    if let target {
      ...
      case .projectAnalysis(_, _, let itemID):
        ...
        return ok(
          "grep: \(matches.count) match(es) for '\(query)'\n"
  ...
  private static func handleTouch(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    let fallbackTitle: String? = {
      ...
      if !stripped.isEmpty && stripped != "tasks" && stripped != "items"
        && UUID(uuidString: stripped) == nil
      {
    ...
    case .inbox, .projectItems, .projectItem:
      guard let t = effectiveTitle else {
        return shellErr("touch: --title is required. Or use: touch items/my-title.json")
      }
      guard let kt = KnowledgeItemType(rawValue: type) else {
        return shellErr(
          "touch: unknown type '\(type)'. Valid: audio, note, journalEntry, webBookmark, image")
      }
      // Validate type-specific requirements
      if kt == .image {
        return shellErr(
          "touch: image items cannot be created via shell. Use the Scan or Photo capture buttons in the app to create image items."
        )
      }
      ...
      guard
        let item = try? svc.createItem(
          type: kt, title: t, bodyText: body, tags: tags, inboxDate: Date())
      else {
      ...
      let proj = resolvedProjectID.flatMap {
        try? ProjectService(context: ctx.modelContext).fetch(id: $0)
      }
      if let p = proj {
    ...
    case .projects:
      ...
      guard let name = cmd.flags["name"] else {
        return shellErr("touch: --name is required to create a project")
      }
      let summary = cmd.flags["summary"]
      let project = try? ProjectService(context: ctx.modelContext).create(
    ...
    case .projectTasks, .projectTask:
      guard let t = effectiveTitle else {
        return shellErr("touch: --title is required. Or use: touch tasks/my-task-name.json")
      }
      guard let pid = ctx.activeProjectID else {
        return shellErr("touch: no active project. cd /projects/{slug} first")
      }
      ...
      if priority != prio.rawValue {
        ...
        AppLog.agent.warning(
          "touch: invalid priority '\(priority)' — valid: \(valid). Using medium.")
      }
    ...
    case .projectEdges:
      ...
      guard let fromStr = cmd.flags["from"],
        ...
        return shellErr(
          "touch edges/: --from <uuid> --to <uuid> required. --type <edgeType> optional.")
      }
    ...
    case .unknown(let msg):
      ...
      if let t = fallbackTitle, let pid = ctx.activeProjectID {
        ...
        return ok(
          "Created /projects/\(ctx.activeProjectSlug ?? "?")/tasks/\(task.id.uuidString.prefix(8)).json  (\(t) [\(priority)])"
        )
      }
      return shellErr(
    ...
    default:
      return shellErr(
        "touch: cannot create in this location. Use /inbox/ or /projects/{slug}/items/ or /projects/{slug}/tasks/"
      )
    }
  }
  ...
  private static func handleEcho(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    case .agentPrompts:
      ...
      if PromptStore.shared.prompt(named: promptName) != nil {
        ...
        _ = PromptStore.shared.createPrompt(
          name: promptName, category: category, content: content, description: description)
    ...
    case .projectAnalysis(_, _, let itemID):
      ...
      if let data = rawJSON.data(using: .utf8),
        (try? JSONSerialization.jsonObject(with: data)) != nil
      {
        let fileURL = ctx.fileStore.itemDirectoryURL(for: iid).appendingPathComponent(
          "analysis.json")
        do {
  ...
  private static func handleRm(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    case .projectItemContents(_, _, let itemID), .inboxItemFile(let itemID):
      return shellErr(
        "rm: cannot delete individual files inside items. Delete the parent item instead, or overwrite the file with empty content."
      )
    ...
    case .unknown(let msg):
      ...
      if target.contains("edges/"), let pid = ctx.activeProjectID {
        ...
        if let prefix = edgeIDStr,
          let edge = edges.first(where: { $0.id.uuidString.hasPrefix(prefix) })
        {
          try? gsvc.deleteEdge(edge)
    ...
    default:
      return shellErr(
        "rm: can only remove items, tasks, edges, prompts, memories, or conversations.")
    }
  }
  ...
  private static func handleHistory(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    guard let target = cmd.args.first else {
      ...
      let numbered = commandHistory.enumerated().map { "\($0.offset + 1): \($0.element)" }.joined(
        separator: "\n")
    ...
    case .project(_, let pid):
      let changes = VersioningService.shared.changes(
        for: pid, limit: limit, context: ctx.modelContext)
      ...
      for c in changes {
        ...
        lines.append(
          "[\(c.timestamp.formatted())] \(c.entityType).\(c.field): \(prev) → \(next)  [\(c.originRaw)]"
        )
      }
      return ok(lines.joined(separator: "\n"))
    ...
    case .projectItem(_, _, let itemID):
      let changes = VersioningService.shared.changes(
        for: itemID, limit: limit, context: ctx.modelContext)
    ...
    default:
      return shellErr(
        "history: can only show history for /projects/{slug}/project.json or items/{id}.json")
    }
  }
  ...
  private static func handleExtract(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    let extractSvc = ContentExtractionService(
      modelContext: ctx.modelContext, fileStore: ctx.fileStore)
  ...
  private static func handleAskUser(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    if isFreeText {
      ...
      return ToolResult(
        content: "[ASK_USER] \(question)", blocks: [block], citations: [], isError: false,
        displaySummary: "Asking (text): \(question.prefix(60))")
    }

    // Options mode: multiple choice
    if let opts = optionsStr {
      let choices = opts.components(separatedBy: ",").map {
      ...
      return ToolResult(
        content: "[ASK_USER] \(question)", blocks: [block], citations: [], isError: false,
        displaySummary: "Asking: \(question.prefix(60))")
    }
  ...
  private static func handleSemantic(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    guard cmd.args.count >= 1 else {
      return shellErr("semantic: usage: semantic \"query\" [--limit N]")
    }
    ...
    guard let provider = try? ProviderRouter.resolveActive(context: ctx.modelContext) else {
      return shellErr(
        "semantic: no AI provider configured. Semantic search requires an embedding model.")
    }
    // Run async search and return stub for now - agent can check back
    let svc = SemanticSearchService(fileStore: ctx.fileStore)
    Task {
      let results =
        (try? await svc.findRelevant(
          query: query, itemIDs: allItems.map(\.id), limit: limit, using: provider)) ?? []
  ...
  private static func handlePerson(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    if let calPermErr = ensureCalendarAccess() {
      ...
      let sixMonths = DateInterval(
        start: Date().addingTimeInterval(-180 * 86400), duration: 180 * 86400)
      ...
      for event in events {
        ...
        if title.contains(q) {
          ...
          calHits.append(
            "  [attendee] \"\(event.title)\" — \(formattedDate(event.startDate)) — \(matched)")
          total += 1
        }
    ...
    for item in allItems where item.type == .audio {
      guard
        let transcript = try? ctx.fileStore.readArtifact(
      ...
      if fullText.lowercased().contains(q) {
        ...
        transcriptHits.append(
          "  [\(tag)] \"\(item.title)\" \(item.createdAt.formatted(date: .numeric, time: .omitted)) — \"...\(snippet)...\""
        )
        total += 1
      }
    ...
    for item in allItems {
      ...
      guard
        let analysisData = try? ctx.fileStore.readArtifact(
          MeetingAnalysis.self, fileName: "analysis.json", meetingId: item.id)
      else { continue }
      // Check if person appears in analysis fields
      let decisionText = analysisData.decisions.map { $0.title + " " + ($0.details ?? "") }.joined(
        separator: " ")
      let actionText = analysisData.actionItems.map { $0.task + " " + ($0.owner ?? "") }.joined(
        separator: " ")
      let analysisText = analysisData.shortSummary + " " + decisionText + " " + actionText
      if analysisText.lowercased().contains(q) {
        memHits.append(
          "  [exata] \"\(query)\" citada em análise de \"\(item.title)\" — \(item.createdAt.formatted(date: .numeric, time: .omitted))"
        )
        total += 1
      }
    ...
    if total == 0 {
      return ok(
        "person \"\(query)\": Nenhum resultado encontrado em contacts, calendário, transcrições, chamadas ou memória interna."
      )
    }

    let header =
      "## person \"\(query)\" — \(total) resultados em \(lines.filter { $0.hasPrefix("###") }.count) fontes\n"
  ...
  private static func handlePersonShow(_ args: [String], _ ctx: ToolContext) -> ToolResult {
    ...
    if !contacts.isEmpty {
      ...
      for (cname, email, org, job, _) in contacts.prefix(1)
      where cname.lowercased() == name.lowercased() {
  ...
  private static func searchContacts(query: String) -> (
  ...
  private static func handleCal(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    switch sub {
    case "add":
      guard let title = cmd.flags["title"] else {
        return shellErr("cal add: --title required. --start (ISO8601), --end, --notes optional.")
      }
      ...
      do {
        let eventID = try svc.createEvent(
          title: title, startDate: start, endDate: end, notes: notes)
    ...
    case "list":
      ...
      let label =
        fromDate != nil
        ? "\(formattedDate(range.start)) → \(formattedDate(range.end))" : "next 7 days"
    ...
    case "attendees":
      ...
      for name in sorted {
        let count = events.filter { ($0.attendees ?? []).compactMap({ $0.name }).contains(name) }
          .count
    ...
    default:
      return shellErr(
        "cal: usage: cal list|find|attendees|add [--from YYYY-MM-DD] [--to YYYY-MM-DD]")
    }
  }
  ...
  private static func handleCleanup(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    guard let idStr = cmd.args.first, let itemID = UUID(uuidString: idStr) else {
      return shellErr(
        "cleanup: usage: cleanup <item-id>   Deletes the raw audio file for processed items to free disk space."
      )
    }
    guard let item = try? KnowledgeItemService(context: ctx.modelContext).fetchItem(id: itemID)
    else {
    ...
    guard item.type == .audio else {
      return shellErr("cleanup: only audio items have raw audio files to clean up.")
    }
    guard item.transcriptionEngineId != nil else {
      return shellErr("cleanup: item must be transcribed first. The raw audio is still needed.")
    }
    do {
  ...
  private static func handleProgress(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    return ToolResult(
      content: "[PROGRESS] Step \(step)/\(total): \(label)", blocks: [block], citations: [],
      isError: false, displaySummary: "Step \(step)/\(total)")
  }
  ...
  private static func handleVision(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    guard FileManager.default.fileExists(atPath: scanURL.path) else {
      return shellErr(
        "vision: no image found for item '\(item.title)'. It must have a scanned image (scan_0.jpg)."
      )
    }
    guard let provider = try? ProviderRouter.resolveActive(context: ctx.modelContext) else {
      return shellErr(
        "vision: no AI provider configured. Vision requires a provider with image support.")
    }
    let question =
      cmd.args.count > 1
    ...
    let visionTask = Task {
      do {
        ...
        let analysisData = try? JSONEncoder().encode([
          "question": question, "response": resultText, "timestamp": Date().ISO8601Format(),
        ])
        ...
        if saveAsNote, !resultText.isEmpty {
            ...
            tags: ["vision", "ai-analysis"], inboxDate: Date())
          {
            if let pid = item.projectID {
              try? ProjectService(context: ctx.modelContext).addItem(note.id, to: pid)
            }
  ...
  private static func handleExport(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    guard let target = cmd.args.first else {
      return shellErr("export: usage: export <item-id | project-id> [--format md|json]")
    }
      ...
      let item = try? KnowledgeItemService(context: ctx.modelContext).fetchItem(id: id)
    {
      ...
      return ok(
        "Exported to \(url.path)\n\n\(output.prefix(500))\(output.count > 500 ? "..." : "")")
    }
    // Try as project ID
    if let pid = UUID(uuidString: target),
      let project = try? ProjectService(context: ctx.modelContext).fetch(id: pid)
    {
  ...
  private static func handleHelp(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
    ...
    case "cd":
      return ok(
        "cd <path> — Change current directory. cd .. to go up, cd / for root.\nExamples: cd /projects/my-project, cd tasks/, cd /inbox"
      )
    case "cat":
      return ok(
    ...
    default:
      return ok(
        "Help topics: vfs, ls, cd, cat, find, grep, touch, echo, ask_user. Use 'help vfs' to see the full filesystem layout."
      )
    }
  }
  ...
  private static func checkSandbox(_ itemID: UUID, _ ctx: ToolContext) -> ToolResult? {
    ...
    return shellErr(
      "Access denied: item \(target) is outside the current analysis scope (\(allowed)). Only the item being analyzed can be accessed."
    )
  }
  ...
  private static func ok(_ content: String, blocks: [ChatBlock]? = nil) -> ToolResult {
    ToolResult(
      content: content, blocks: blocks, citations: [], isError: false,
      displaySummary: String(content.prefix(80)))
  }
  ...
  private static let knownCommands: Set<String> = [
    ...
    "grep", "wc", "man", "history", "help", "pwd", "mkdir", "ask_user", "plan", "assign",
    "progress", "cleanup", "person",
  ]
}
