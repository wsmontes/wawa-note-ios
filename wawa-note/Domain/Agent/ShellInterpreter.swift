...
enum ShellInterpreter {
        ...
        case .projectItems(let slug, let pid):
            ...
            for item in filtered {
                ...
                cards.append(
                    .itemCard(
                        ItemCardData(
                            itemID: item.id.uuidString, title: item.title, type: item.typeRaw, status: item.statusRaw ?? "draft",
                            durationSeconds: item.durationSeconds, projectSlug: slug, hasTranscript: hasTrans, hasAnalysis: hasAnalysis)))
            }
            return ok(lines.joined(separator: "\n"), blocks: cards)
        ...
        if case .projectTasks(_, let pid) = vpath {
            ...
            for t in tasks {
                ...
                cards.append(
                    .taskCard(
                        TaskCardData(
                            taskID: t.id.uuidString, title: t.title, status: raw, priority: t.priorityRaw ?? "medium", owner: t.ownerName,
                            projectSlug: ctx.activeProjectSlug, needsConfirmation: raw != "done")))
            }
            return ok(lines.joined(separator: "\n"), blocks: cards)
        }
        ...
        for item in results {
            ...
            cards.append(
                .itemCard(
                    ItemCardData(
                        itemID: item.id.uuidString, title: item.title, type: item.typeRaw, status: item.statusRaw ?? "draft",
                        durationSeconds: item.durationSeconds, projectSlug: pn, hasTranscript: false, hasAnalysis: false)))
        }
        return ok(lines.joined(separator: "\n"), blocks: cards)
    }
    ...
    private static func handleTouch(_ cmd: ShellCommand, _ ctx: ToolContext) -> ToolResult {
        ...
        case .inbox, .projectItems, .projectItem:
            ...
            guard
                let item = try? svc.createItem(
                    type: kt, title: t, bodyText: body, folderID: nil, durationSeconds: nil, languageCode: nil, tags: tags, inboxDate: Date())
            else {
        ...
        case .projectTasks, .projectTask:
            ...
            guard
                let task = try? ctx.services.derived.createTask(
                    title: t, projectID: pid, sourceItemID: nil, priority: prio,
                    ownerName: owner, dueAt: due, bodyJSON: nil)
            else {
