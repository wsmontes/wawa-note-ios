...
struct SynthesizeProjectTool: AgentTool {
    ...
    func execute(_ arguments: [String: any Sendable], context: ToolContext) async throws -> ToolResult {
        ...
        let sections: [SynthesisSection] =
            (arguments["sections"] as? [[String: Any]])?.enumerated().compactMap { idx, dict in
...
struct EmitSignalTool: AgentTool {
    ...
    var parameters: AIToolParameters {
        AIToolParameters(
            properties: [
                ...
                "signalType": AIToolProperty(type: "string", description: "risk, alert, opportunity, doubt, pattern, contradiction"),
    ...
    func execute(_ arguments: [String: any Sendable], context: ToolContext) async throws -> ToolResult {
        ...
        let body = SignalBody(
            ...
            relatedItemIDs: (arguments["relatedItemIDs"] as? [String])?.compactMap(UUID.init(uuidString:)),
