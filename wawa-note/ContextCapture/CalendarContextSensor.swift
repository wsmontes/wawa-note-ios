  ...
  func capture() async throws -> [CapturedAnnotation] {
    ...
    for event in events {
      ...
      if event.startDate <= now && event.endDate >= now {
        annotations.append(
          CapturedAnnotation(source: sensorName, key: "event_proximity", value: "during"))
        annotations.append(CapturedAnnotation(source: sensorName, key: "event_title", value: title))
        if let location = event.location, !location.isEmpty {
          annotations.append(
            CapturedAnnotation(source: sensorName, key: "event_location", value: location))
        }
      } else if now < event.startDate
        && event.startDate.timeIntervalSince(now) < Self.proximityThreshold
      {
        annotations.append(
          CapturedAnnotation(source: sensorName, key: "event_proximity", value: "before"))
        annotations.append(CapturedAnnotation(source: sensorName, key: "event_title", value: title))
      } else if now > event.endDate
        && now.timeIntervalSince(event.endDate) < Self.proximityThreshold
      {
        annotations.append(
          CapturedAnnotation(source: sensorName, key: "event_proximity", value: "after"))
        annotations.append(CapturedAnnotation(source: sensorName, key: "event_title", value: title))
      }
    }
