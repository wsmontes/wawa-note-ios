...
final class AudioRouteSensor: ContextSensor, @unchecked Sendable {
  ...
  func capture() async throws -> [CapturedAnnotation] {
    ...
    for output in outputs {
      annotations.append(
        CapturedAnnotation(source: sensorName, key: "route_type", value: output.portType.rawValue))
      annotations.append(
        CapturedAnnotation(source: sensorName, key: "route_name", value: output.portName))
    }
