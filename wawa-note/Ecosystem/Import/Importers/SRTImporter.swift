...
final class SRTImporter: FormatImporter, @unchecked Sendable {
  ...
  func importFromURL(_ url: URL) async throws -> ImportResult {
    ...
    let transcript = Transcript(
      meetingId: item.id, languageCode: nil, segments: segments, sourceEngineId: "srt_import")
    ...
    let transcriptURL = fileStore.itemDirectoryURL(for: item.id).appendingPathComponent(
      "transcript.json")
    return ImportResult(
      knowledgeItem: item, artifacts: ["transcript.json": transcriptURL], warnings: warnings)
  }
