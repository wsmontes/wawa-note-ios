...
final class MarkdownImporter: FormatImporter, @unchecked Sendable {
  ...
  func importFromURL(_ url: URL) async throws -> ImportResult {
      ...
      if let endRange = rest.range(of: "\n---\n") {
        ...
        for line in frontmatter.split(separator: "\n") {
          ...
          if let colonIdx = trimmed.firstIndex(of: ":") {
            ...
            var value = String(trimmed[trimmed.index(after: colonIdx)...]).trimmingCharacters(
              in: .whitespaces)
            ...
            case "tags":
              if value.hasPrefix("[") {
                tags = value.dropFirst().dropLast().split(separator: ",").map {
                  String($0).trimmingCharacters(in: .whitespaces).replacingOccurrences(
                    of: "\"", with: "")
                }
              }
            default: break
    ...
    if body.hasPrefix("# ") {
      ...
      let extractedTitle = String(firstLine).replacingOccurrences(of: "# ", with: "")
