...
struct MeetilyImporter: FormatImporter {
  ...
  private func isMeetilyJSON(_ content: String) -> Bool {
    guard
      let json = try? JSONSerialization.jsonObject(with: content.data(using: .utf8)!)
        as? [String: Any]
    else {
      return false
    }
  ...
  private func importJSON(_ data: Data, sourceURL: URL) async throws -> ImportResult {
    ...
    if !transcriptText.isEmpty {
      ...
      do {
        try fileStore.writeArtifact(
          transcriptData, fileName: "meetily_transcript.json", meetingId: item.id)
      } catch {
    ...
    do {
      ...
      let dest = fileStore.itemDirectoryURL(for: item.id).appendingPathComponent(
        "meetily_original.json")
    ...
    return ImportResult(
      knowledgeItem: item, artifacts: ["transcript": fileStore.itemDirectoryURL(for: item.id)],
      warnings: warnings)
  }
