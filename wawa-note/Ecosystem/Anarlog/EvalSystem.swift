...
struct EvalSystem {
    ...
    func validateSchema(_ jsonData: Data, against schema: ExpectedSchema) -> EvalResult {
        guard let json = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] else {
            return EvalResult(
                ...
                errors: [EvalError(field: "root", rule: "valid_json",
    ...
    private func validateField(_ field: ExpectedSchema.Field, value: Any?) -> (Bool, Int, [EvalError]) {
        ...
        case .array:
            ...
            if let minItems = field.minItems, arr.count < minItems {
                errors.append(EvalError(field: field.name, rule: "min_items",
        ...
        case .date:
            if let dateStr = value as? String {
                if ISO8601DateFormatter().date(from: dateStr) == nil {
                    errors.append(EvalError(field: field.name, rule: "date_format",
    ...
    func checkHallucinations(in text: String) -> [EvalError] {
        ...
        if lowercased.contains("as an ai") || lowercased.contains("as a language model") {
            errors.append(EvalError(field: "content", rule: "no_self_reference",
        ...
        if lowercased.contains("[insert") || lowercased.contains("[todo]") || lowercased.contains("[placeholder") {
            errors.append(EvalError(field: "content", rule: "no_placeholders",
        ...
        for i in 1..<lines.count where !lines[i].isEmpty {
            ...
            if repeatCount > 3 {
                errors.append(EvalError(field: "content", rule: "no_repetition",
    ...
    func validateAnalysis(_ json: [String: Any]) -> EvalResult {
        ...
        if let summary = json["short_summary"] as? String {
            ...
            checks.append(EvalCheck(name: "short_summary", passed: passed, score: passed ? 100 : 50,
                detail: "\(wordCount) words"))
            if !passed {
                errors.append(EvalError(field: "short_summary", rule: "length",
            ...
            errors.append(EvalError(field: "short_summary", rule: "required",
        ...
        if let decisions = json["decisions"] as? [[String: Any]] {
            ...
            checks.append(EvalCheck(name: "decisions", passed: passed, score: passed ? 100 : 60,
                detail: "\(withTitles)/\(decisions.count) have titles"))
            if !passed {
                errors.append(EvalError(field: "decisions", rule: "items_have_titles",
        ...
        if let actions = json["action_items"] as? [[String: Any]] {
            ...
            checks.append(EvalCheck(name: "action_items", passed: passed, score: passed ? 100 : 50,
                detail: "\(withTasks)/\(actions.count) have tasks"))
            if !passed {
                errors.append(EvalError(field: "action_items", rule: "items_have_tasks",
