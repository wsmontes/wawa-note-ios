...
struct EvalSystem {
  ...
  static let meetingSchema = ExpectedSchema(fields: [
    ExpectedSchema.Field(
      name: "short_summary", type: .string, required: true, minItems: nil, maxLength: 500,
      allowedValues: nil),
    ExpectedSchema.Field(
      name: "detailed_summary", type: .string, required: false, minItems: nil, maxLength: 5000,
      allowedValues: nil),
    ExpectedSchema.Field(
      name: "decisions", type: .array, required: false, minItems: nil, maxLength: nil,
      allowedValues: nil),
    ExpectedSchema.Field(
      name: "action_items", type: .array, required: false, minItems: nil, maxLength: nil,
      allowedValues: nil),
    ExpectedSchema.Field(
      name: "risks", type: .array, required: false, minItems: nil, maxLength: nil,
      allowedValues: nil),
    ExpectedSchema.Field(
      name: "open_questions", type: .array, required: false, minItems: nil, maxLength: nil,
      allowedValues: nil),
    ExpectedSchema.Field(
      name: "entities", type: .array, required: false, minItems: nil, maxLength: nil,
      allowedValues: nil),
  ])
  ...
  func checkHallucinations(in text: String) -> [EvalError] {
    ...
    if lowercased.contains("[insert") || lowercased.contains("[todo]")
      ...
      if !passed {
        errors.append(
          EvalError(
            ...
            message: "\(actions.count - withTasks) action items missing task descriptions",
            severity: .major))
      }
    }
...
extension EvalSystem {
  ...
  static func validatePipelineOutput(itemID: UUID, fileStore: FileArtifactStore) -> Bool {
    let eval = EvalSystem()
    let analysisURL = fileStore.itemDirectoryURL(for: itemID).appendingPathComponent(
      "analysis.json")
