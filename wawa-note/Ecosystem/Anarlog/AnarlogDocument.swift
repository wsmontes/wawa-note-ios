...
struct AnarlogDocument: Equatable {
  ...
  static func parse(from markdown: String) throws -> AnarlogDocument {
    ...
    let jsonData = try JSONSerialization.data(
      withJSONObject: jsonCompatible, options: [.sortedKeys])
...
private enum MinimalYAMLParser {
  ...
  private static func parseObject(lines: [String], index: inout Int, currentIndent: Int) throws -> (
    [String: Any], Int
  ) {
    ...
    while index < lines.count {
      ...
      let afterColon = trimmed[trimmed.index(after: colonIdx)...].trimmingCharacters(
        in: .whitespaces)
      ...
      if afterColon.isEmpty {
        ...
        if index < lines.count {
          ...
          if nextTrimmed.hasPrefix("- ") {
            ...
            let (obj, newIndex) = try parseObject(
              lines: lines, index: &index, currentIndent: indent + 2)
            index = newIndex
            result[key] = obj
          }
        }
  ...
  private static func parseArray(lines: [String], index: inout Int, baseIndent: Int) throws -> (
    [Any], Int
  ) {
    var result: [Any] = []
    ...
    while index < lines.count {
      ...
      if trimmed.hasPrefix("- ") {
        ...
        if value.isEmpty {
          ...
          let afterColon = value[value.index(after: colonIdx)...].trimmingCharacters(
            in: .whitespaces)
          ...
          if afterColon.isEmpty {
            ...
            index += 1
            let (restObj, newIndex) = try parseObject(
              lines: lines, index: &index, currentIndent: indent + 2)
            index = newIndex
            obj = restObj
            ...
            let (restObj, newIndex2) = try parseObject(
              lines: lines, index: &index, currentIndent: indent + 2)
            index = newIndex2
        ...
        let (obj, newIndex) = try parseObject(
          lines: lines, index: &index, currentIndent: arrayIndent + 2)
        index = newIndex
  ...
  private static func findKeyValueColon(in trimmed: String) -> String.Index? {
    ...
    for (i, c) in trimmed.enumerated() {
      if (c == "\"" || c == "'")
        && (i == 0 || trimmed[trimmed.index(trimmed.startIndex, offsetBy: i - 1)] != "\\")
      {
        if !inQuotes {
          inQuotes = true
          quoteChar = c
  ...
  private static func serializeScalar(_ value: Any) -> String {
    ...
    case let s as String:
      ...
      if s.isEmpty || s.contains(":") || s.contains("#") || s.contains("{") || s.contains("}")
        ...
        || s.contains("!") || s.contains("|") || s.contains(">") || s.contains("%")
        || s.contains("@") || s.contains("`") || s.contains(",") || s.hasPrefix(" ")
        || s.hasSuffix(" ") || s == "true" || s == "false" || s == "null" || s == "~"
      {
