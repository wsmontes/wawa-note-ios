...
struct AnarlogImporter: FormatImporter {
  ...
  private func isAnarlogDocument(_ content: String) -> Bool {
    ...
    return fm.participants != nil || fm.transcript != nil || fm.session != nil || fm.template != nil
      || fm.duration != nil
  }
  ...
  func importFromURL(_ url: URL) async throws -> ImportResult {
    ...
    if let transcript = fm.transcript, !transcript.segments.isEmpty {
      ...
      do {
        try fileStore.writeArtifact(
          anarlogTranscript, fileName: "transcript.json", meetingId: item.id)
      } catch {
    ...
    do {
      ...
      let destURL = fileStore.itemDirectoryURL(for: item.id).appendingPathComponent(
        "anarlog_original.md")
    ...
    return ImportResult(
      knowledgeItem: item,
      artifacts: [
        "transcript": fileStore.itemDirectoryURL(for: item.id).appendingPathComponent(
          "transcript.json")
      ],
      warnings: warnings
    )
