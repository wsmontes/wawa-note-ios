...
struct AnarlogExporter {
  ...
  func exportMarkdown(
    ...
    if let bodyText = item.bodyText, !bodyText.isEmpty {
      content = bodyText
    } else if let analysis = try? fileStore.readArtifact(
  ...
  func exportToFile(
    ...
    let markdown = try exportMarkdown(
      item: item, participants: participants, transcript: transcript)
  ...
  func exportToItemDirectory(
    ...
    try exportToFile(
      item: item, destination: destURL, participants: participants, transcript: transcript)
    return destURL
  }
