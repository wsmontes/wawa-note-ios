...
struct TranscriptPatchService {
    ...
    func applyResponse(_ rawResponse: String, to originalWords: [EditableWord]) throws -> PatchResult {
        ...
        logger.info("Transcript patch: \(corrections.count) corrections applied out of \(originalWords.count) words")
    ...
    func prepareRequest(from transcriptJSON: Data) throws -> (systemPrompt: String, userPrompt: String, wordCount: Int) {
    ...
    func processResponse(_ rawResponse: String, originalTranscriptJSON: Data) throws -> (correctedJSON: Data, corrections: [Correction]) {
