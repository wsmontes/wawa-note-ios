...
final class InstanceExportService {
        ...
        if let a = try? store.readArtifact(MeetingAnalysis.self, fileName: "analysis.json", meetingId: item.id) {
            analysis = AnalysisExport(
                ...
                actionItems: a.actionItems.map {
                    ActionItemExport(task: $0.task, owner: $0.owner, dueDate: $0.dueDate.map { iso.string(from: $0) }, confidence: $0.confidence)
                },
            ...
            transcript = TranscriptExport(
                ...
                segments: t.segments.map { seg in
                    TranscriptSegmentExport(
                        id: seg.id.uuidString, meetingId: seg.meetingId.uuidString, speakerId: seg.speakerId.map({ $0.uuidString }), text: seg.text,
                        startTime: seg.startTime, endTime: seg.endTime, confidence: seg.confidence)
                })
        }
        return ItemExportFull(
            id: item.id.uuidString, type: item.type.rawValue, title: item.title.precomposedStringWithCanonicalMapping,
    ...
    private func exportSignal(_ sig: AgentSuggestion) -> SignalExport {
        SignalExport(
            id: sig.id.uuidString, projectID: sig.projectID.map({ $0.uuidString }), type: sig.type, title: sig.title,
    ...
    private func exportFrame(_ f: ProjectFrame) -> FrameExport {
        FrameExport(
            id: f.id.uuidString, projectID: f.projectID.uuidString, parentFrameID: f.parentFrameID.map({ $0.uuidString }),
        ...
        (try? context.fetch(FetchDescriptor<KnowledgeItem>()))?.compactMap { item in
            ...
            if let a = try? store.readArtifact(MeetingAnalysis.self, fileName: "analysis.json", meetingId: item.id) {
                analysis = AnalysisExport(
                    ...
                    actionItems: a.actionItems.map {
                        ActionItemExport(
                            task: $0.task, owner: $0.owner, dueDate: $0.dueDate.map { ISO8601DateFormatter().string(from: $0) }, confidence: $0.confidence)
                    },
                    risks: a.risks.map { RiskExport(risk: $0.risk, details: $0.details, confidence: $0.confidence) },
                    openQuestions: a.openQuestions.map { QuestionExport(question: $0.question, confidence: $0.confidence) },
                    importantDates: a.importantDates.map { DateExport(date: $0.date, meaning: $0.meaning) },
            ...
            if let t = try? store.readArtifact(Transcript.self, fileName: "transcript.json", meetingId: item.id) {
                transcript = TranscriptExport(
                    ...
                    segments: t.segments.map { seg in
                        TranscriptSegmentExport(
                            id: sid(seg.id), meetingId: sid(seg.meetingId), speakerId: seg.speakerId.map(sid), text: seg.text, startTime: seg.startTime,
                            endTime: seg.endTime, confidence: seg.confidence)
                    })
            }
            return ItemExportFull(
                id: sid(item.id), type: item.type.rawValue, title: item.title.precomposedStringWithCanonicalMapping,
