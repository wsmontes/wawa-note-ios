...
struct ProjectExportService {
  ...
  func exportMarkdown(
    project: Project, items: [KnowledgeItem], tasks taskRows: [String], edges: [GraphEdge]
  ) -> String {
    var md = "# \(project.name)\n\n"
    ...
    md +=
      "**Items:** \(items.count) | **Tasks:** \(taskRows.count) | **Connections:** \(edges.count)\n\n---\n\n"
    ...
    if !items.isEmpty {
      ...
      for item in items {
        ...
        md +=
          "**Type:** \(item.type.label) | **Date:** \(item.createdAt.formatted(date: .abbreviated, time: .shortened))\n\n"
    ...
    if !edges.isEmpty {
      ...
      for edge in edges.prefix(50) {
        md +=
          "- \(edge.fromID.uuidString.prefix(8)) → [\(edge.edgeType.rawValue)] → \(edge.toID.uuidString.prefix(8))\n"
      }
      md += "\n"
    }
    return md
  ...
  func exportMarkdown(
    ...
    md += "**Created:** \(project.createdAt.formatted(date: .long, time: .shortened))\n"
    md +=
      "**Items:** \(items.count) | **Tasks:** \(tasks.count) | **Connections:** \(edges.count)\n\n"
    md += "---\n\n"
    ...
    if !tasks.isEmpty {
      ...
      for task in tasks {
        ...
        if let due = task.dueAt {
          md += " · Due: \(due.formatted(date: .abbreviated, time: .omitted))"
        }
        md += "\n"
      }
      md += "\n"
  ...
  func exportJSON(project: Project, items: [KnowledgeItem], tasks: [TaskItem], edges: [GraphEdge])
    throws -> Data
  {
  ...
  func exportGraph(edges: [GraphEdge], allItems: [KnowledgeItem], allTasks: [TaskItem]) throws
    -> Data
  {
  ...
  private func resolveNode(_ id: UUID, items: [KnowledgeItem], tasks: [TaskItem]) -> [(
    String, String, String
  )] {
    if let item = items.first(where: { $0.id == id }) {
      return [
        (item.id.uuidString, item.title.isEmpty ? "Untitled" : item.title, item.type.rawValue)
      ]
    }
...
struct InstanceExport: Codable {
  ...
  var projects: [ProjectExportFull]
  var items: [ItemExportFull]
  var tasks: [TaskExportFull]
  var people: [PersonExport]
  var entities: [EntityExport]
  var edges: [EdgeExportFull]
  var annotations: [AnnotationExport]
  var signals: [SignalExport]
  var frames: [FrameExport]
  var snapshots: [SnapshotExport]
  var changeHistory: [ChangeRecordExport]?
  var queue: [QueueEntryExport]
  var prompts: [PromptExport]
...
struct ProjectExportFull: Codable {
  ...
  var synthesisSourceItemID: String?
...
struct ItemExportFull: Codable {
  ...
  var dynamicAnalysis: DynamicAnalysisExport?
...
struct AnalysisExport: Codable {
  ...
  var decisions: [DecisionExport]
  var actionItems: [ActionItemExport]
  var risks: [RiskExport]
  var openQuestions: [QuestionExport]
  var importantDates: [DateExport]
  var entities: [EntityMentionExport]
}
...
struct DateExport: Codable {
  var date: String
  var meaning: String?
}
struct EntityMentionExport: Codable {
  var name: String
  var type: String
}
...
struct TranscriptExport: Codable {
  ...
  var segments: [TranscriptSegmentExport]
}
...
final class InstanceExportService {
  ...
  func buildItemExport(item: KnowledgeItem) -> ItemExportFull {
    ...
    var analysis: AnalysisExport?
    if let a = try? store.readArtifact(
      MeetingAnalysis.self, fileName: "analysis.json", meetingId: item.id)
    {
      analysis = AnalysisExport(
        id: a.id.uuidString, providerId: a.providerId, model: a.model,
        createdAt: iso.string(from: a.createdAt), shortSummary: a.shortSummary,
        detailedSummary: a.detailedSummary,
        decisions: a.decisions.map {
          DecisionExport(title: $0.title, details: $0.details, confidence: $0.confidence)
        },
        actionItems: a.actionItems.map {
          ActionItemExport(
            task: $0.task, owner: $0.owner, dueDate: $0.dueDate.map { iso.string(from: $0) },
            confidence: $0.confidence)
        },
        risks: a.risks.map {
          RiskExport(risk: $0.risk, details: $0.details, confidence: $0.confidence)
        },
        openQuestions: a.openQuestions.map {
          QuestionExport(question: $0.question, confidence: $0.confidence)
        },
    ...
    if let d = try? store.readArtifact(
      DynamicAnalysis.self, fileName: "analysis.dynamic.json", meetingId: item.id)
    {
      dynAnalysis = DynamicAnalysisExport(
        id: d.id.uuidString, providerId: d.providerId, model: d.model, schemaId: d.schemaId,
        resultsJSON: nil)
    }
    var transcript: TranscriptExport?
    if let t = try? store.readArtifact(
      Transcript.self, fileName: "transcript.json", meetingId: item.id)
    {
      transcript = TranscriptExport(
        meetingId: t.meetingId.map({ $0.uuidString }), languageCode: t.languageCode,
        sourceEngineId: t.sourceEngineId, createdAt: iso.string(from: t.createdAt),
        segmentCount: t.segments.count,
        segments: t.segments.map { seg in
          TranscriptSegmentExport(
            id: seg.id.uuidString, meetingId: seg.meetingId.uuidString,
            speakerId: seg.speakerId.map({ $0.uuidString }), text: seg.text,
            startTime: seg.startTime, endTime: seg.endTime, confidence: seg.confidence)
        })
    }
    return ItemExportFull(
      id: item.id.uuidString, type: item.type.rawValue, title: item.title,
      createdAt: iso.string(from: item.createdAt), updatedAt: iso.string(from: item.updatedAt),
      status: item.status.rawValue,
      tags: item.tags, bodyText: item.bodyText, projectID: item.projectID.map({ $0.uuidString }),
      folderID: item.folderID.map({ $0.uuidString }), isFlagged: item.isFlagged,
      inboxDate: item.inboxDate.map({ iso.string(from: $0) }),
      ...
      analysisProviderId: item.analysisProviderId,
      calendarEventIdentifier: item.calendarEventIdentifier,
      scheduledDate: item.scheduledDate.map({ iso.string(from: $0) }), isImported: item.isImported,
      importSourceURL: item.importSourceURL,
      contextCalendarEventTitle: item.contextCalendarEventTitle,
      contextPlaceName: item.contextPlaceName,
      ...
      contextMotionActivity: item.contextMotionActivity,
      contextBatteryLevel: item.contextBatteryLevel,
      fieldProvenanceJSON: item.fieldProvenanceJSON, analysis: analysis,
      dynamicAnalysis: dynAnalysis, transcript: transcript)
  }

  func exportSingleProject(_ project: Project, context: ModelContext) -> SingleProjectExport {
    let pid = project.id
    let items = (try? ProjectService(context: context).items(in: pid)) ?? []
    ...
    return SingleProjectExport(
      version: 1, schema: "wawa-note/project-export/v2",
      exportedAt: ISO8601DateFormatter().string(from: Date()),
  ...
  private func exportProjectFull(_ p: Project) -> ProjectExportFull {
    ProjectExportFull(
      id: p.id.uuidString, name: p.name, slug: p.slug, summary: p.summary,
  ...
  private func exportTaskFull(_ t: TaskItem) -> TaskExportFull {
    TaskExportFull(
      id: t.id.uuidString, projectID: t.projectID.map({ $0.uuidString }), title: t.title,
      status: t.status.rawValue, priority: t.priority.rawValue, ownerName: t.ownerName,
      dueAt: t.dueAt.map({ iso.string(from: $0) }),
      sourceItemID: t.sourceItemID.map({ $0.uuidString }),
  ...
  private func exportPerson(_ p: Person) -> PersonExport {
    PersonExport(
      id: p.id.uuidString, displayName: p.displayName, canonicalKey: p.canonicalKey,
      email: p.email, role: p.role, createdAt: iso.string(from: p.createdAt))
  }

  private func exportEntity(_ e: Entity) -> EntityExport {
    EntityExport(
      id: e.id.uuidString, kind: e.kind.rawValue, displayName: e.displayName,
      canonicalKey: e.canonicalKey)
  }

  private func exportEdgeFull(_ e: GraphEdge) -> EdgeExportFull {
    EdgeExportFull(
      id: e.id.uuidString, fromID: e.fromID.uuidString, toID: e.toID.uuidString,
      edgeType: e.edgeType.rawValue, weight: e.weight,
      provenanceItemID: e.provenanceItemID.map({ $0.uuidString }),
  ...
  private func exportSignal(_ sig: AgentSuggestion) -> SignalExport {
    SignalExport(
      id: sig.id.uuidString, projectID: sig.projectID.map({ $0.uuidString }), type: sig.type,
      title: sig.title,
      ...
      payloadJSON: sig.payloadJSON, createdAt: iso.string(from: sig.createdAt),
      resolvedAt: sig.resolvedAt.map({ iso.string(from: $0) }),
      impactScore: sig.impactScore, urgencyScore: sig.urgencyScore,
      relevanceScore: sig.relevanceScore,
      resolutionReason: sig.resolutionReason, resolvedBy: sig.resolvedByRaw,
      isCritical: sig.isCritical)
  }

  private func exportFrame(_ f: ProjectFrame) -> FrameExport {
    FrameExport(
      id: f.id.uuidString, projectID: f.projectID.uuidString,
      parentFrameID: f.parentFrameID.map({ $0.uuidString }),
      name: f.name, lensID: f.lensID, filterTags: f.filterTags,
      filterDateStart: f.filterDateStart.map({ iso.string(from: $0) }),
      filterDateEnd: f.filterDateEnd.map({ iso.string(from: $0) }),
  ...
  private func exportSnapshot(_ snap: ProjectSnapshot) -> SnapshotExport {
    SnapshotExport(
      id: snap.id.uuidString, projectID: snap.projectID.uuidString, label: snap.label,
      trigger: snap.trigger.rawValue, createdAt: iso.string(from: snap.createdAt),
      changeCount: snap.changeCount, summary: snap.summary)
  }
  ...
  func exportStatistics(context: ModelContext) -> ExportStatistics {
    ...
    return ExportStatistics(
      projectCount: projects.count, itemCount: items.count, taskCount: tasks.count,
      ...
      queueCount: queue.filter { $0.status == .queued || $0.status == .processing }.count,
  ...
  private func buildExport(context: ModelContext, includeHistory: Bool) throws -> InstanceExport {
    ...
    return InstanceExport(
      version: 1, schema: "wawa-note/instance-export-v1",
  ...
  private func exportProjects(context: ModelContext) -> [ProjectExportFull] {
    (try? context.fetch(FetchDescriptor<Project>()))?.map { p in
      ProjectExportFull(
        id: sid(p.id), name: p.name, slug: p.slug, summary: p.summary,
        synthesis: p.synthesis, customInstructions: p.customInstructions,
        frameworkId: p.frameworkId, frameworkJSON: p.frameworkJSON,
        status: p.status.rawValue, colorHex: p.colorHex, iconName: p.iconName,
  ...
  private func exportItems(context: ModelContext) -> [ItemExportFull] {
    (try? context.fetch(FetchDescriptor<KnowledgeItem>()))?.compactMap { item in
      ...
      {
        analysis = AnalysisExport(
          id: sid(a.id), providerId: a.providerId, model: a.model,
          createdAt: s(a.createdAt), shortSummary: a.shortSummary,
          detailedSummary: a.detailedSummary,
          decisions: a.decisions.map {
            DecisionExport(title: $0.title, details: $0.details, confidence: $0.confidence)
          },
          actionItems: a.actionItems.map {
            ActionItemExport(
              task: $0.task, owner: $0.owner,
              dueDate: $0.dueDate.map { ISO8601DateFormatter().string(from: $0) },
              confidence: $0.confidence)
          },
          risks: a.risks.map {
            RiskExport(risk: $0.risk, details: $0.details, confidence: $0.confidence)
      ...
      {
        dynAnalysis = DynamicAnalysisExport(
          id: sid(d.id), providerId: d.providerId, model: d.model, schemaId: d.schemaId,
          resultsJSON: nil)
      }
      var transcript: TranscriptExport?
      if let t = try? store.readArtifact(
      ...
      {
        transcript = TranscriptExport(
          meetingId: t.meetingId.map(sid), languageCode: t.languageCode,
          sourceEngineId: t.sourceEngineId, createdAt: s(t.createdAt),
          segmentCount: t.segments.count,
          segments: t.segments.map { seg in
            TranscriptSegmentExport(
              id: sid(seg.id), meetingId: sid(seg.meetingId), speakerId: seg.speakerId.map(sid),
              text: seg.text, startTime: seg.startTime, endTime: seg.endTime,
              confidence: seg.confidence)
          })
      }
      return ItemExportFull(
        id: sid(item.id), type: item.type.rawValue, title: item.title,
        ...
        analysisProviderId: item.analysisProviderId,
        calendarEventIdentifier: item.calendarEventIdentifier,
        scheduledDate: item.scheduledDate.map(s), isImported: item.isImported,
        importSourceURL: item.importSourceURL,
        contextCalendarEventTitle: item.contextCalendarEventTitle,
        contextPlaceName: item.contextPlaceName,
        contextAudioRoute: item.contextAudioRoute, contextLatitude: item.contextLatitude,
  ...
  private func exportTasks(context: ModelContext) -> [TaskExportFull] {
    (try? context.fetch(FetchDescriptor<TaskItem>()))?.map { t in
      TaskExportFull(
        id: sid(t.id), projectID: opt(t.projectID), title: t.title,
        status: t.status.rawValue, priority: t.priority.rawValue, ownerName: t.ownerName,
  ...
  private func exportPeople(context: ModelContext) -> [PersonExport] {
    (try? context.fetch(FetchDescriptor<Person>()))?.map { p in
      PersonExport(
        id: sid(p.id), displayName: p.displayName, canonicalKey: p.canonicalKey,
  ...
  private func exportEntities(context: ModelContext) -> [EntityExport] {
    (try? context.fetch(FetchDescriptor<Entity>()))?.map { e in
      EntityExport(
        id: sid(e.id), kind: e.kind.rawValue, displayName: e.displayName,
        canonicalKey: e.canonicalKey)
    } ?? []
  }
  ...
  private func exportEdges(context: ModelContext) -> [EdgeExportFull] {
    (try? context.fetch(FetchDescriptor<GraphEdge>()))?.map { e in
      EdgeExportFull(
        id: sid(e.id), fromID: sid(e.fromID), toID: sid(e.toID),
  ...
  private func exportAnnotations(context: ModelContext) -> [AnnotationExport] {
    (try? context.fetch(FetchDescriptor<Annotation>()))?.map { a in
      AnnotationExport(
        id: sid(a.id), source: a.source, key: a.key, value: a.value,
  ...
  private func exportSignals(context: ModelContext) -> [SignalExport] {
    (try? context.fetch(FetchDescriptor<AgentSuggestion>()))?.map { sig in
      SignalExport(
        id: sid(sig.id), projectID: opt(sig.projectID), type: sig.type, title: sig.title,
        body: sig.body, status: sig.status, confidence: sig.confidence,
        sourceItemID: opt(sig.sourceItemID), sourceSegmentIDs: sig.sourceSegmentIDs,
        payloadJSON: sig.payloadJSON, createdAt: s(sig.createdAt),
        resolvedAt: sig.resolvedAt.map({ s($0) }),
        impactScore: sig.impactScore, urgencyScore: sig.urgencyScore,
        relevanceScore: sig.relevanceScore,
        resolutionReason: sig.resolutionReason, resolvedBy: sig.resolvedByRaw,
        isCritical: sig.isCritical)
  ...
  private func exportFrames(context: ModelContext) -> [FrameExport] {
    (try? context.fetch(FetchDescriptor<ProjectFrame>()))?.map { f in
      FrameExport(
        id: sid(f.id), projectID: sid(f.projectID), parentFrameID: opt(f.parentFrameID),
        name: f.name, lensID: f.lensID, filterTags: f.filterTags,
  ...
  private func exportSnapshots(context: ModelContext) -> [SnapshotExport] {
    (try? context.fetch(FetchDescriptor<ProjectSnapshot>()))?.map { snap in
      SnapshotExport(
        id: sid(snap.id), projectID: sid(snap.projectID), label: snap.label,
        trigger: snap.trigger.rawValue, createdAt: s(snap.createdAt), changeCount: snap.changeCount,
  ...
  private func exportHistory(context: ModelContext) -> [ChangeRecordExport] {
    (try? context.fetch(FetchDescriptor<ChangeRecord>()))?.map { r in
      ChangeRecordExport(
        id: sid(r.id), entityType: r.entityType, entityID: sid(r.entityID),
  ...
  private func exportQueue(context: ModelContext) -> [QueueEntryExport] {
    ((try? context.fetch(FetchDescriptor<QueueEntry>())) ?? []).filter {
      $0.status == .queued || $0.status == .processing
    }.map { q in
      QueueEntryExport(
        id: sid(q.id), itemID: sid(q.itemID), projectID: opt(q.projectID),
        ...
        retryCount: q.retryCount, maxRetries: q.maxRetries, lastError: q.lastError,
        position: q.position)
    }
  }
  ...
  private func exportMemories() -> [MemoryExport] {
    AgentMemoryStore.shared.listAll().map { m in
      MemoryExport(
        id: m.id.uuidString, pattern: m.pattern, strategy: m.strategy,
  ...
  private func exportConfig() -> ConfigExport {
    ...
    return ConfigExport(
      activeProviderID: activeProviderID, preferredModel: preferredModel,
  ...
  func exportSRT(for itemId: UUID) -> String? {
    ...
    guard
      let transcript = try? store.readArtifact(
        Transcript.self, fileName: "transcript.json", meetingId: itemId),
