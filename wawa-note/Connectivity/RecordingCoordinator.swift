...
final class RecordingCoordinator: ObservableObject {
  ...
  /// Item IDs currently being restored from a crash checkpoint. Cleanup must
  /// not start a second, competing M4A rebuild for these recordings.
  private var recoveryInFlightIDs = Set<UUID>()
  ...
  func startRecording(
    ...
    guard self.state == .idle else {
      AppLog.warn(
        "audio",
        "RecordingCoordinator: startRecording called but state is \(String(describing: self.state))"
      )
      return
    }
    ...
    AppLog.event(
      "audio",
      "Recording requested — title=\(title ?? "nil") projectID=\(projectID?.uuidString.prefix(8) ?? "nil")"
    )
    ...
    do {
      ...
      AppLog.error(
        "audio", "Failed to save knowledge item for recording: \(error.localizedDescription)")
    ...
    Task { @MainActor [weak self] in
      ...
      do {
        ...
        AppLog.event(
          "audio",
          "Recording started — itemID=\(itemId.uuidString.prefix(8)) input=\(self.captureService.currentInputPortName)"
        )
  ...
  func deleteItem(_ itemId: UUID) {
    ...
    do {
      ...
      AppLog.storage.error(
        "RecordingCoordinator: deleteItem file removal failed — \(itemId.uuidString): \(error.localizedDescription)"
      )
    ...
    if let item = try? context.fetch(descriptor).first {
      ...
      do {
        ...
        AppLog.storage.error(
          "RecordingCoordinator: deleteItem SwiftData save failed — \(itemId.uuidString): \(error.localizedDescription)"
        )
      }
    }
  }
  ...
  func createItemFromImport(
    ...
    let item = KnowledgeItem(
      type: .audio, title: title, createdAt: date, updatedAt: date, status: .recorded,
  ...
  func stopRecording() {
    ...
    AppLog.event(
      "audio",
      "Stopping recording — elapsed=\(elapsedTimeFormatted) pausedDur=\(Int(pausedDuration))s itemID=\(itemId?.uuidString.prefix(8) ?? "nil")"
    )
  ...
  private func commitUIRecordingState() {
    ...
    activateLockScreenControls()
    nowPlayingController.update(
      title: recordingTitle, elapsedTime: elapsedTime - pausedDuration, isPlaying: true)
    notifyStatusChange()
  }
    ...
    Task { @MainActor [weak self] in
      ...
      for attempt in 1...3 {
        ...
        if attempt < 3 {
          ...
          AppLog.audio.info(
            "Resume attempt \(attempt) failed — retrying in \(delayNs / 1_000_000)ms")
  ...
  func onAppForeground() {
    ...
    AppLog.event(
      "audio", "App returned to foreground — attempting recovery from system interruption")
    try? captureService.resumeRecording()
    if captureService.state == .recording {
      state = .recording
      startObservation()
  ...
  func attemptCrashCheckpointRecovery() -> Bool {
    ...
    AppLog.audio.info(
      "Crash checkpoint found: meetingId=\(meetingId.uuidString.prefix(8)) segment=\(segmentIndex) sampleRate=\(sampleRate)Hz"
    )
    ...
    guard let item = try? bgContext.fetch(descriptor).first else {
      AppLog.audio.warning(
        "Crash recovery: item \(meetingId.uuidString.prefix(8)) not found — discarding checkpoint")
      AudioFileWriter.clearCrashCheckpoint()
      return false
    }
    ...
    guard item.statusRaw == "recording" else {
      AppLog.audio.info(
        "Crash recovery: item \(meetingId.uuidString.prefix(8)) already in state \(item.statusRaw) — discarding checkpoint"
      )
      AudioFileWriter.clearCrashCheckpoint()
      return false
    }
    ...
    recoveryInFlightIDs.insert(meetingId)
    Task { [weak self] in
      defer { self?.recoveryInFlightIDs.remove(meetingId) }
      let concatOK = await AudioSegmentConcatenator.concatenate(
  ...
  /// Checks whether a consolidated M4A can be decoded. MP4 atom placement is
  /// not fixed: a valid `moov` atom may appear after the first several KB, so
  /// inspecting a prefix falsely classified real recordings as broken.
  static func m4aNeedsRepair(at url: URL) -> Bool {
  ...
  func cleanupOrphanedRecordings() {
    ...
    do {
      ...
      let orphans = (try? bgContext.fetch(descriptor)) ?? []
      if !orphans.isEmpty {
      ...
      for item in recordedItems {
        // The recovered-item task below (or checkpoint recovery above) owns
        // this reconstruction. Starting a parallel export races on audio.m4a.
        guard !recoveredIds.contains(item.id), !recoveryInFlightIDs.contains(item.id) else {
          continue
        }
        let store = FileArtifactStore()
      ...
      try bgContext.save()
      AppLog.audio.info(
        "Recovered \(recoveredIds.count)/\(orphans.count) interrupted recording(s), repaired \(repairedIds.count) broken M4A(s)"
      )
  ...
  private func rollbackItem(_ item: KnowledgeItem, context: ModelContext) {
    ...
    do {
      ...
      AppLog.error(
        "audio", "Failed to rollback item after recording error: \(error.localizedDescription)")
    }
  }
  ...
  private func updateItemOnStop(
    ...
    if hasAudio {
      ...
      AppLog.audio.info(
        "Item finalized: \(item.id) hasAudio=true duration=\(effectiveDuration)s sampleRate=\(Int(sampleRate))Hz input=\(inputPortType)"
      )
    } else {
