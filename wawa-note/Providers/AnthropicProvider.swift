...
final class AnthropicProvider: AIProvider, @unchecked Sendable {
  ...
  private func buildRequestBody(_ request: AIRequest, stream: Bool = false) throws
    -> AnthropicRequest
  {
          ...
          if !textContent.isEmpty {
            blocks.append(
              AnthropicRequest.Message.ContentBlock(
                type: "text", text: textContent, id: nil, name: nil, inputJSON: nil, toolUseId: nil,
                content: nil))
          }
          for tc in tcs {
            blocks.append(
              AnthropicRequest.Message.ContentBlock(
                type: "tool_use", text: nil, id: tc.id, name: tc.name, inputJSON: tc.arguments,
        ...
        if msg.role == .tool {
          ...
          return [
            AnthropicRequest.Message(
              ...
              content: .blocks([
                AnthropicRequest.Message.ContentBlock(
                  type: "tool_result", text: nil, id: nil, name: nil, inputJSON: nil,
                  toolUseId: msg.toolCallId ?? "", content: toolResultText)
              ]))
          ]
        }
    ...
    let anthropicTools: [AnthropicRequest.AnthropicTool]? = request.tools?.map { tool in
      let props = tool.parameters.properties.mapValues { p in
        AnthropicRequest.AnthropicTool.InputSchema.PropertyDef(
    ...
    return AnthropicRequest(
      ...
      toolChoice: (anthropicTools != nil)
        ? AnthropicRequest.AnthropicToolChoice(type: "auto") : nil,
  ...
  func send(_ request: AIRequest) async throws -> AIResponse {
    ...
    let text = decoded.content.filter { $0.type == "text" }.compactMap(\.text).joined(
      separator: "\n")
    let thinkingText = decoded.content.filter { $0.type == "thinking" }.compactMap(\.text).joined(
      separator: "\n"
    ).nilIfEmpty
    ...
    return AIResponse(
      ...
      reasoningContent: thinkingText, usage: usage, toolCalls: toolCalls,
      ...
      Task {
        do {
          ...
          guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode)
            ...
            continuation.finish(
              throwing: ProviderError.apiError(
                statusCode: (response as? HTTPURLResponse)?.statusCode ?? 0, body: errorBody))
            return
          }
            ...
            case "content_block_delta":
              if let delta = obj["delta"] as? [String: Any] {
                  ...
                  let partial = delta["partial_json"] as? String
                {
                  ...
                  if currentToolID.isEmpty, let index = obj["index"] as? Int {
                    currentToolID = "tc_\(index)"
                  }
                  continuation.yield(
                    .toolCallDelta(id: currentToolID, name: currentToolName, arguments: partial))
                }
              }
