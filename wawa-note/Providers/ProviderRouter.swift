...
final class ProviderRouter: Sendable {
  ...
  func resolveWithFallback(
    context: ModelContext, preferredTypes: [ProviderType] = [.openAICompatible, .local]
  ) throws -> any AIProvider {
    let allConfigs = (try? context.fetch(FetchDescriptor<AIProviderConfigModel>())) ?? []

    for type in preferredTypes {
      if let config = allConfigs.first(where: {
        $0.type.normalizedForRouting == type.normalizedForRouting
      }) {
        do {
          return try provider(for: config)
        } catch {
          AppLog.provider.warning(
            "Provider \(config.name) failed: \(error.localizedDescription) — trying next")
          continue
        }
      }
    }
  ...
  func provider(for config: AIProviderConfigModel) throws -> any AIProvider {
    ...
    if config.type.requiresAPIKey {
      ...
      do { apiKey = try keychain.loadAPIKey(for: keychainId) } catch {
        throw ProviderError.missingAPIKey
      }
    } else {
      apiKey = ""
    ...
    case .openAI, .openAICompatible, .localNetwork, .appleLocal, .local:
      ...
      let endpointPath =
        AIConfigService.shared.config.providers[config.providerConfigId]?.endpoints?["chat"]
        ?? "chat/completions"
