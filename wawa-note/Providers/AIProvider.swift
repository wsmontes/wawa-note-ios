...
extension AIProvider {
  ...
  func fetchModels() async throws -> [String] { [] }
  func fetchModelInfos() async throws -> [ModelInfo] {
    try await fetchModels().map {
      ModelInfo(id: $0, created: nil, contextWindow: nil, isRecommended: false, recommendedFor: nil)
    }
  }
}
...
final class BudgetTracker: @unchecked Sendable {
  ...
  func recordSpend(tokens: Int, costUSD: Double? = nil) {
    ...
    AppLog.provider.info(
      "Budget: spent $\(String(format: "%.4f", cost)) (total today: $\(String(format: "%.4f", self.spentToday)))"
    )
...
final class MetricsHistoryStore: @unchecked Sendable {
  ...
  struct DailyAggregate: Codable {
    ...
    var avgTTFTMs: Double {
      requestCount > 0 && minTTFTMs != .infinity ? (minTTFTMs + maxTTFTMs) / 2 : 0
    }
  }
...
final class MetricsTracker {
  ...
  func build(promptTokens: Int? = nil, completionTokens: Int? = nil) -> ProviderMetrics {
    ...
    return ProviderMetrics(
      ...
      costUSD: totalCost > 0 ? totalCost : nil)
  }
}
...
struct UnifiedModelsResponse: Decodable {
  ...
  init(from decoder: any Decoder) throws {
    ...
    } else if let models = try? container.decode(
      [OllamaItem].self, forKey: DynamicKeys(stringValue: "models")!)
    {
      self.modelIDs = models.map {
        $0.name.hasPrefix("models/") ? String($0.name.dropFirst(7)) : $0.name
      }
    } else {
      self.modelIDs = []
    }
...
struct AIMessage: Identifiable, Sendable {
  ...
  init(
    id: UUID = UUID(), role: AIRole, content: [AIContentBlock], toolCalls: [AIToolCall]? = nil,
    ...
    AsyncThrowingStream { continuation in
      Task {
        do {
          ...
          let finish: AIFinishReason? = response.finishReason.flatMap {
            AIFinishReason(rawValue: $0)
          }
    ...
    for attempt in 0..<maxAttempts {
      do {
        ...
        AppLog.provider.warning(
          "Retry \(attempt + 1)/\(maxAttempts) after \(String(format: "%.1f", wait))s — \(error.userMessage.prefix(80))"
        )
...
final class LocalProviderScanner: @unchecked Sendable {
  ...
  private func probeLocalProvider(_ def: LocalProviderDef) async -> DiscoveredProvider? {
    ...
    AppLog.provider.info(
      "Local scan: found \(def.name) with \(models.count) models (latency: \(String(format: "%.0f", latencyMs ?? 0))ms)"
    )
  ...
  private func scanBonjour() async -> [DiscoveredProvider] {
    ...
    return await withCheckedContinuation {
      (continuation: CheckedContinuation<[DiscoveredProvider], Never>) in
      let browser = NWBrowser(for: .bonjour(type: "_http._tcp", domain: "local."), using: .tcp)
      browser.stateUpdateHandler = { s in
        if case .failed(let error) = s {
          AppLog.provider.debug("Bonjour failed: \(error.localizedDescription)")
        }
      }
          ...
          if case .service(let name, _, _, _) = result.endpoint {
            ...
            if known.contains(where: { lower.contains($0) }) {
              let port: Int =
                ...
                ? 11434 : (lower.contains("lmstudio") || lower.contains("lm-studio") ? 1234 : 8080)
