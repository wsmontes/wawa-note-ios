...
final class OpenAICompatibleProvider: AIProvider, @unchecked Sendable {
  ...
  init(
    id: String, displayName: String, providerType: ProviderType, baseURL: URL, apiKey: String,
    model: String,
  ...
  func send(_ request: AIRequest) async throws -> AIResponse {
    ...
    guard (200...299).contains(httpResponse.statusCode) else {
      ...
      if let reqBody = urlRequest.httpBody, let reqStr = String(data: reqBody, encoding: .utf8) {
        AppLog.provider.error(
          "Provider returned \(httpResponse.statusCode): \(bodyStr.prefix(300))")
        AppLog.provider.error(
          "Failing request body (last 1000 chars): \(String(reqStr.suffix(1000)))")
      }
    ...
    return AIResponse(
      ...
      reasoningContent: reasoningContent, usage: usage, toolCalls: toolCalls,
  ...
  func warmUp() async {
    ...
    do {
      ...
      AppLog.provider.warning(
        "Warm-up failed: \(error.localizedDescription) — model may load on first real request")
    }
  }
  ...
  func fetchOllamaModelDetail(model: String) async throws -> OllamaModelDetail {
    ...
    return OllamaModelDetail(
      quantization: quant, contextWindow: contextWindow, parameterSize: parameterSize,
      family: family)
  }
