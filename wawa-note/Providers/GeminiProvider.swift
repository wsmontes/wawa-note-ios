  ...
  func send(_ request: AIRequest) async throws -> AIResponse {
    ...
    let systemInstruction = systemText.map {
      GeminiRequest.SystemInstruction(parts: [
        GeminiRequest.Part(text: $0, functionCall: nil, functionResponse: nil)
      ])
    }

    // Non-system messages (Gemini uses "user" and "model" roles)
    let conversationMessages = request.messages.filter { $0.role != .system }
    let contents: [GeminiRequest.Content] = conversationMessages.flatMap {
      msg -> [GeminiRequest.Content] in
      ...
      if msg.role == .assistant, let tcs = msg.toolCalls, !tcs.isEmpty {
        ...
        if !textContent.isEmpty {
          parts.append(
            GeminiRequest.Part(text: textContent, functionCall: nil, functionResponse: nil))
        }
        for tc in tcs {
      ...
      if msg.role == .tool {
        ...
        let part = GeminiRequest.Part(
          ...
          functionResponse: GeminiRequest.Part.FunctionResponse(
            name: toolName, response: responseObj))
      ...
      return [
        GeminiRequest.Content(
          role: role,
          parts: [GeminiRequest.Part(text: textContent, functionCall: nil, functionResponse: nil)])
      ]
    }
    ...
    let geminiTools: [GeminiRequest.GeminiTool]? = request.tools?.map { tool in
      let props = tool.parameters.properties.mapValues { p in
        GeminiRequest.FunctionDeclaration.Parameters.PropertyDef(
    ...
    return AIResponse(
      id: nil, model: effectiveModel, content: text, usage: usage, toolCalls: toolCalls,
