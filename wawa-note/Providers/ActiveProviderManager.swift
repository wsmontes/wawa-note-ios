      ...
      if lower.hasPrefix("gpt-") || lower.hasPrefix("o1") || lower.hasPrefix("o3") {
        if provider.type == .openAI
          || (provider.type == .openAICompatible
            && provider.baseURLString?.contains("api.openai.com") == true)
        {
          return provider
        }
      }
...
final class ProviderHealthMonitor: @unchecked Sendable {
  ...
  private func checkProvider(_ config: AIProviderConfigModel, context: ModelContext) async {
    ...
    do {
      ...
      AppLog.provider.debug(
        "Health: \(config.name) → \(models.count) models, \(String(format: "%.0f", elapsed))ms")
    } catch {
      lock.withLock {
        health[id] = (status: .unhealthy, avgLatencyMs: 9999, lastCheck: Date())
      }
      AppLog.provider.warning(
        "Health: \(config.name) → UNHEALTHY: \(error.localizedDescription.prefix(80))")
  ...
  private func triggerFailover(context: ModelContext, failedProviderID: String) async {
    ...
    AppLog.provider.warning(
      "Failover: switched from \(failedProviderID.prefix(8)) to \(fallback.name) — previous provider unhealthy"
    )
  }
}
...
final class ProviderPool: @unchecked Sendable {
  ...
  func resolve(for task: ProviderTaskType) async throws -> (any AIProvider, AIProviderConfigModel) {
    let healthy = await MainActor.run {
      ...
      let primaryHealthy =
        pool.primary.map { monitor.status(for: $0.config.id.uuidString) != .unhealthy } ?? false
      let secondaryHealthy =
        pool.secondary.map { monitor.status(for: $0.config.id.uuidString) != .unhealthy } ?? false
