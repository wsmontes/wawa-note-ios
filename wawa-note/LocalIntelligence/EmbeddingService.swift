...
final class EmbeddingService: @unchecked Sendable {
  ...
  func embeddingURL(for itemId: UUID) -> URL {
    fileStore.itemDirectoryURL(for: itemId).appendingPathComponent(
      AppFileConstants.embeddingFileName)
  }

  func generateAndStore(for itemId: UUID, text: String, using provider: any AIProvider) async throws
    -> [Float]
  {
    ...
    AppLog.general.info(
      "Embedding stored for item \(itemId): \(vector.count) dims (v\(v), model=\(m))")
    return vector
  }
  ...
  func load(for itemId: UUID) -> [Float]? {
    ...
    if let container = try? JSONDecoder().decode(EmbeddingContainer.self, from: data) {
      if container.model != self.embeddingModel {
        let m = self.embeddingModel
        AppLog.general.info(
          "Embedding model mismatch for \(itemId): stored=\(container.model), current=\(m) — invalidating"
        )
        return nil
      }
    ...
    if let vector = try? JSONDecoder().decode([Float].self, from: data) {
      AppLog.general.info(
        "Legacy embedding for \(itemId): \(vector.count) dims — migrating to container format")
      let container = EmbeddingContainer(
  ...
  func hasEmbedding(for itemId: UUID) -> Bool {
    guard FileManager.default.fileExists(atPath: embeddingURL(for: itemId).path) else {
      return false
    }
